...
(function(cloudkid, undefined) {
  ...
  var Particle = function(emitter) {
    var art =
      emitter.particleImages[0] instanceof PIXI.Texture
  ...
  p.init = p.Particle_init = function() {
    ...
    this._doNormalMovement =
      this._doSpeed || this.startSpeed !== 0 || this.acceleration;
...
(function(cloudkid, undefined) {
   ...
   */
  p.applyArt = function(art) {
    if (Array.isArray(art)) this.textures = art;
    else {
...
(function(cloudkid, undefined) {
  ...
  Object.defineProperty(p, "parent", {
    ...
    set: function(value) {
      ...
      this._parent = value;
      this._parentIsPC =
        ParticleContainer && value && value instanceof ParticleContainer;
    }
  });
  ...
  p.init = function(particleImages, config) {
    ...
    this.particleImages =
      particleImages instanceof PIXI.Texture
        ? [particleImages]
        : particleImages;
    ...
    if (true && this.particleImages.length > 1) {
      for (var i = this.particleImages.length - 1; i > 0; --i) {
        if (
          this.particleImages[i].baseTexture !=
          this.particleImages[i - 1].baseTexture
        ) {
          if (window.console)
            console.warn(
  ...
  p.recycle = function(particle) {
    ...
    if (index < this._activeParticles.length - 1)
      this._activeParticles[index] = this._activeParticles[
        this._activeParticles.length - 1
      ];
        ...
        if (-this._spawnTimer < lifetime) {
          ...
          for (
            var len = Math.min(
              ...
              this.maxParticles - this._activeParticles.length
            );
            i < len;
            ++i
          ) {
            ...
            if (!this._parentIsPC || !p.parent) {
              ...
              if (index < 1) children.shift();
              else if (index == children.length - 1) children.pop();
