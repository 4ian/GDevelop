      ...
      switch (node.type) {
        ...
        case "Property":
          ...
          if (
            ...
            UglifyJS.is_identifier(key.value)
          ) {
            node.key = {
              type: "Identifier",
              name: key.value
            ...
            key.type === "Identifier" &&
            !UglifyJS.is_identifier(key.name)
          ) {
            node.key = {
              type: "Literal",
              value: key.name
        ...
        case "SequenceExpression":
          node.expressions = node.expressions.reduce(function flatten(
            list,
            expr
          ) {
            return list.concat(
              expr.type === "SequenceExpression"
                ? expr.expressions.reduce(flatten, [])
                : [expr]
            );
          },
          []);
...
module.exports = function(options) {
  ...
  for (var counter = 0; counter < options.iterations; counter++) {
    ...
    if (jsPair[0].value !== jsPair[1].value) {
      ...
      throw new Error(
        "Got different outputs, check out tmp/dump_*.{js,json} for codes and ASTs."
      );
    }
  }
