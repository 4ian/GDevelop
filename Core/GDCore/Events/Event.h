...
class GD_CORE_API BaseEvent {
 public:
  ...
  /**
   * \brief Get the persistent UUID, assigning a new one if missing. Identifies
   * an event across serialization, undo/redo and code generation (breakpoints).
   */
  const gd::String& GetOrCreatePersistentUuid();

  /**
   * \brief Get the persistent UUID (empty if the event was never assigned one).
   */
  const gd::String& GetPersistentUuid() const { return persistentUuid; }

  /**
   * \brief Set the persistent UUID (used when unserializing).
   */
  BaseEvent& SetPersistentUuid(const gd::String& persistentUuid_) {
    persistentUuid = persistentUuid_;
    return *this;
  }

  /**
   * \brief Assign a new random persistent UUID, discarding the previous one.
   */
  BaseEvent& ResetPersistentUuid();

  /**
   * \brief Clear the persistent UUID, so it no longer needs to be recognized
   * between serializations.
   */
  BaseEvent& ClearPersistentUuid();
  ///@}
 ...
 private:
  ...
  mutable gd::String persistentUuid;  ///< A persistent random version 4 UUID,
                                      ///< used to identify the event across
