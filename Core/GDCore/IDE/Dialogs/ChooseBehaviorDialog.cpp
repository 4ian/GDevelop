...
ChooseBehaviorDialog::ChooseBehaviorDialog(wxWindow* parent,
                                           Project& project_,
                                           gd::Layout& layout_,
                                           gd::String parentObject_,
                                           gd::String behaviorTypeAllowed_)
    : project(project_),
      layout(layout_),
  ...
  StaticBitmap3 =
      new wxStaticBitmap(Panel1,
                         ID_STATICBITMAP3,
                         wxBitmap(wxImage(_T("res/behavior64.png"))),
                         wxDefaultPosition,
                         wxDefaultSize,
                         wxNO_BORDER,
                         _T("ID_STATICBITMAP3"));
  ...
  StaticText1 = new wxStaticText(Panel1,
                                 ID_STATICTEXT1,
                                 _("Choose a behavior of the object."),
                                 wxDefaultPosition,
                                 wxDefaultSize,
                                 wxALIGN_CENTRE,
                                 _T("ID_STATICTEXT1"));
  ...
  FlexGridSizer1->Add(
      ...
      wxALL | wxEXPAND | wxALIGN_CENTER_HORIZONTAL | wxALIGN_CENTER_VERTICAL,
      0);
  behaviorsList = new wxListBox(this,
                                ID_LISTBOX1,
                                wxDefaultPosition,
                                wxSize(255, 128),
                                0,
  ...
  FlexGridSizer1->Add(
      ...
      wxALL | wxEXPAND | wxALIGN_CENTER_HORIZONTAL | wxALIGN_CENTER_VERTICAL,
      5);
  searchCtrl = new wxSearchCtrl(this,
                                ID_TEXTCTRL1,
                                wxEmptyString,
                                wxDefaultPosition,
                                wxDefaultSize,
  ...
  ChoisirBt = new wxButton(this,
                           ...
                           _T("ID_BUTTON1"));
  FlexGridSizer2->Add(
      ChoisirBt, 1, wxALL | wxALIGN_RIGHT | wxALIGN_CENTER_VERTICAL, 5);
  ...
  Connect(ID_LISTBOX1,
          ...
          (wxObjectEventFunction)&ChooseBehaviorDialog::OnChoisirBtClick);
  Connect(ID_TEXTCTRL1,
          wxEVT_COMMAND_TEXT_UPDATED,
          (wxObjectEventFunction)&ChooseBehaviorDialog::OnsearchCtrlText);
  Connect(ID_BUTTON1,
          wxEVT_COMMAND_BUTTON_CLICKED,
          (wxObjectEventFunction)&ChooseBehaviorDialog::OnChoisirBtClick);
  Connect(ID_BUTTON2,
          wxEVT_COMMAND_BUTTON_CLICKED,
          (wxObjectEventFunction)&ChooseBehaviorDialog::OnCancelBtClick);
  //*)
...
bool ChooseBehaviorDialog::DeduceBehavior() {
  if (behaviorsList->GetCount() == 1) {
    ...
    const BehaviorMetadata& metadata = MetadataProvider::GetBehaviorMetadata(
        project.GetCurrentPlatform(), behaviorTypeAllowed);

    if (metadata.GetFullName().empty())
      gd::LogMessage(
          _("This object doesn't have the appropriate behavior attached to "
            "it.\nCheck that you selected the right object or add the behavior "
            "in the object properties."));
    else {
      gd::LogMessage(wxString::Format(
          wxString(_("This object doesn't have behavior \"%s\" attached to "
                     "it.\nCheck that you selected the right object or add "
                     "this behavior in the object properties.")),
...
void ChooseBehaviorDialog::RefreshList() {
  ...
  std::vector<gd::String> behaviors =
      gd::GetBehaviorsOfObject(project, layout, parentObject);

  behaviorsList->Clear();
  for (std::size_t i = 0; i < behaviors.size(); ++i) {
    gd::String behaviorName = behaviors[i];

    if ((behaviorTypeAllowed.empty() ||
         behaviorTypeAllowed ==
             gd::GetTypeOfBehavior(project, layout, behaviorName)) &&
        (!searching ||
         (searching &&
          behaviorName.CaseFold().find(search.CaseFold()) != gd::String::npos)))
      behaviorsList->Append(behaviorName);
  }

  if (behaviorsList->GetCount() == 1) behaviorsList->SetSelection(0);
}
