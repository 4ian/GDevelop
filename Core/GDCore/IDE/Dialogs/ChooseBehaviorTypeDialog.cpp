...
ChooseBehaviorTypeDialog::ChooseBehaviorTypeDialog(wxWindow* parent,
                                                   gd::Project& project_)
    : project(project_) {
  ...
  StaticText1 = new wxStaticText(this,
                                 ID_STATICTEXT1,
                                 _("Choose the behavior to add to the object:"),
                                 wxDefaultPosition,
                                 wxDefaultSize,
                                 0,
                                 _T("ID_STATICTEXT1"));
  ...
  behaviorsList = new wxListCtrl(this,
                                 ID_LISTCTRL1,
                                 wxDefaultPosition,
                                 wxSize(277, 175),
  ...
  StaticText2 =
      new wxStaticText(this,
                       ID_STATICTEXT2,
                       _("You can also use the grayed behaviors:\ntheir "
  ...
  okBt = new wxButton(this,
                      ...
                      _T("ID_BUTTON1"));
  FlexGridSizer2->Add(
      okBt, 1, wxALL | wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 5);
  ...
  Connect(ID_LISTCTRL1,
          ...
          (wxObjectEventFunction)&ChooseBehaviorTypeDialog::
              OnbehaviorsListItemSelect);
  Connect(ID_LISTCTRL1,
          wxEVT_COMMAND_LIST_ITEM_ACTIVATED,
          (wxObjectEventFunction)&ChooseBehaviorTypeDialog::
              OnbehaviorsListItemActivated);
  Connect(
      ID_CHOICE1,
      wxEVT_COMMAND_CHOICE_SELECTED,
      (wxObjectEventFunction)&ChooseBehaviorTypeDialog::OnplatformChoiceSelect);
  Connect(ID_HYPERLINKCTRL2,
          wxEVT_COMMAND_HYPERLINK,
          (wxObjectEventFunction)&ChooseBehaviorTypeDialog::OnhelpBtClick);
  Connect(ID_BUTTON1,
          wxEVT_COMMAND_BUTTON_CLICKED,
          (wxObjectEventFunction)&ChooseBehaviorTypeDialog::OnokBtClick);
  Connect(ID_BUTTON2,
          wxEVT_COMMAND_BUTTON_CLICKED,
          (wxObjectEventFunction)&ChooseBehaviorTypeDialog::OncancelBtClick);
  Connect(wxEVT_SIZE,
          (wxObjectEventFunction)&ChooseBehaviorTypeDialog::OnResize);
  //*)

#if defined(__WXMSW__)  // Offer nice look to list
  wxUxThemeEngine* theme = wxUxThemeEngine::GetIfActive();
  if (theme)
    theme->SetWindowTheme((HWND)behaviorsList->GetHWND(), L"EXPLORER", NULL);
#endif
  ...
  wxConfigBase::Get()->Read(
      "ChooseBehaviorTypeDialog/Width", &width, wxDefaultCoord);
  wxConfigBase::Get()->Read(
      "ChooseBehaviorTypeDialog/Height", &height, wxDefaultCoord);
...
ChooseBehaviorTypeDialog::~ChooseBehaviorTypeDialog() {
  ...
  wxConfigBase::Get()->Write("ChooseBehaviorTypeDialog/Width",
                             GetSize().GetWidth());
  wxConfigBase::Get()->Write("ChooseBehaviorTypeDialog/Height",
                             GetSize().GetHeight());
}
    ...
    for (std::size_t j = 0; j < behaviorsTypes.size(); ++j) {
      if (!behaviorsTypes[j].empty()) {
        ...
        gd::TreeItemStringData* associatedData =
            new gd::TreeItemStringData(behaviorsTypes[j]);
        behaviorsList->InsertItem(index,
                                  extensions[i]
                                      ->GetBehaviorMetadata(behaviorsTypes[j])
                                      .GetFullName());
        behaviorsList->SetItem(index,
                               1,
                               extensions[i]
                                   ->GetBehaviorMetadata(behaviorsTypes[j])
                                   .GetDescription());
        ...
        if (!extensionEnabled)
          behaviorsList->SetItemTextColour(index, wxColor(128, 128, 128));
      }
    }
  }
}
...
void ChooseBehaviorTypeDialog::OnbehaviorsListItemActivated(
    wxListEvent& event) {
...
void ChooseBehaviorTypeDialog::OnokBtClick(wxCommandEvent& event) {
  ...
  for (std::size_t i = 0; i < extensions.size(); ++i) {
    std::vector<gd::String> behaviorsTypes = extensions[i]->GetBehaviorsTypes();
    if (find(behaviorsTypes.begin(),
             behaviorsTypes.end(),
             selectedBehaviorType) != behaviorsTypes.end()) {
...
void ChooseBehaviorTypeDialog::UpdateListColumnsWidth() {
  behaviorsList->SetColumnWidth(
      0, behaviorsList->GetSize().GetWidth() * 2.0 / 5.0 - 5);
  behaviorsList->SetColumnWidth(
      1, behaviorsList->GetSize().GetWidth() * 3.0 / 5.0 - 5);
}
...
void ChooseBehaviorTypeDialog::OnhelpBtClick(wxCommandEvent& event) {
  gd::HelpFileAccess::Get()->OpenPage(
      "game_develop/documentation/manual/edit_object");  // TODO: Behavior help
                                                         // page
}
...
bool ChooseBehaviorTypeDialog::ChooseAndAddBehaviorToObject(
    wxWindow* parent,
    gd::Project& project,
    gd::Object* object,
    gd::Layout* layout,
  ...
  if (dialog.ShowModal() == 1) {
    ...
    std::shared_ptr<gd::PlatformExtension> extension =
        std::shared_ptr<gd::PlatformExtension>();
    std::vector<std::shared_ptr<gd::PlatformExtension> > extensions =
        project.GetCurrentPlatform().GetAllPlatformExtensions();
    for (std::size_t i = 0; i < extensions.size(); ++i) {
      std::vector<gd::String> behaviorsTypes =
          extensions[i]->GetBehaviorsTypes();
      if (find(behaviorsTypes.begin(),
               behaviorsTypes.end(),
               dialog.GetSelectedBehaviorType()) != behaviorsTypes.end())
        extension = extensions[i];
    }
    gd::BehaviorMetadata metadata =
        extension->GetBehaviorMetadata(dialog.GetSelectedBehaviorType());
    ...
    for (std::size_t j = 0; j < project.GetUsedPlatforms().size(); ++j)
      project.GetUsedPlatforms()[j]->GetChangesNotifier().OnBehaviorAdded(
          project,
          isGlobalObject ? NULL : layout,
          *object,
          object->GetBehavior(autoName));
