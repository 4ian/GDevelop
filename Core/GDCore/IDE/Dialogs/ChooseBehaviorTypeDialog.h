...
/**
 * \brief Dialog displaying all the behaviors types available in a project,
 * allowing the user to choose one. \ingroup IDEDialogs
 */
class GD_CORE_API ChooseBehaviorTypeDialog : public wxDialog {
 public:
   ...
   */
  const gd::String& GetSelectedBehaviorType() const {
    return selectedBehaviorType;
  }

  /**
   * \brief Display the dialog and add the selected behavior to the object.
   */
  static bool ChooseAndAddBehaviorToObject(wxWindow* parent,
                                           gd::Project& project,
                                           gd::Object* object,
                                           gd::Layout* layout,
                                           bool isGlobalObject);
