...
/**
 * \brief Dialog used to choose an object (or a group) among project/layout
 * objects or groups.
 *
 * \ingroup IDEDialogs
 */
class GD_CORE_API ChooseObjectDialog : public wxDialog {
 public:
   ...
   * \param onlyObjectOfType if not empty, only objects of this type will be
   * displayed \param allowMultipleSelection if true, more than one object can
   ...
   */
  const std::vector<gd::String>& GetChosenObjects() const {
 ...
 private:
  ...
  std::vector<gd::String>
      objectsChosen;  ///< Used if dialog support multiple selection.
