...
ChooseObjectTypeDialog::ChooseObjectTypeDialog(wxWindow* parent,
                                               gd::Project& project_)
    : project(project_) {
  ...
  StaticText2 = new wxStaticText(this,
                                 ID_STATICTEXT2,
                                 _("Choose the kind of object to add:"),
                                 wxDefaultPosition,
                                 wxDefaultSize,
                                 0,
                                 _T("ID_STATICTEXT2"));
  ...
  objectsList = new wxListCtrl(
      this,
      ID_LISTCTRL1,
      wxDefaultPosition,
      wxDefaultSize,
      wxLC_REPORT | wxLC_ALIGN_TOP | wxLC_NO_HEADER | wxLC_SINGLE_SEL,
      wxDefaultValidator,
      _T("ID_LISTCTRL1"));
  FlexGridSizer1->Add(
      objectsList,
  ...
  StaticText1 =
      new wxStaticText(this,
                       ID_STATICTEXT1,
                       _("You can also use the grayed objects:\nTheir "
                         "associated extension will be automatically enabled."),
                       wxDefaultPosition,
                       wxDefaultSize,
                       0,
                       _T("ID_STATICTEXT1"));
  ...
  platformChoice = new wxChoice(this,
                                ID_CHOICE1,
                                wxDefaultPosition,
                                wxDefaultSize,
                                0,
  ...
  platformChoice->Hide();
  FlexGridSizer2->Add(
      platformChoice, 1, wxALL | wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 5);
  FlexGridSizer17 = new wxFlexGridSizer(0, 3, 0, 0);
  FlexGridSizer17->AddGrowableRow(0);
  StaticBitmap2 = new wxStaticBitmap(this,
                                     ID_STATICBITMAP5,
  ...
  FlexGridSizer17->Add(
      ...
      5);
  FlexGridSizer2->Add(
      FlexGridSizer17, 1, wxALL | wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 0);
  ...
  Connect(
      ...
      (wxObjectEventFunction)&ChooseObjectTypeDialog::OnobjectsListItemSelect);
  Connect(ID_LISTCTRL1,
          wxEVT_COMMAND_LIST_ITEM_ACTIVATED,
          (wxObjectEventFunction)&ChooseObjectTypeDialog::
              OnobjectsListItemActivated);
  Connect(
      ID_CHOICE1,
      wxEVT_COMMAND_CHOICE_SELECTED,
      (wxObjectEventFunction)&ChooseObjectTypeDialog::OnplatformChoiceSelect);
  Connect(ID_HYPERLINKCTRL2,
          wxEVT_COMMAND_HYPERLINK,
          (wxObjectEventFunction)&ChooseObjectTypeDialog::OnhelpBtClick);
  Connect(ID_BUTTON1,
          wxEVT_COMMAND_BUTTON_CLICKED,
          (wxObjectEventFunction)&ChooseObjectTypeDialog::OnokBtClick);
  Connect(ID_BUTTON2,
          wxEVT_COMMAND_BUTTON_CLICKED,
          (wxObjectEventFunction)&ChooseObjectTypeDialog::OncancelBtClick);
  Connect(wxID_ANY,
          wxEVT_CLOSE_WINDOW,
          (wxObjectEventFunction)&ChooseObjectTypeDialog::OnClose);
  ...
  if (theme)
    theme->SetWindowTheme((HWND)objectsList->GetHWND(), L"EXPLORER", NULL);
#endif

  for (std::size_t i = 0; i < project.GetUsedPlatforms().size(); ++i) {
    platformChoice->Append(project.GetUsedPlatforms()[i]->GetFullName());
    if (project.GetUsedPlatforms()[i] == &project.GetCurrentPlatform())
      platformChoice->SetSelection(i);
  }
  ...
  wxConfigBase::Get()->Read(
      "ChooseObjectTypeDialog/Width", &width, wxDefaultCoord);
  wxConfigBase::Get()->Read(
      "ChooseObjectTypeDialog/Height", &height, wxDefaultCoord);

  if (x <= wxSystemSettings::GetMetric(wxSYS_SCREEN_X) &&
      y <= wxSystemSettings::GetMetric(wxSYS_SCREEN_Y))
...
ChooseObjectTypeDialog::~ChooseObjectTypeDialog() {
  ...
  wxConfigBase::Get()->Write("ChooseObjectTypeDialog/Width",
                             GetSize().GetWidth());
  wxConfigBase::Get()->Write("ChooseObjectTypeDialog/Height",
                             GetSize().GetHeight());
}
...
void ChooseObjectTypeDialog::RefreshList() {
  ...
  for (std::size_t i = 0; i < extensions.size(); ++i) {
    ...
    bool extensionEnabled =
        find(project.GetUsedExtensions().begin(),
    ...
    for (std::size_t j = 0; j < objectsTypes.size(); ++j) {
      ...
      {
        // Generate the icon
        wxBitmap objectIcon = extensions[i]
        ...
        imageList->Add(extensionEnabled ? objectIcon
                                        : objectIcon.ConvertToImage()
                                              .ConvertToGreyscale()
                                              .ConvertToDisabled(255));

        // And add the object to the list
        long index = extensionEnabled ? 0 : objectsList->GetItemCount();
        gd::TreeItemStringData* associatedData =
            new gd::TreeItemStringData(objectsTypes[j]);
        objectsList->InsertItem(
            index,
            extensions[i]->GetObjectMetadata(objectsTypes[j]).GetFullName());
        objectsList->SetItem(
            index,
            1,
            extensions[i]->GetObjectMetadata(objectsTypes[j]).GetDescription());
        objectsList->SetItemImage(index, imageList->GetImageCount() - 1);
        objectsList->SetItemPtrData(index, wxPtrToUInt(associatedData));
        if (!extensionEnabled)
          objectsList->SetItemTextColour(index, wxColor(128, 128, 128));

        // Ensure the selected object type is the focused element (focused
        // element is not considered as selected with wxGTK).
...
void ChooseObjectTypeDialog::OnobjectsListItemSelect(wxListEvent& event) {
  ...
  gd::TreeItemStringData* associatedData =
      reinterpret_cast<gd::TreeItemStringData*>(
          item.GetData());  // Why GetData return long ?
...
void ChooseObjectTypeDialog::OnokBtClick(wxCommandEvent& event) {
  ...
  // We need to find the extension the selected object type belongs to so as to
  // activate it if necessary
  const vector<std::shared_ptr<PlatformExtension> > extensions =
      project.GetCurrentPlatform().GetAllPlatformExtensions();
  std::shared_ptr<PlatformExtension> extension =
      std::shared_ptr<PlatformExtension>();

  for (std::size_t i = 0; i < extensions.size(); ++i) {
    std::vector<gd::String> objectsTypes =
        extensions[i]->GetExtensionObjectsTypes();
    if (find(objectsTypes.begin(), objectsTypes.end(), selectedObjectType) !=
        objectsTypes.end()) {
  ...
  if (extension != std::shared_ptr<PlatformExtension>()) {
    ...
    if (!extensionEnabled) {
      if (wxMessageBox(_("This object is provided by the ") +
                           ...
                           _(" extension, but this extension is not activated "
                             "for the current game.\n\nDo you want to use this "
                             "extension in your game?"),
                       _("Activate extension ") + extension->GetFullName(),
                       wxYES_NO | wxICON_QUESTION | wxYES_DEFAULT) == wxNO) {
        return;
      } else
...
void ChooseObjectTypeDialog::OnClose(wxCloseEvent& event) { EndModal(0); }
...
void ChooseObjectTypeDialog::OnhelpBtClick(wxCommandEvent& event) {
  gd::HelpFileAccess::Get()->OpenPage(
      "en/game_develop/documentation/manual/edit_object");
}

void ChooseObjectTypeDialog::UpdateListColumnsWidth() {
  objectsList->SetColumnWidth(
      0, objectsList->GetSize().GetWidth() * 2.0 / 5.0 - 5);
  objectsList->SetColumnWidth(
      1, objectsList->GetSize().GetWidth() * 3.0 / 5.0 - 5);
}
...
void ChooseObjectTypeDialog::OnplatformChoiceSelect(wxCommandEvent& event) {
  ...
  project.SetCurrentPlatform(
      project.GetUsedPlatforms()[event.GetInt()]->GetName());
  RefreshList();
}
