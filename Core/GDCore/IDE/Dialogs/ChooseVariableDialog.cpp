...
ChooseVariableDialog::ChooseVariableDialog(
    wxWindow* parent,
    gd::VariablesContainer& variablesContainer_,
    bool editingOnly_)
  ...
  variablesList = new wxTreeListCtrl(this,
                                     ID_TREELISTCTRL1,
                                     wxDefaultPosition,
                                     wxDefaultSize,
                                     0,
  ...
  MenuItem1 = new wxMenuItem((&contextMenu),
                             ID_MENUITEM1,
                             _("Edit initial value"),
                             wxEmptyString,
                             wxITEM_NORMAL);
  contextMenu.Append(MenuItem1);
  MenuItem2 = new wxMenuItem(
      (&contextMenu), ID_MENUITEM2, _("Rename"), wxEmptyString, wxITEM_NORMAL);
  contextMenu.Append(MenuItem2);
  contextMenu.AppendSeparator();
  MenuItem3 = new wxMenuItem((&contextMenu),
                             ID_MENUITEM3,
                             _("Add a child variable"),
                             wxEmptyString,
                             wxITEM_NORMAL);
  contextMenu.Append(MenuItem3);
  MenuItem4 = new wxMenuItem(
      (&contextMenu), ID_MENUITEM4, _("Remove"), wxEmptyString, wxITEM_NORMAL);
  ...
  variablesList->Connect(
      wxEVT_KEY_DOWN,
      (wxObjectEventFunction)&ChooseVariableDialog::OnvariablesListKeyDown1,
      0,
      this);
  Connect(ID_HYPERLINKCTRL1,
          wxEVT_COMMAND_HYPERLINK,
          (wxObjectEventFunction)&ChooseVariableDialog::OnhelpBtClick);
  Connect(ID_BUTTON1,
          wxEVT_COMMAND_BUTTON_CLICKED,
          (wxObjectEventFunction)&ChooseVariableDialog::OnokBtClick);
  Connect(ID_BUTTON3,
          wxEVT_COMMAND_BUTTON_CLICKED,
          (wxObjectEventFunction)&ChooseVariableDialog::OncancelBtClick);
  Connect(ID_MENUITEM1,
          wxEVT_COMMAND_MENU_SELECTED,
          (wxObjectEventFunction)&ChooseVariableDialog::OnEditValueSelected);
  Connect(ID_MENUITEM2,
          wxEVT_COMMAND_MENU_SELECTED,
          (wxObjectEventFunction)&ChooseVariableDialog::OnRenameSelected);
  Connect(ID_MENUITEM3,
          wxEVT_COMMAND_MENU_SELECTED,
          (wxObjectEventFunction)&ChooseVariableDialog::OnAddChildSelected);
  Connect(ID_MENUITEM4,
          wxEVT_COMMAND_MENU_SELECTED,
          (wxObjectEventFunction)&ChooseVariableDialog::OnRemoveSelected);
  ...
  Connect(ID_TREELISTCTRL1,
          ...
          (wxObjectEventFunction)&ChooseVariableDialog::OnItemActivated);
  Connect(ID_TREELISTCTRL1,
          wxEVT_COMMAND_TREELIST_ITEM_CONTEXT_MENU,
          (wxObjectEventFunction)&ChooseVariableDialog::OnRightClick);
  Connect(ID_TREELISTCTRL1,
          wxEVT_COMMAND_TREELIST_SELECTION_CHANGED,
          (wxObjectEventFunction)&ChooseVariableDialog::OnItemSelectionChanged);
  Connect(idAddVar,
          wxEVT_COMMAND_MENU_SELECTED,
          (wxObjectEventFunction)&ChooseVariableDialog::OnAddVarSelected);
  Connect(idDelVar,
          wxEVT_COMMAND_MENU_SELECTED,
          (wxObjectEventFunction)&ChooseVariableDialog::OnRemoveSelected);
  Connect(idEditVar,
  ...
  Connect(idMoveUpVar,
          wxEVT_COMMAND_MENU_SELECTED,
          (wxObjectEventFunction)&ChooseVariableDialog::OnMoveUpVarSelected);
  Connect(idMoveDownVar,
          wxEVT_COMMAND_MENU_SELECTED,
          (wxObjectEventFunction)&ChooseVariableDialog::OnMoveDownVarSelected);
  Connect(ID_Help,
          wxEVT_COMMAND_MENU_SELECTED,
          (wxObjectEventFunction)&ChooseVariableDialog::OnhelpBtClick);
  Connect(
      idFindUndeclared,
      wxEVT_COMMAND_MENU_SELECTED,
      (wxObjectEventFunction)&ChooseVariableDialog::OnFindUndeclaredSelected);

  variablesList->AppendColumn(_("Variable"), 150);
  variablesList->AppendColumn(_("Initial value"), 130);

  toolbar->SetToolBitmapSize(wxSize(16, 16));
  toolbar->AddTool(idAddVar,
                   _("Add a variable"),
                   gd::SkinHelper::GetIcon("add", 16),
                   _("Add a variable"));
  toolbar->AddTool(idEditVar,
                   _("Edit the initial value of the variable"),
                   gd::SkinHelper::GetIcon("edit", 16),
                   _("Edit the initial value of the variable"));
  toolbar->AddTool(idRenameVar,
                   _("Rename the variable"),
                   gd::SkinHelper::GetIcon("editname", 16),
                   _("Rename the variable"));
  toolbar->AddTool(idDelVar,
                   _("Delete the selected variable"),
                   gd::SkinHelper::GetIcon("delete", 16),
                   _("Delete the selected variable"));
  toolbar->AddSeparator();
  toolbar->AddTool(idMoveUpVar,
                   _("Move up"),
                   gd::SkinHelper::GetIcon("up", 16),
                   _("Move up"));
  toolbar->AddTool(idMoveDownVar,
                   _("Move down"),
                   gd::SkinHelper::GetIcon("down", 16),
                   _("Move down"));
  toolbar->AddSeparator();
  toolbar->AddTool(idFindUndeclared,
                   _("Search for undeclared variables"),
                   gd::SkinHelper::GetIcon("find", 16),
                   _("Search for undeclared variables inside the project"));
  toolbar->AddSeparator();
  toolbar->AddTool(ID_Help,
                   _("Help about variables"),
                   gd::SkinHelper::GetIcon("help", 16),
                   _("Help about variables"));
  ...
  for (wxTreeListItem item = variablesList->GetFirstItem(); item.IsOk();
...
void ChooseVariableDialog::UpdateTitle() {
  if (editingOnly) {
    wxString context = "";
    if (associatedProject != NULL && associatedLayout == NULL)
      context = _("Global variables");
    else if (associatedProject != NULL && associatedLayout != NULL &&
             associatedObject == NULL)
      context = wxString::Format(_("\"%s\" scene variables").ToWxString(),
                                 associatedLayout->GetName().ToWxString());
    else if (associatedProject != NULL && associatedLayout != NULL &&
             associatedObject != NULL)
      context = wxString::Format(_("\"%s\" object variables").ToWxString(),
                                 associatedObject->GetName().ToWxString());
    else
      context = "Instance variables";

    SetTitle(wxString::Format(wxString(_("Edit the variables (%s)")), context));
    okBt->SetLabel(_("Ok"));
  }
}
 ...
 */
void ChooseVariableDialog::RefreshVariable(wxTreeListItem item,
  ...
  if (!variable.IsStructure()) {
    ...
    for (auto it = children.begin(); it != children.end(); ++it) {
      if (!currentChildItem.IsOk())
        currentChildItem = variablesList->AppendItem(item, it->first);
...
void ChooseVariableDialog::RefreshAll() {
  ...
  for (std::size_t i = 0; i < temporaryContainer->Count(); ++i) {
    ...
    wxTreeListItem item =
        variablesList->AppendItem(variablesList->GetRootItem(), name);
...
void ChooseVariableDialog::OncancelBtClick(wxCommandEvent& event) {
  if (modificationCount > 4) {
    wxMessageDialog msgDlg(
        this,
        _("You made ") + gd::String::From(modificationCount) +
            _(" changes. Are you sure you want to cancel all changes\?"),
        _("Lot's of changes made."),
        wxYES_NO | wxICON_QUESTION);
    if (msgDlg.ShowModal() == wxID_NO) return;
  }
...
void ChooseVariableDialog::OnAddVarSelected(wxCommandEvent& event) {
  ...
  newName =
      wxGetTextFromUser(_("Please choose a new name for the new variable"),
                        _("New variable name"),
                        newName);
  ...
  wxTreeListItem item =
      variablesList->AppendItem(variablesList->GetRootItem(), newName);
...
void ChooseVariableDialog::OnhelpBtClick(wxCommandEvent& event) {
  gd::HelpFileAccess::Get()->OpenPage(
      "game_develop/documentation/manual/global_variables");
}
...
void ChooseVariableDialog::OnFindUndeclaredSelected(wxCommandEvent& event) {
  ...
  if (associatedProject != NULL && associatedLayout == NULL)
    allVariables = EventsVariablesFinder::FindAllGlobalVariables(
        associatedProject->GetCurrentPlatform(), *associatedProject);
  else if (associatedProject != NULL && associatedLayout != NULL &&
           associatedObject == NULL)
    allVariables = EventsVariablesFinder::FindAllLayoutVariables(
        associatedProject->GetCurrentPlatform(),
        *associatedProject,
        *associatedLayout);
  else if (associatedProject != NULL && associatedLayout != NULL &&
           associatedObject != NULL)
    allVariables = EventsVariablesFinder::FindAllObjectVariables(
        associatedProject->GetCurrentPlatform(),
        *associatedProject,
        *associatedLayout,
        *associatedObject);
  ...
  for (std::set<gd::String>::const_iterator it = allVariables.begin();
       it != allVariables.end();
       ++it) {
    if (!temporaryContainer->Has(*it)) variablesNotDeclared.push_back(*it);
  }

  // Request the user to choose which variables to add.
  wxMultiChoiceDialog dialog(
      this,
      _("These variables are used but not declared:\nCheck the variables to be "
        "added to the list."),
      _("Adding undeclared variables"),
      variablesNotDeclared,
  ...
  for (std::size_t i = 0; i < selection.size(); ++i) {
    temporaryContainer->InsertNew(variablesNotDeclared[selection[i]],
                                  temporaryContainer->Count());
    modificationCount++;
  }
...
void ChooseVariableDialog::OnEditValueSelected(wxCommandEvent& event) {
  ...
  wxTextEntryDialog editDialog(this,
                               _("Enter the initial value of the variable"),
                               _("Initial value"),
                               selectedVariable->GetString(),
  ...
  if (editDialog.ShowModal() == wxID_OK) {
    ...
    RefreshVariable(
        variablesList->GetSelection(), selectedVariableName, *selectedVariable);
    modificationCount++;
  }
}
...
void ChooseVariableDialog::OnRenameSelected(wxCommandEvent& event) {
  ...
  gd::String newName =
      wxGetTextFromUser(_("Enter the new name of the variable"),
                        _("New name"),
...
void ChooseVariableDialog::UpdateSelectedAndParentVariable() {
  ...
  if (parent == variablesList->GetRootItem()) {
    ...
    if (parentVariable)
      selectedVariable = &parentVariable->GetChild(selectedVariableName);
    selectedVariableFullName += selectedVariableName;
  }
}
...
void ChooseVariableDialog::SetAssociatedLayout(const gd::Project* project,
...
void ChooseVariableDialog::SetAssociatedObject(const gd::Project* project,
                                               const gd::Layout* layout,
...
wxTreeListItem ChooseVariableDialog::GetPreviousSibling(wxTreeListCtrl* ctrl,
                                                        wxTreeListItem item) {
