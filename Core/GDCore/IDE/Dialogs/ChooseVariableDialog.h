...
/**
 * \brief Dialog used to display variables of a gd::VariablesContainer, edit
 * them and/or choose one.
 *
 * Also offer a nice feature to scan the associated project/layout for
 * undeclared variables.<br> The dialog can be used as an editor only, see the
 * constructor.
 *
 * \ingroup IDEDialogs
 */
class GD_CORE_API ChooseVariableDialog : public wxDialog {
 public:
   ...
   * \param editingOnly If set to true, the dialog will act as a dialog to edit
   * the variables and not to choose one ( Double click won't close the dialog
   * for example ).
   */
  ChooseVariableDialog(wxWindow* parent,
                       gd::VariablesContainer& variablesContainer,
                       bool editingOnly = false);

  /**
   * \brief Specify an optional associated project
   * \param project Associated project. If different from NULL, global variables
   * from this project will be scanned when searching for undeclared variables
   */
   ...
   * \param project Associated layout. If different from NULL, layout variables
   * from this layout will be scanned when searching for undeclared variables
   */
  void SetAssociatedLayout(const gd::Project* project,
   ...
   * \param project Associated object. If different from NULL, object variables
   * from this layout will be scanned when searching for undeclared variables
   */
  void SetAssociatedObject(const gd::Project* project,
                           const gd::Layout* layout,
 ...
 private:
  ...
  void RefreshVariable(wxTreeListItem item,
                       const gd::String& name,
                       const gd::Variable& variable);
  wxTreeListItem GetPreviousSibling(wxTreeListCtrl* ctrl, wxTreeListItem item);

  gd::VariablesContainer&
      variablesContainer;  ///< gd::VariablesContainer storing the variables
  std::shared_ptr<gd::VariablesContainer>
      temporaryContainer;  ///< Temporary container used to allow to make
                           ///< temporary changes before applying them to the
                           ///< real variables container if Ok is pressed.
  bool editingOnly;  ///< If set to true, the dialog will act as a dialog to
                     ///< edit the variables and not to choose one ( Double
                     ///< click won't close the dialog for example ).
  const gd::Project* associatedProject;
  const gd::Layout* associatedLayout;
  const gd::Object* associatedObject;

  gd::String selectedVariableName;  ///< Contains the name of the last selected
                                    ///< variable.
  gd::String
      selectedVariableFullName;  ///< Contains the full name of the last
                                 ///< selected variable. ( full gd::String to
                                 ///< access to the variable )
  gd::Variable*
      selectedVariable;  ///< A pointer to the focused variable ( Can be NULL ).
  gd::Variable*
      parentVariable;  ///< A pointer to the parent of the selected variable (
                       ///< Can be NULL if not applicable ).

  gd::String oldName;  ///< Used to remember the variable old name when renaming
  unsigned int modificationCount;  ///< Track the number of modification. If the
                                   ///< user made lots of modifications and
                                   ///< wants to cancel, he will be warned.
