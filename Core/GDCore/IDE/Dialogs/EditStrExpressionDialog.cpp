...
EditStrExpressionDialog::EditStrExpressionDialog(wxWindow* parent,
                                                 gd::String expression,
                                                 gd::Project& project_,
                                                 gd::Layout& layout_)
    : project(project_), layout(layout_), lastErrorPos(gd::String::npos) {
  ...
  TexteEdit = new wxStyledTextCtrl(centerPanel,
                                   ID_CUSTOM1,
                                   wxDefaultPosition,
                                   wxSize(300, 28),
                                   0,
  ...
  FlexGridSizer17->Add(
      ...
      5);
  FlexGridSizer3->Add(
      FlexGridSizer17, 1, wxRIGHT | wxALIGN_RIGHT | wxALIGN_CENTER_VERTICAL, 5);
  FlexGridSizer9->Add(
      FlexGridSizer3,
      1,
      wxALL | wxEXPAND | wxALIGN_CENTER_HORIZONTAL | wxALIGN_CENTER_VERTICAL,
  ...
  mgr->AddPane(centerPanel,
               wxAuiPaneInfo()
                   ...
                   .Movable(false)
                   .PaneBorder(false));
  objectsFunctionsPanel = new wxPanel(this,
                                      ID_PANEL2,
                                      wxDefaultPosition,
                                      wxDefaultSize,
                                      wxTAB_TRAVERSAL,
  ...
  ObjList = new wxTreeCtrl(objectsFunctionsPanel,
                           ID_TREECTRL1,
                           wxDefaultPosition,
                           wxSize(195, 177),
  ...
  FlexGridSizer7->Add(
      ...
      wxALL | wxEXPAND | wxALIGN_CENTER_HORIZONTAL | wxALIGN_CENTER_VERTICAL,
      0);
  AddPropBt = new wxButton(objectsFunctionsPanel,
                           ID_BUTTON10,
                           _("Add"),
                           wxDefaultPosition,
                           wxDefaultSize,
  ...
  mgr->AddPane(objectsFunctionsPanel,
               wxAuiPaneInfo()
                   .Name(_T("objectsFunctionsPane"))
                   .DefaultPane()
  ...
  FlexGridSizer10->Add(
      ...
      wxALL | wxEXPAND | wxALIGN_CENTER_HORIZONTAL | wxALIGN_CENTER_VERTICAL,
      0);
  AddFunctionBt = new wxButton(freeFunctionsPanel,
                               ID_BUTTON7,
                               _("Add"),
                               wxDefaultPosition,
                               wxDefaultSize,
  ...
  Connect(ID_BUTTON2,
          ...
          (wxObjectEventFunction)&EditStrExpressionDialog::OnOkBtClick);
  Connect(ID_HYPERLINKCTRL1,
          wxEVT_COMMAND_HYPERLINK,
          (wxObjectEventFunction)&EditStrExpressionDialog::OnerrorTxtClick);
  Connect(ID_HYPERLINKCTRL2,
          wxEVT_COMMAND_HYPERLINK,
          (wxObjectEventFunction)&EditStrExpressionDialog::OnhelpBtClick);
  Connect(
      ID_TREECTRL1,
      wxEVT_COMMAND_TREE_ITEM_ACTIVATED,
      (wxObjectEventFunction)&EditStrExpressionDialog::OnObjListItemActivated);
  Connect(ID_TREECTRL1,
          wxEVT_COMMAND_TREE_SEL_CHANGED,
          (wxObjectEventFunction)&EditStrExpressionDialog::
              OnObjListSelectionChanged);
  Connect(ID_BUTTON10,
          wxEVT_COMMAND_BUTTON_CLICKED,
          (wxObjectEventFunction)&EditStrExpressionDialog::OnAddPropBtClick);
  Connect(ID_TREECTRL2,
          wxEVT_COMMAND_TREE_ITEM_ACTIVATED,
          (wxObjectEventFunction)&EditStrExpressionDialog::
              OnTreeCtrl1ItemActivated);
  Connect(ID_TREECTRL2,
          wxEVT_COMMAND_TREE_SEL_CHANGED,
          (wxObjectEventFunction)&EditStrExpressionDialog::
              OnTreeCtrl1SelectionChanged);
  Connect(
      ID_BUTTON7,
      wxEVT_COMMAND_BUTTON_CLICKED,
      (wxObjectEventFunction)&EditStrExpressionDialog::OnAddFunctionBtClick);
  //*)
  Connect(ID_CUSTOM1,
          wxEVT_STC_MODIFIED,
          (wxObjectEventFunction)&EditStrExpressionDialog::TextModified);
  Connect(ID_CUSTOM1,
          wxEVT_STC_UPDATEUI,
          (wxObjectEventFunction)&EditStrExpressionDialog::UpdateTextCtrl);
  ...
  for (std::size_t i = 0; i < extensions.size(); ++i) {
    ...
    for (std::size_t j = 0; j < objectsTypes.size(); ++j) {
      ...
      std::map<gd::String, gd::ExpressionMetadata> allObjExpr =
          extensions[i]->GetAllStrExpressionsForObject(objectsTypes[j]);
      ...
      wxTreeItemId behaviorTypeItem =
          behaviorsTypes[j] == ""
              ...
              : ObjList->AppendItem(
                    ...
                    _("Behavior") + wxString(" ") +
                        extensions[i]
                            ->GetBehaviorMetadata(behaviorsTypes[j])
                            .GetFullName(),
                    0);

      // Add each behavior expression
      std::map<gd::String, gd::ExpressionMetadata> allAutoExpr =
          extensions[i]->GetAllStrExpressionsForBehavior(behaviorsTypes[j]);
      for (std::map<gd::String, gd::ExpressionMetadata>::const_iterator it =
               allAutoExpr.begin();
           it != allAutoExpr.end();
           ++it) {
        if (it->second.IsShown()) {
          ...
          wxTreeItemIdValue cookie;
          wxTreeItemId groupItem =
              ObjList->GetFirstChild(behaviorTypeItem, cookie);
          while (groupItem.IsOk() &&
                 ObjList->GetItemText(groupItem) != it->second.GetGroup()) {
            groupItem = ObjList->GetNextSibling(groupItem);
          }
          if (!groupItem.IsOk())
            groupItem =
                ObjList->AppendItem(behaviorTypeItem, it->second.GetGroup(), 0);
    ...
    std::map<gd::String, gd::ExpressionMetadata> allExpr =
    ...
    for (std::map<gd::String, gd::ExpressionMetadata>::const_iterator it =
      ...
      if (it->second.IsShown()) {
        ...
        while (groupItem.IsOk() &&
               ValList->GetItemText(groupItem) != it->second.GetGroup()) {
          groupItem = ValList->GetNextSibling(groupItem);
        }
        if (!groupItem.IsOk())
          groupItem =
              ValList->AppendItem(extensionItem, it->second.GetGroup(), 0);
  ...
  TexteEdit->StyleSetFont(wxSTC_STYLE_DEFAULT,
  ...
  for (std::size_t i = 0; i < extensions.size(); ++i) {
    ...
    const std::map<gd::String, gd::ExpressionMetadata>& allExprs =
    ...
    const std::map<gd::String, gd::ExpressionMetadata>& allStrExprs =
        extensions[i]->GetAllStrExpressions();
    for (std::map<gd::String, gd::ExpressionMetadata>::const_iterator it =
             allStrExprs.begin();
         it != allStrExprs.end();
    ...
    for (std::size_t j = 0; j < behaviorsTypes.size(); ++j) {
      const std::map<gd::String, gd::ExpressionMetadata>& allExprs =
          extensions[i]->GetAllExpressionsForBehavior(behaviorsTypes[j]);
      for (std::map<gd::String, gd::ExpressionMetadata>::const_iterator it =
               allExprs.begin();
           it != allExprs.end();
           ++it)
...
void EditStrExpressionDialog::OnOkBtClick(wxCommandEvent& event) {
  ...
  if (!expressionParser.ParseStringExpression(
          project.GetCurrentPlatform(), project, layout, callbacks)) {
    if (wxMessageBox(_("The expression is malformed. Are you sur you want to "
 ...
 */
gd::String EditStrExpressionDialog::ShowParameterDialog(
    const gd::ParameterMetadata& parameterMetadata,
    bool& userCancelled,
    gd::String objectNameAssociated) {
  if (parameterMetadata.type == "expression") {
    gd::AdvancedTextEntryDialog dialog(
        this,
        _("Parameter"),
        parameterMetadata.description,
        "0",
        gd::AdvancedTextEntryDialog::MathExpression,
        &project,
        &layout);
    ...
    gd::AdvancedTextEntryDialog dialog(
        ...
        parameterMetadata.description,
        "\"\"",
        gd::AdvancedTextEntryDialog::TextExpression,
        &project,
        &layout);
    if (dialog.ShowModal() == wxOK)
      return dialog.text;
    ...
    for (std::size_t i = 1;
      ...
      if (i == 1 &&
        ...
        parametersStr += ShowParameterDialog(
            infos->GetExpressionMetadata().parameters[i], cancelled, object);
        if (cancelled) return;
      }
    }

    if (TexteEdit->GetText() == "\"\"") TexteEdit->SetText("");
    TexteEdit->AddText(object + "." + behaviorStr + infos->GetName() + "(" +
                       parametersStr + ")");
    return;
  }
}
...
void EditStrExpressionDialog::OnAddFunctionBtClick(wxCommandEvent& event) {
  ...
  if (infos != NULL) {
    ...
    for (std::size_t i = 0;
      ...
      if (!parametersStr.empty()) parametersStr += ",";
      parametersStr += ShowParameterDialog(
          infos->GetExpressionMetadata().parameters[i], cancelled);
      if (cancelled) return;
    }
...
void EditStrExpressionDialog::OnerrorTxtClick(wxCommandEvent& event) {
  if (lastErrorPos != gd::String::npos) TexteEdit->GotoPos(lastErrorPos);
}

void EditStrExpressionDialog::OnhelpBtClick(wxCommandEvent& event) {
  gd::HelpFileAccess::Get()->OpenPage(
      "game_develop/documentation/manual/edit_text");
}
