...
bool EventsEditorItemsAreas::IsOnEvent(int x, int y) {
  for (std::size_t i = 0; i < eventsAreas.size(); ++i) {
    if (eventsAreas[i].first.Contains(x, y)) return true;
  }
...
EventItem EventsEditorItemsAreas::GetEventAt(int x, int y) {
  for (std::size_t i = 0; i < eventsAreas.size(); ++i) {
    if (eventsAreas[i].first.Contains(x, y)) return eventsAreas[i].second;
  }
...
wxRect EventsEditorItemsAreas::GetAreaOfEventAt(int x, int y) {
  for (std::size_t i = 0; i < eventsAreas.size(); ++i) {
    if (eventsAreas[i].first.Contains(x, y)) return eventsAreas[i].first;
  }
...
bool EventsEditorItemsAreas::IsOnInstruction(int x, int y) {
  for (std::size_t i = 0; i < instructionsAreas.size(); ++i) {
    if (instructionsAreas[i].first.Contains(x, y)) return true;
  }
...
bool EventsEditorItemsAreas::IsOnInstructionList(int x, int y) {
  for (std::size_t i = 0; i < instructionListsAreas.size(); ++i) {
    if (instructionListsAreas[i].first.Contains(x, y)) return true;
  }
...
InstructionListItem EventsEditorItemsAreas::GetInstructionListAt(int x, int y) {
  ...
  for (std::size_t i = 0; i < instructionListsAreas.size(); ++i) {
    if (instructionListsAreas[i].first.Contains(x, y)) {
      if ((instructionListsAreas[i].first.width *
               instructionListsAreas[i].first.height <
...
bool EventsEditorItemsAreas::IsOnParameter(int x, int y) {
  for (std::size_t i = 0; i < parametersAreas.size(); ++i) {
    if (parametersAreas[i].first.Contains(x, y)) return true;
  }
...
bool EventsEditorItemsAreas::IsOnFoldingItem(int x, int y) {
  for (std::size_t i = 0; i < foldingAreas.size(); ++i) {
    if (foldingAreas[i].first.Contains(x, y)) return true;
  }
...
FoldingItem EventsEditorItemsAreas::GetFoldingItemAt(int x, int y) {
  for (std::size_t i = 0; i < foldingAreas.size(); ++i) {
    if (foldingAreas[i].first.Contains(x, y)) return foldingAreas[i].second;
  }
...
void EventsEditorItemsAreas::AddInstructionArea(
    wxRect area, gd::InstructionItem& instruction) {
  instructionsAreas.push_back(std::make_pair(area, instruction));
}

void EventsEditorItemsAreas::AddParameterArea(wxRect area,
...
void EventsEditorItemsAreas::AddInstructionListArea(wxRect area,
...
bool EventItem::operator==(const gd::EventItem& other) const {
  return (event == other.event && eventsList == other.eventsList &&
          positionInList == other.positionInList);
}

EventItem::EventItem(std::shared_ptr<gd::BaseEvent> event_,
                     gd::EventsList* eventsList_,
                     std::size_t positionInList_)
    : event(event_), eventsList(eventsList_), positionInList(positionInList_) {}

EventItem::EventItem() : eventsList(NULL), positionInList(0) {}
...
bool gd::InstructionItem::operator==(const gd::InstructionItem& other) const {
  return (instruction == other.instruction &&
          ...
          positionInList == other.positionInList && event == other.event);
}

InstructionItem::InstructionItem(gd::Instruction* instruction_,
                                 bool isCondition_,
...
bool InstructionListItem::operator==(const InstructionListItem& other) const {
  return (isConditionList == other.isConditionList &&
          instructionList == other.instructionList && event == other.event);
}

InstructionListItem::InstructionListItem(bool isCondition_,
                                         gd::InstructionsList* instructionList_,
                                         gd::BaseEvent* event_)
...
InstructionListItem::InstructionListItem()
    : isConditionList(true), instructionList(NULL), event(NULL) {}
// ParameterItem stuff :
...
ParameterItem::ParameterItem(gd::Expression* parameter_, gd::BaseEvent* event_)
    : parameter(parameter_), event(event_) {}

ParameterItem::ParameterItem() : parameter(NULL), event(NULL) {}
...
FoldingItem::FoldingItem(gd::BaseEvent* event_) : event(event_) {}

FoldingItem::FoldingItem() : event(NULL) {}
