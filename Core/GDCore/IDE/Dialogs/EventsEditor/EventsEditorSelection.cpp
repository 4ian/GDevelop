...
void EventsEditorSelection::AddEvent(const gd::EventItem& eventSelection) {
  if (eventSelection.event == std::shared_ptr<gd::BaseEvent>() ||
      eventSelection.eventsList == NULL) {
...
std::vector<EventItem> EventsEditorSelection::GetAllSelectedEvents() {
  ...
  for (std::unordered_set<EventItem>::iterator it = eventsSelected.begin();
       it != eventsSelected.end();
       ++it)
...
EventsEditorSelection::GetAllSelectedEventsWithoutSubEvents() {
  std::vector<EventItem> results;
  for (std::unordered_set<EventItem>::iterator it = eventsSelected.begin();
       it != eventsSelected.end();
       ++it) {
    bool isAlreadyIncludedAsSubEvent = false;
    for (std::unordered_set<EventItem>::iterator it2 = eventsSelected.begin();
         it2 != eventsSelected.end();
         ++it2) {
    ...
    if (!isAlreadyIncludedAsSubEvent) results.push_back(*it);
  }
 ...
 */
std::vector<gd::InstructionItem>
EventsEditorSelection::GetAllSelectedInstructions() {
  std::vector<gd::InstructionItem> results;
  for (std::unordered_set<gd::InstructionItem>::iterator it =
           instructionsSelected.begin();
       it != instructionsSelected.end();
       ++it)
    results.push_back(*it);
...
bool EventsEditorSelection::InstructionSelected(
    const gd::InstructionItem& instr) {
  return instructionsSelected.find(instr) != instructionsSelected.end();
}

void EventsEditorSelection::SetHighlighted(
    const gd::EventItem& eventSelection) {
  eventHighlighted = eventSelection;
}
void EventsEditorSelection::SetHighlighted(
    const gd::InstructionItem& instructionSelection) {
...
void EventsEditorSelection::BeginDragEvent() { dragging = true; }

bool EventsEditorSelection::IsDraggingEvent() { return dragging; }

bool EventsEditorSelection::EndDragEvent(bool deleteDraggedEvent,
                                         bool dropAfterHighlightedElement) {
  ...
  for (std::unordered_set<EventItem>::iterator it = eventsSelected.begin();
    ...
    if ((*it).event == eventHighlighted.event ||
        ((*it).event->CanHaveSubEvents() &&
         (*it).event->GetSubEvents().Contains(*eventHighlighted.event))) {
      return false;
    }
  }

  // Insert copy of dragged events
  size_t positionInList = !dropAfterHighlightedElement
                              ? eventHighlighted.positionInList
                              : eventHighlighted.positionInList + 1;
  for (std::unordered_set<EventItem>::iterator it = eventsSelected.begin();
       it != eventsSelected.end();
       ++it) {
    if ((*it).event != std::shared_ptr<gd::BaseEvent>())
...
gd::InstructionsList* EventsEditorSelection::EndDragInstruction(
    bool deleteDraggedInstruction, bool dropAfterHighlightedElement) {
  ...
  if (instructionHighlighted.instructionList != NULL) {
    ...
    if (instructionHighlighted.event != NULL)
      instructionHighlighted.event->eventHeightNeedUpdate = true;
    else
      std::cout << "WARNING : Instruction hightlighted event is not valid! "
                << std::endl;
    ...
    if (instructionListHighlighted.event != NULL)
      instructionListHighlighted.event->eventHeightNeedUpdate = true;
    else
      std::cout
          << "WARNING : Instruction list hightlighted event is not valid! "
          << std::endl;
  } else
    return NULL;
  ...
  if (instructionHighlighted.instruction != NULL) {
    for (std::unordered_set<gd::InstructionItem>::iterator it =
      ...
      if ((*it).instruction == instructionHighlighted.instruction ||
          (FindInInstructionsAndSubInstructions(
              (*it).instruction->GetSubInstructions(),
              instructionHighlighted.instruction))) {
        return NULL;
      }
      if (FindInInstructionsAndSubInstructions(
              instructionHighlighted.instruction->GetSubInstructions(),
              (*it).instruction)) {
        return NULL;
      }
    }
  ...
  if (deleteDraggedInstruction) {
    ...
    for (std::unordered_set<gd::InstructionItem>::iterator it =
         ...
         it != instructionsSelected.end();
         ++it) {
      if ((*it).instructionList == instructionHighlighted.instructionList &&
          (*it).positionInList > instructionHighlighted.positionInList) {
        ...
        newInstructionsSelected.insert(*it);
    }
...
void EventsEditorSelection::InstructionHighlightedOnBottomPart(
    bool isOnbottomHandSide) {
  isOnbottomHandSideOfInstruction = isOnbottomHandSide;
}

void EventsEditorSelection::EventHighlightedOnBottomPart(
    bool isOnbottomHandSide) {
...
void EventsEditorSelection::DeleteAllInstructionSelected() {
  ...
  std::map<gd::InstructionsList*, std::list<std::size_t> >
      mapOfDeletionsRequest;
  for (std::unordered_set<gd::InstructionItem>::iterator it =
           instructionsSelected.begin();
       it != instructionsSelected.end();
       ++it) {
    ...
    if ((*it).instructionList != NULL)
      mapOfDeletionsRequest[(*it).instructionList].push_back(
          (*it).positionInList);
  }
  // 2) For each list, erase each index
  for (auto it = mapOfDeletionsRequest.begin();
       it != mapOfDeletionsRequest.end();
       ++it) {
    std::list<std::size_t>& listOfIndexesToDelete = it->second;
    listOfIndexesToDelete.sort();
    listOfIndexesToDelete.reverse();  // We have erase from end to start to
                                      // prevent index changing

    for (std::list<std::size_t>::iterator index = listOfIndexesToDelete.begin();
         index != listOfIndexesToDelete.end();
         ++index)
      it->first->Remove(*index);
  }
}

EventsEditorSelection::EventsEditorSelection(
    gd::EventsEditorRefreshCallbacks& eventsEditorCallback_)
    : dragging(false),
      draggingInstruction(false),
      eventsEditorCallback(eventsEditorCallback_) {}

bool EventsEditorSelection::FindInInstructionsAndSubInstructions(
    gd::InstructionsList& list, const gd::Instruction* instrToSearch) {
  for (std::size_t i = 0; i < list.size(); ++i) {
    if (&list[i] == instrToSearch) return true;
    if (FindInInstructionsAndSubInstructions(list[i].GetSubInstructions(),
                                             instrToSearch))
      return true;
  }
