...
class GD_CORE_API EventsEditorSelection {
 public:
  EventsEditorSelection(
      gd::EventsEditorRefreshCallbacks& eventsEditorCallback_);
  ...
  /**
   * Set if the mouse is on the bottom hand side of the instruction, or on the
   * top hand side.
   */
  void InstructionHighlightedOnBottomPart(bool isOnbottomHandSide);

  /**
   * Set if the mouse is on the bottom hand side of the event, or on the top
   * hand side.
   */
   ...
   */
  bool EventHighlighted(const gd::EventItem& event) {
    return eventHighlighted == event;
  };

  /**
   * Return true if the mouse is on the bottom hand side of the event
   * highlighted. ( Used to known if we have to drop the dragged event(s) afer
   * or before the highlighted one )
   */
   ...
   */
  bool InstructionHighlighted(const gd::InstructionItem& instr) {
    return instructionHighlighted == instr;
  };

  /**
   * Return true if the mouse is on the bottom hand side of the instruction
   * highlighted
   */
  bool IsInstructionHighlightedOnBottomPart() {
    return isOnbottomHandSideOfInstruction;
  };

  /**
   * Return true if an instruction list item is highlighted
   */
  bool InstructionListHighlighted(const InstructionListItem& list) {
    return instructionListHighlighted == list;
  };

  /**
   * Return true if a parameter is highlighted
   */
  bool ParameterHighLighted(const ParameterItem& parameter) {
    return parameterHighlighted == parameter;
  };
   ...
   */
  InstructionListItem& GetHighlightedInstructionList() {
    return instructionListHighlighted;
  };
  ...
  /**
   * As it is more difficult than for event, this function is dedicaced to
   * deleting all instructions selected
   */
  void DeleteAllInstructionSelected();

  void BeginDragEvent();
  bool IsDraggingEvent();
  bool EndDragEvent(bool deleteDraggedEvent = true,
                    bool dropAfterHighlightedElement = false);

  void BeginDragInstruction();
  bool IsDraggingInstruction();
  gd::InstructionsList* EndDragInstruction(
      bool deleteDraggedInstruction = true,
      bool dropAfterHighlightedElement = false);

 private:
  std::unordered_set<EventItem> eventsSelected;  ///< Events selection
  std::unordered_set<gd::InstructionItem>
      instructionsSelected;  ///< Events selection
  ...
  /**
   * Return true if an event is found in the list ( sub events are also taken in
   * account )
   */
  bool FindInEventsAndSubEvents(const gd::EventsList& list,
                                const gd::BaseEvent& eventToSearch);
  /**
   * Return true if an instruction is found in the list ( sub instructions are
   * also taken in account )
   */
  bool FindInInstructionsAndSubInstructions(
      gd::InstructionsList& list, const gd::Instruction* instrToSearch);
