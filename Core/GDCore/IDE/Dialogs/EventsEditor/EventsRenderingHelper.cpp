...
wxPoint EventsRenderingHelper::DrawTextInArea(gd::String text,
                                              wxDC& dc,
                                              wxRect rect,
                                              wxPoint point) {
  ...
  int spacesToInsert = static_cast<float>(point.x - rect.x) /
                       static_cast<float>(fontCharacterWidth);
  if (spacesToInsert < 0) spacesToInsert = 0;
  for (unsigned int i = 0; i < spacesToInsert; ++i) {
    text = " " + text;
  }
  point.x =
      rect.x;  // We can now start from rect.x as spaces have been inserted.

  int charactersInALine = floor(static_cast<double>(rect.width) /
                                static_cast<double>(fontCharacterWidth));
  if (charactersInALine <= 0) charactersInALine = 1;

  int textLength = text.size();

  int cutCount = ceil(static_cast<double>(textLength) /
                      static_cast<double>(charactersInALine));
  ...
  return wxPoint(point.x + displayedText.length() * fontCharacterWidth,
                 point.y);
}

unsigned int EventsRenderingHelper::GetTextHeightInArea(
    const gd::String& text, unsigned int widthAvailable) {
  if (text.empty() || widthAvailable == 0) return 0;

  int charactersInALine = floor(static_cast<double>(widthAvailable) /
                                static_cast<double>(fontCharacterWidth));
  if (charactersInALine <= 0) charactersInALine = 1;

  int cutCount = ceil(static_cast<double>(text.length()) /
                      static_cast<double>(charactersInALine));
  if (cutCount <= 0) cutCount = 1;
...
#else
  wxFont font(
      8, wxFONTFAMILY_TELETYPE, wxFONTSTYLE_NORMAL, wxFONTWEIGHT_NORMAL, false);
#endif
...
int EventsRenderingHelper::DrawConditionsList(
  ...
  const int conditionsHeight =
      GetRenderedConditionsListHeight(conditions, width, platform);
  ...
  if (conditions.empty()) {
    if (selection.IsDraggingInstruction() &&
        selection.InstructionListHighlighted(item)) {
      dc.SetPen(wxPen(wxColour(0, 0, 0)));
      dc.SetBrush(wxBrush(wxColour(0, 0, 0)));
      dc.DrawRectangle(x + 2, y, width - 2, 2);
    }

    dc.SetTextForeground(!event->IsDisabled() ? wxColour(0, 0, 0)
  ...
  for (std::size_t j = 0; j < conditions.size(); j++) {
    ...
    const gd::InstructionMetadata& instructionMetadata =
        MetadataProvider::GetConditionMetadata(platform,
                                               conditions[j].GetType());
    gd::InstructionItem accessor(
        &conditions[j], /*isCondition=*/true, &conditions, j, event);
    ...
    if (selection.InstructionSelected(accessor)) {
      gd::String text = InstructionSentenceFormatter::Get()->Translate(
          conditions[j], instructionMetadata);
      height = GetTextHeightInArea(text, freeWidth);

      dc.SetPen(selectionRectangleOutline);
      dc.SetBrush(selectionRectangleFill);
      dc.DrawRectangle(x, y, width, height);
      DrawInstruction(conditions[j],
                      instructionMetadata,
                      /*isCondition=*/true,
                      dc,
                      wxPoint(x + leftIconsWidth, y),
                      freeWidth,
                      event,
                      areas,
                      selection);
      ...
      if (selection.IsDraggingInstruction()) {
        ...
        dc.DrawRectangle(
            x + leftIconsWidth + 2,
            selection.IsInstructionHighlightedOnBottomPart() ? y + height : y,
            width - 2 - iconWidth - 2,
            2);
      }
    } else
      height = DrawInstruction(conditions[j],
                               instructionMetadata,
                               /*isCondition=*/true,
                               dc,
                               wxPoint(x + leftIconsWidth, y),
    ...
    if (conditions[j].IsInverted()) {
      dc.DrawBitmap(
          gd::CommonBitmapProvider::Get()->invertedCondition, x + 1, y, true);
      if (bmp->IsOk()) dc.DrawBitmap(*bmp, x + iconWidth + 1, y, true);
    } else if (bmp->IsOk())
      dc.DrawBitmap(*bmp, x + 1, y, true);
    ...
    if (instructionMetadata.CanHaveSubInstructions())
      y += DrawConditionsList(conditions[j].GetSubInstructions(),
                              dc,
                              x + 18,
                              y,
                              width - 18,
...
int EventsRenderingHelper::DrawActionsList(gd::InstructionsList& actions,
                                           wxDC& dc,
                                           int x,
                                           int y,
                                           int width,
  ...
  const int actionsHeight =
      GetRenderedActionsListHeight(actions, width, platform);
  ...
  for (std::size_t j = 0; j < actions.size(); j++) {
    ...
    const gd::InstructionMetadata& instructionMetadata =
        MetadataProvider::GetActionMetadata(platform, actions[j].GetType());
    gd::InstructionItem accessor(
        &actions[j], /*isCondition=*/false, &actions, j, event);
    ...
    if (selection.InstructionSelected(accessor)) {
      gd::String text = InstructionSentenceFormatter::Get()->Translate(
          actions[j], instructionMetadata);
      height = GetTextHeightInArea(text, freeWidth);

      dc.SetPen(selectionRectangleOutline);
      dc.SetBrush(selectionRectangleFill);
      dc.DrawRectangle(x, y, width, height);
      DrawInstruction(actions[j],
                      instructionMetadata,
                      /*isCondition=*/false,
                      dc,
                      wxPoint(x + iconWidth, y),
                      freeWidth,
                      event,
                      areas,
                      selection);
      ...
      height = DrawInstruction(actions[j],
                               instructionMetadata,
                               /*isCondition=*/false,
                               dc,
                               wxPoint(x + iconWidth, y),
    ...
    // Draw sub actions
    if (instructionMetadata.CanHaveSubInstructions())
      y += DrawActionsList(actions[j].GetSubInstructions(),
                           dc,
                           x + 18,
                           y,
                           width - 18,
...
unsigned int EventsRenderingHelper::GetRenderedConditionsListHeight(
    const gd::InstructionsList& conditions,
    int width,
    const gd::Platform& platform) {
  int y = 0;
  ...
  if (conditions.empty()) return fontCharacterHeight;
  ...
  for (std::size_t j = 0; j < conditions.size(); j++) {
    ...
    int height =
        GetTextHeightInArea(InstructionSentenceFormatter::Get()->Translate(
                                conditions[j], instructionMetadata),
                            freeWidth);
    y += height;
    ...
    // Sub conditions
    if (instructionMetadata.CanHaveSubInstructions())
      y += GetRenderedConditionsListHeight(
          conditions[j].GetSubInstructions(), width - 18, platform);
  }

  return y;
}

unsigned int EventsRenderingHelper::GetRenderedActionsListHeight(
    const gd::InstructionsList& actions,
    int width,
    const gd::Platform& platform) {
  int y = 0;
  ...
  // Draw Actions rectangle
  if (actions.empty()) return fontCharacterHeight;
  ...
  for (std::size_t j = 0; j < actions.size(); j++) {
    ...
    // Draw sub actions
    if (instructionMetadata.CanHaveSubInstructions())
      y += GetRenderedActionsListHeight(
          actions[j].GetSubInstructions(), width - 18, platform);
  }

  return y;
}

int EventsRenderingHelper::DrawInstruction(
  ...
  std::vector<std::pair<gd::String, TextFormatting> > formattedStr =
      InstructionSentenceFormatter::Get()->GetAsFormattedText(
  ...
  for (std::size_t i = 0; i < formattedStr.size(); ++i) {
    ...
    dc.SetTextForeground(!event->IsDisabled()
                             ? formattedStr[i].second.GetWxColor()
                             : wxColour(160, 160, 160));
    font.SetWeight(formattedStr[i].second.bold ? wxFONTWEIGHT_BOLD
                                               : wxFONTWEIGHT_NORMAL);
    font.SetStyle(formattedStr[i].second.italic ? wxFONTSTYLE_ITALIC
                                                : wxFONTSTYLE_NORMAL);
    ...
    if (formattedStr[i].second.userData != gd::String::npos) {
      ParameterItem item(
          formattedStr[i].second.userData < instruction.GetParameters().size()
              ? &instruction.GetParameter(formattedStr[i].second.userData)
              : NULL,
          event);

      std::size_t textLength = text.size();
      int parameterWidth =
          (textLength * fontCharacterWidth <= freeWidth - lastPos.x + point.x
               ? textLength * fontCharacterWidth
               : freeWidth - lastPos.x + point.x);
      if (selection.ParameterHighLighted(item)) {
        dc.SetBrush(wxBrush(wxColour(255, 163, 163)));
        dc.SetPen(wxPen(wxColour(209, 0, 0)));
        dc.DrawRectangle(
            lastPos.x, lastPos.y, parameterWidth, fontCharacterHeight);
      }

      areas.AddParameterArea(
          wxRect(lastPos.x, lastPos.y, parameterWidth, fontCharacterHeight),
          item);
    }

    dc.SetFont(font);
    lastPos = DrawTextInArea(
        text, dc, wxRect(point.x, point.y, freeWidth, 0 /*Useless*/), lastPos);
  }
...
void EventsRenderingHelper::DrawNiceRectangle(wxDC& dc,
                                              const wxRect& rect) const {
  {
    ...
    dc.GradientFillLinear(
        background, niceRectangleFill1, niceRectangleFill2, wxSOUTH);
  }
...
EventsRenderingHelper* EventsRenderingHelper::Get() {
  if (NULL == singleton) singleton = new EventsRenderingHelper;
