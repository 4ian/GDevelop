...
class GD_CORE_API EventsRenderingHelper {
 public:
   ...
   * \param areas EventsEditorItemsAreas object when drawn areas will be
   * registered \param selection EventsEditorSelection object providing
   * information about selection \param platform The platform currently used
   * \return Height used for the drawing
   */
  int DrawConditionsList(gd::InstructionsList &conditions,
                         wxDC &dc,
                         int x,
                         int y,
                         int width,
   ...
   */
  int DrawActionsList(gd::InstructionsList &actions,
                      wxDC &dc,
                      int x,
                      int y,
                      int width,
   ...
   * \return Height used for the drawing
   */
  unsigned int GetRenderedConditionsListHeight(
   ...
   */
  unsigned int GetRenderedActionsListHeight(const gd::InstructionsList &actions,
                                            int width,
                                            const gd::Platform &platform);

  inline unsigned int GetConditionsColumnWidth() const {
    return conditionsColumnWidth;
  };
  inline void SetConditionsColumnWidth(unsigned int conditionsColumnWidth_) {
   ...
   */
  unsigned int GetTextHeightInArea(const gd::String &text,
                                   unsigned int widthAvailable);
   ...
   * \param areas EventsEditorItemsAreas object when drawn areas will be
   * registered \param selection EventsEditorSelection object providing
   * information about selection \return Height used for the drawing
   */
  ...
  inline const wxHtmlDCRenderer &GetHTMLRenderer() const {
    return htmlRenderer;
  };
  inline wxHtmlDCRenderer &GetHTMLRenderer() { return htmlRenderer; };

  const wxBrush &GetSelectedRectangleFillBrush() const {
    return selectionRectangleFill;
  }
  wxBrush &GetSelectedRectangleFillBrush() { return selectionRectangleFill; }

  const wxPen &GetSelectedRectangleOutlinePen() const {
    return selectionRectangleOutline;
  }
  wxPen &GetSelectedRectangleOutlinePen() { return selectionRectangleOutline; }

  const wxBrush &GetHighlightedRectangleFillBrush() const {
    return highlightRectangleFill;
  }
  wxBrush &GetHighlightedRectangleFillBrush() { return highlightRectangleFill; }

  const wxPen &GetHighlightedRectangleOutlinePen() const {
    return highlightRectangleOutline;
  }
  wxPen &GetHighlightedRectangleOutlinePen() {
    return highlightRectangleOutline;
  }

  const wxPen &GetConditionsRectangleOutlinePen() const {
    return conditionsRectangleOutline;
  }
  wxPen &GetConditionsRectangleOutlinePen() {
    return conditionsRectangleOutline;
  }

  const wxPen &GetActionsRectangleOutlinePen() const {
    return actionsRectangleOutline;
  }
  wxPen &GetActionsRectangleOutlinePen() { return actionsRectangleOutline; }

  const wxBrush &GetConditionsRectangleFillBrush() const {
    return conditionsRectangleFill;
  }
  wxBrush &GetConditionsRectangleFillBrush() { return conditionsRectangleFill; }

  const wxBrush &GetActionsRectangleFillBrush() const {
    return actionsRectangleFill;
  }
