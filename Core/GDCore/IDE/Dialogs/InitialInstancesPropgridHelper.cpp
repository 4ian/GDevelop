...
void InitialInstancesPropgridHelper::RefreshFrom(
    const std::vector<gd::InitialInstance*>& selectedInitialInstances) {
  if (grid == NULL) return;
  ...
  for (std::size_t i = 0; i < selectedInitialInstances.size(); ++i) {
    if (i == 0) {
      ...
      zOrderProperty =
          gd::String::From(selectedInitialInstances[0]->GetZOrder());
      layerProperty = selectedInitialInstances[0]->GetLayer();
      customSizeProperty = selectedInitialInstances[0]->HasCustomSize();
      widthProperty =
          gd::String::From(selectedInitialInstances[0]->GetCustomWidth());
      heightProperty =
          gd::String::From(selectedInitialInstances[0]->GetCustomHeight());
      lockedProperty = selectedInitialInstances[0]->IsLocked();
      customProperties =
          selectedInitialInstances[0]->GetCustomProperties(project, layout);
    } else {
      if (gd::String::From(selectedInitialInstances[i]->GetX()) != xProperty)
        xProperty = _("(Multiples values)");
      if (gd::String::From(selectedInitialInstances[i]->GetY()) != yProperty)
        yProperty = _("(Multiples values)");
      if (gd::String::From(selectedInitialInstances[i]->GetAngle()) !=
          angleProperty)
        angleProperty = _("(Multiples values)");
      if (gd::String::From(selectedInitialInstances[i]->GetZOrder()) !=
          zOrderProperty)
        zOrderProperty = _("(Multiples values)");
      if (selectedInitialInstances[i]->GetLayer() != layerProperty)
        layerProperty = _("(Multiples values)");
      if (!selectedInitialInstances[i]->HasCustomSize())
        customSizeProperty = false;
      if (!selectedInitialInstances[i]->IsLocked()) lockedProperty = false;
      if (gd::String::From(selectedInitialInstances[i]->GetCustomWidth()) !=
          widthProperty)
        widthProperty = _("(Multiples values)");
      if (gd::String::From(selectedInitialInstances[i]->GetCustomHeight()) !=
          heightProperty)
        heightProperty = _("(Multiples values)");

      // Merge custom properties
      std::map<gd::String, gd::PropertyDescriptor> instanceCustomProperties =
          selectedInitialInstances[i]->GetCustomProperties(project, layout);
      for (std::map<gd::String, gd::PropertyDescriptor>::iterator it =
               instanceCustomProperties.begin();
           it != instanceCustomProperties.end();
           ++it) {
        if (customProperties.find(it->first) == customProperties.end())
          continue;
      ...
      for (std::map<gd::String, gd::PropertyDescriptor>::iterator it =
               customProperties.begin();
           it != customProperties.end();) {
        if (instanceCustomProperties.find(it->first) ==
            instanceCustomProperties.end())
  ...
  grid->Append(
      new wxStringProperty(_("Angle"), "INSTANCE_ANGLE", angleProperty));
  grid->Append(
      new wxStringProperty(_("Z Order"), "INSTANCE_Z", zOrderProperty));
  grid->Append(
      new wxStringProperty(_("Layer"), "INSTANCE_LAYER", layerProperty));
  grid->Append(
      new wxBoolProperty(_("Locked"), "INSTANCE_LOCKED", lockedProperty));
  grid->Append(new wxBoolProperty(
      _("Custom size?"), "INSTANCE_CUSTOM_SIZE", customSizeProperty));
  grid->EnableProperty(
      grid->AppendIn("INSTANCE_CUSTOM_SIZE",
                     new wxStringProperty(
                         _("Width"), "INSTANCE_SIZE_WIDTH", widthProperty)),
      customSizeProperty);
  grid->EnableProperty(
      grid->AppendIn("INSTANCE_CUSTOM_SIZE",
                     new wxStringProperty(
                         _("Height"), "INSTANCE_SIZE_HEIGHT", heightProperty)),
      customSizeProperty);
  for (std::map<gd::String, gd::PropertyDescriptor>::iterator it =
           customProperties.begin();
       it != customProperties.end();
       ++it) {
    grid->Append(
        new wxStringProperty(it->first, wxPG_LABEL, it->second.GetValue()));
  }
  ...
  if (selectedInitialInstances.size() == 1) {
    grid->Append(new wxStringProperty(
        _("Variables") + " (" +
            gd::String::From(
                selectedInitialInstances[0]->GetVariables().Count()) +
            ")",
        "INSTANCE_VARIABLES",
        _("Click to edit...")));

    grid->SetPropertyCell("INSTANCE_VARIABLES",
                          1,
                          _("Click to edit..."),
                          wxNullBitmap,
  ...
  if (event.GetColumn() == 1)  // Manage button-like properties
  {
    ...
    if (event.GetPropertyName() == "INSTANCE_VARIABLES") {
      gd::ChooseVariableDialog dialog(
          NULL, selectedInitialInstances[0]->GetVariables(), true);
      dialog.ShowModal();
...
void InitialInstancesPropgridHelper::OnPropertyChanged(
    const std::vector<gd::InitialInstance*>& selectedInitialInstances,
    wxPropertyGridEvent& event) {
  if (grid == NULL) return;
  if (selectedInitialInstances.empty()) return;
  ...
  if (event.GetPropertyName() == "INSTANCE_CUSTOM_SIZE") {
    bool hasCustomSize =
        grid->GetProperty("INSTANCE_CUSTOM_SIZE")->GetValue().GetBool();

    grid->EnableProperty("INSTANCE_CUSTOM_SIZE.INSTANCE_SIZE_WIDTH",
                         hasCustomSize);
    grid->EnableProperty("INSTANCE_CUSTOM_SIZE.INSTANCE_SIZE_HEIGHT",
                         hasCustomSize);

    for (std::size_t i = 0; i < selectedInitialInstances.size(); ++i)
      selectedInitialInstances[i]->SetHasCustomSize(hasCustomSize);
  } else if (event.GetPropertyName() ==
             "INSTANCE_CUSTOM_SIZE.INSTANCE_SIZE_WIDTH") {
    for (std::size_t i = 0; i < selectedInitialInstances.size(); ++i)
      selectedInitialInstances[i]->SetCustomWidth(event.GetValue().GetReal());
  } else if (event.GetPropertyName() ==
             "INSTANCE_CUSTOM_SIZE.INSTANCE_SIZE_HEIGHT") {
    for (std::size_t i = 0; i < selectedInitialInstances.size(); ++i)
    ...
    for (std::size_t i = 0; i < selectedInitialInstances.size(); ++i) {
      selectedInitialInstances[i]->UpdateCustomProperty(
