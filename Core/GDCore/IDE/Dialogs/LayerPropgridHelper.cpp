...
LayerPropgridHelper::LayerPropgridHelper(gd::Project& project_,
                                         gd::Layout& layout_)
...
void LayerPropgridHelper::RefreshFrom(const Layer& layer) {
  ...
  grid->Append(
      new wxBoolProperty(_("Visible"), "LAYER_VISIBLE", layer.GetVisibility()));
  grid->Append(
      new wxStringProperty(_("Help"), "HELP", _("Click to see help...")));
  grid->SetPropertyCell("HELP",
                        1,
                        _("Help"),
                        wxNullBitmap,
  ...
  grid->Append(
      new wxStringProperty(_("Add a camera"), "CAMERA_ADD", _("Add...")));
  grid->SetPropertyCell("CAMERA_ADD",
                        1,
                        _("Add..."),
                        wxNullBitmap,
                        wxSystemSettings::GetColour(wxSYS_COLOUR_HOTLIGHT));
  ...
  for (std::size_t i = 0; i < layer.GetCameraCount(); ++i) {
    ...
    grid->AppendIn(
        ...
        new wxPropertyCategory(
            wxString::Format(
                wxString(i == 0 ? _("Camera %d")
                                : _("Camera %d (native games only)")),
                i),
            "CAMERA" + suffix));

    grid->Append(
        new wxStringProperty(_(""), "CAMERA_REMOVE" + suffix, _("Remove")));
    grid->SetPropertyCell("CAMERA_REMOVE" + suffix,
                          1,
                          _("Remove"),
                          wxNullBitmap,
                          wxSystemSettings::GetColour(wxSYS_COLOUR_HOTLIGHT));
    ...
    grid->AppendIn("CAMERA" + suffix,
                   new wxBoolProperty(_("Custom size"),
                                      "CAMERA_CUSTOM_SIZE" + suffix,
                                      !camera.UseDefaultSize()));
    grid->EnableProperty(
        grid->Append(new wxStringProperty(_("Width"),
                                          "CAMERA_SIZE_WIDTH" + suffix,
                                          gd::String::From(camera.GetWidth()))),
        !camera.UseDefaultSize());
    grid->EnableProperty(grid->Append(new wxStringProperty(
                             _("Height"),
                             "CAMERA_SIZE_HEIGHT" + suffix,
                             gd::String::From(camera.GetHeight()))),
                         !camera.UseDefaultSize());
    grid->AppendIn("CAMERA" + suffix,
                   new wxBoolProperty(_("Custom viewport (native games only)"),
                                      "CAMERA_CUSTOM_VIEWPORT" + suffix,
                                      !camera.UseDefaultViewport()));
    grid->EnableProperty(grid->Append(new wxStringProperty(
                             _("Top-left x"),
                             "CAMERA_VIEWPORT_X1" + suffix,
                             gd::String::From(camera.GetViewportX1()))),
                         !camera.UseDefaultViewport());
    grid->EnableProperty(grid->Append(new wxStringProperty(
                             _("Top-left y"),
                             "CAMERA_VIEWPORT_Y1" + suffix,
                             gd::String::From(camera.GetViewportY1()))),
                         !camera.UseDefaultViewport());
    grid->EnableProperty(grid->Append(new wxStringProperty(
                             _("Bottom-right x"),
                             "CAMERA_VIEWPORT_X2" + suffix,
                             gd::String::From(camera.GetViewportX2()))),
                         !camera.UseDefaultViewport());
    grid->EnableProperty(grid->Append(new wxStringProperty(
                             _("Bottom-right y"),
                             "CAMERA_VIEWPORT_Y2" + suffix,
                             gd::String::From(camera.GetViewportY2()))),
                         !camera.UseDefaultViewport());
  }
  ...
  grid->Append(
      new wxStringProperty(_("Add an effect"), "EFFECT_ADD", _("Add...")));
  grid->SetPropertyCell("EFFECT_ADD",
                        1,
                        _("Add..."),
                        wxNullBitmap,
                        wxSystemSettings::GetColour(wxSYS_COLOUR_HOTLIGHT));
  ...
  for (std::size_t i = 0; i < layer.GetEffectsCount(); ++i) {
    ...
    grid->AppendIn(
        "EFFECTS",
        new wxPropertyCategory(wxString::Format(wxString(_("Effect %d")), i),
                               "EFFECT" + suffix));

    grid->Append(
        new wxStringProperty(_(""), "EFFECT_REMOVE" + suffix, _("Remove")));
    grid->SetPropertyCell("EFFECT_REMOVE" + suffix,
                          1,
                          _("Remove"),
                          wxNullBitmap,
                          wxSystemSettings::GetColour(wxSYS_COLOUR_HOTLIGHT));
    ...
    grid->Append(new wxStringProperty(
        _("Name"), "EFFECT_NAME" + suffix, effect.GetName()));

    wxEnumProperty* prop =
        new wxEnumProperty(_("Effect"), "EFFECT_EFFECT" + suffix, effectNames);
    ...
    for (auto parameter : effect.GetAllParameters()) {
      ...
      grid->Append(new wxStringProperty(
          name, "EFFECT_PARAMETER_" + name + suffix, gd::String::From(value)));
    }
  }
...
bool LayerPropgridHelper::OnPropertySelected(Layer& layer,
  ...
  if (event.GetColumn() == 1)  // Manage button-like properties
  {
    if (event.GetPropertyName() == "CAMERA_ADD") {
      ...
      gd::String effectName = wxGetSingleChoice(
          _("Choose an effect to add to the layer"), _("Effects"), effectNames);
      ...
      for (auto it : camerasProperties) {
        if (event.GetPropertyName().StartsWith(it.first)) {
          ...
          if (cameraId < 0 || cameraId >= layer.GetCameraCount()) return false;
      ...
      for (auto it : effectsProperties) {
        if (event.GetPropertyName().StartsWith(it.first)) {
          ...
          if (effectId < 0 || effectId >= layer.GetEffectsCount()) return false;
...
bool LayerPropgridHelper::OnPropertyChanged(Layer& layer,
                                            wxPropertyGridEvent& event) {
  if (!grid) return false;
  ...
  auto validateViewportValue = [&event]() {
    ...
    if (value < 0 || value > 1) {
      gd::LogMessage(
          _("Viewport values are a factor of the window size, and must be "
            "between 0 and 1."));
      event.Veto();
      return false;
    }
  ...
  auto validateCameraSize = [&event]() {
    ...
    if (value <= 0) {
      gd::LogMessage(
          _("A camera should have a size that is at least 1 pixel."));
      event.Veto();
      return false;
    }
  ...
  std::map<wxString, std::function<bool(int)>> camerasProperties = {
      ...
      {"CAMERA_SIZE_WIDTH",
       [&](int id) {
         if (validateCameraSize()) {
           layer.GetCamera(id).SetSize(event.GetValue().GetInteger(),
      ...
      {"CAMERA_SIZE_HEIGHT",
       [&](int id) {
         if (validateCameraSize()) {
           layer.GetCamera(id).SetSize(layer.GetCamera(id).GetWidth(),
                                       event.GetValue().GetInteger());
         }
         return false;
       }},
      {"CAMERA_CUSTOM_VIEWPORT",
       [&](int id) {
  ...
  std::map<wxString, std::function<bool(int)>> effectsProperties = {
      ...
      {"EFFECT_EFFECT",
       [&](int id) {
         ...
         if (effectNameId >= effectNames.size()) return false;
      ...
      {"EFFECT_PARAMETER_", [&](int id) {
         ...
         layer.GetEffect(id).SetParameter(
             name, gd::String(event.GetValue().GetString()).To<float>());
         return false;
       }}};
