...
/**
 * \brief Describe a GUI element drawn on a LayoutEditorCanvas, like a resize
 * button or a context button.
 *
 * These buttons are most of time created by the specialization of the
 * LayoutEditorCanvas, during rendering. When LayoutEditorCanvas detects a click
 * on a button, it calls OnGuiElementHovered/Pressed/Released.
 *
...
class LayoutEditorCanvasGuiElement {
 public:
  ...
  wxRect
      area;  ///< The area where the button is displayed, in layout coordinates.
  InitialInstance*
      associatedInitialInstance;  ///< The associated initial instance, if any.
};
...
class GD_CORE_API LayoutEditorCanvas : public wxPanel, public sf::RenderWindow {
 ...
 public:
   ...
   * \param instances The instances to edit: most of the time, they are either
   * the layout instances or the external layout instances. \param options The
   * options of the editor \param mainFrameWrapper Wrapper to let the editor
   * access to the main frame. \param externalLayout The external layout being
   * edited, or NULL.
   */
  ...
  /**
   * \brief Return a reference to the game owning the scene being edited inside
   * the editor.
   */
  ...
  /**
   * \brief Provide an access to the main frame wrapper that can be needed by
   * some previewers.
   */
   ...
   *
   * When LayoutEditorCanvas will make changes, it will be notifying the
   * associated editors thanks to the members functions declared in
   * gd::LayoutEditorCanvasAssociatedEditor.
   *
   * The IDE, when creating a LayoutEditorCanvas, is responsible for calling
   * this method, as it is the IDE which is also creating the others editors.
   *
   * \param editor Pointer to the editor to be notified of changes. Must be not
   * NULL.
   */
  void AddAssociatedEditor(gd::LayoutEditorCanvasAssociatedEditor* editor);

  /**
   * Can be called by the IDE to notify the editor that it is displayed thanks
   * to a wxAuiManager and that it can uses it to display it own sub editors.
   */
   ...
   */
  virtual void SetScrollbars(wxScrollBar* hScrollbar_,
                             wxScrollBar* vScrollbar_) {
    hScrollbar = hScrollbar_;
    vScrollbar = vScrollbar_;
  }

  /**
   * Must be called by the IDE when the parent control size has changed:
   * The editor must update its size ( wxWindow::SetSize ) according to the
   * parentControl size and the scrollbars.
   */
  void UpdateSize();

  /**
   * Called by the IDE when the editor is active to connect ribbon buttons to
   * the editor methods
   */
  void ConnectEvents();
   ...
   * Called by the IDE.
   * \return The wxRibbonButtonBar to be used for the custom tools. Can be
   * accessed then thanks to the MainFrameWrapper object.
   */
  static wxRibbonButtonBar* CreateRibbonPage(wxRibbonPage* page);

  /**
   * Can be called by the IDE to force the recreation of the custom toolbar of
   * the ribbon, for example when switching the focus to a LayoutEditorCanvas.
   *
  ...
  /**
   * Set a function to be called when the ribbon button bar containing the
   * layout editor tools is updated.
   */
  void OnRibbonButtonBarUpdated(std::function<void(wxRibbonButtonBar*)> cb) {
    onRibbonButtonBarUpdatedCb = cb;
  };
  ...
  /**
   * The editors' parent panel can forward the event of the scrollbars to these
   * methods. \see OnhScrollbarScroll
   */
  virtual void OnvScrollbarScroll(wxScrollEvent& event);

  /**
   * The editors' parent panel can forward the event of the scrollbars to these
   * methods. \see OnvScrollbarScroll
   */
  ...
  /**
   * Can be called so that the editor make the initial instance passed in
   * parameters visible.
   */
  ...
  /**
   * Return true if the scene is being previewed ( i.e. : IsEditing() == false )
   * but the preview is paused.
   */
  ...
  /**
   * \brief Can be called ( by previewers for example ) to ask the editor to go
   * back to editing state.
   */
  void GoToEditingState();

  /**
   * \brief Enable or disable idle events. Disabling them avoid the scene to be
   * constantly rendered. \note Used by the scene editor when the user switch to
   * the event editor.
   */
  void EnableIdleEvents(bool enable = true) { enableIdleEvents = enable; };

  virtual sf::Vector2f GetInitialInstanceSize(
      gd::InitialInstance& instance) const;
  virtual sf::Vector2f GetInitialInstanceOrigin(
      gd::InitialInstance& instance) const;
 ...
 protected:
  ...
  virtual void OnGuiElementHovered(
      const LayoutEditorCanvasGuiElement& guiElement);
  virtual void OnGuiElementPressed(
      const LayoutEditorCanvasGuiElement& guiElement);
  virtual void OnGuiElementReleased(
      const LayoutEditorCanvasGuiElement& guiElement);
  ...
  void AddSmallButtonGuiElement(
      std::vector<std::shared_ptr<sf::Shape> >& target,
      const sf::Vector2f& position,
      const gd::String& buttonName);
  void DrawSelectionRectangleGuiElement(
      std::vector<std::shared_ptr<sf::Shape> >& target,
      const sf::FloatRect& rectangle);
  void DrawAngleButtonGuiElement(
      std::vector<std::shared_ptr<sf::Shape> >& target,
      const sf::Vector2f& position,
      float angle);
  void DrawHighlightRectangleGuiElement(
      std::vector<std::shared_ptr<sf::Shape> >& target,
      const sf::FloatRect& rectangle);
  sf::Vector2f ConvertToWindowCoordinates(float x,
                                          float y,
                                          const sf::View& view);

  /**
   * Can be called ( most of the time when the layout is rendered when editing )
   * to declare that there is an active gui element at a position, along with
   * some other informations.
   *
   * \see LayoutEditorCanvasGuiElement
   * \see ClearAllGuiElements
   */
  void AddGuiElement(LayoutEditorCanvasGuiElement element) {
    guiElements.push_back(element);
  };

  /**
   * Clear the list containing the description of gui elements. Should be called
   * at each rendering when editing, before adding the new gui elements just
   * rendered.
   */
  void ClearAllGuiElements() { guiElements.clear(); };

  /**
   * Called when the ribbon buttons of edition mode must be created inside
   * ribbonToolbar. \see CreatePreviewRibbonTools
   */
  void CreateEditionRibbonTools();

  /**
   * \brief Toggle the disabled state of the Edition and Preview button
   * according to the state of the editor (editing or previewing).
   */
  ...
  InitialInstance* GetInitialInstanceUnderCursor(
      bool pickOnlyLockedInstances = false) {
    return GetInitialInstanceAtPosition(
        GetMouseXOnLayout(), GetMouseYOnLayout(), pickOnlyLockedInstances);
  };

  /**
   * Return a pointer to the smallest initial instance which intersect with the
   * specified position Can be NULL.
   */
  InitialInstance* GetInitialInstanceAtPosition(
      double xPosition, double yPosition, bool pickOnlyLockedInstances = false);

  /**
   * Called when a changes has been made ( Something which is lasting, e.g. when
   * an object has been moved, but not when it is being moved ).
   */
  void ChangesMade();

  /**
   * Tool function returning the object used to display during the \a instance
   * when editing.
   */
  gd::Object* GetObjectLinkedToInitialInstance(
      gd::InitialInstance& instance) const;
  ...
  gd::Layout&
      layout;  ///< The layout being edited or used to edit the instances
  gd::ExternalLayout*
      externalLayout;  ///< The external layout being edited, if any.
  gd::InitialInstancesContainer&
      instances;  ///< The initial instances of objects being edited
  LayoutEditorCanvasOptions& options;
  gd::MainFrameWrapper& mainFrameWrapper;
  ...
  wxWindow* parentControl;  ///< The wxWidgets control owning the editor (
                            ///< probably a wxPanel )
  wxAuiManager* parentAuiManager;  ///< Pointer to the wxAuiManager displayed
  ...
  bool altPressed;
  bool shiftPressed;
  double oldMouseX;  ///< The mouse X position which was usually stored the last
                     ///< time a right click happened.
  double oldMouseY;  ///< The mouse Y position which was usually stored the last
                     ///< time a right click happened.
  ...
  std::vector<std::shared_ptr<gd::InitialInstancesContainer> >
      history;  ///< History of changes
  std::vector<std::shared_ptr<gd::InitialInstancesContainer> >
      redoHistory;  ///< Histoy of changes so as to "redo"
  std::shared_ptr<gd::InitialInstancesContainer>
      latestState;  ///< Necessary to keep track of what changed

  std::vector<LayoutEditorCanvasGuiElement> guiElements;

  // State
  bool editing;  ///< True if the layout is being edited, false if a preview is
                 ///< running.
  std::shared_ptr<gd::LayoutEditorPreviewer>
      currentPreviewer;  ///< The previewer being used to preview the layout.
