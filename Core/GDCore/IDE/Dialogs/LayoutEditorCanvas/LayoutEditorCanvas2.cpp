...
void LayoutEditorCanvas::OnUpdate() {
  if (!editing) {
    ...
    if (mainFrameWrapper.GetIDEWorkingDirectory() !=
        wxGetCwd())  // Force the correct working directory.
    ...
    if (!project.imagesChanged.empty()) {
      if (wxDirExists(wxFileName::FileName(project.GetProjectFile()).GetPath()))
        wxSetWorkingDirectory(
            wxFileName::FileName(project.GetProjectFile())
                .GetPath());  // Resources loading stuff incoming: Switch
                              // current work dir.
      ...
      wxSetWorkingDirectory(
          mainFrameWrapper
              .GetIDEWorkingDirectory());  // Go back to the IDE cwd.
    }
    if (firstRefresh) {
      firstRefresh = false;
      ReloadResources();
    ...
    RenderEdittime();
    if (vScrollbar && hScrollbar && !hScrollbar->HasFocus() &&
        !vScrollbar->HasFocus())
      UpdateScrollbars();
  }
}

void LayoutEditorCanvas::DrawSelectionRectangleGuiElement(
    std::vector<std::shared_ptr<sf::Shape> >& target,
    const sf::FloatRect& rectangle) {
  // Create the shapes
  std::shared_ptr<sf::Shape> selection = std::make_shared<sf::RectangleShape>(
      sf::Vector2f(rectangle.width, rectangle.height));
...
void LayoutEditorCanvas::DrawAngleButtonGuiElement(
    std::vector<std::shared_ptr<sf::Shape> >& target,
  ...
  std::shared_ptr<sf::Shape> angleButton = std::make_shared<sf::RectangleShape>(
      sf::Vector2f(smallButtonSize, smallButtonSize));
  angleButton->setPosition(position +
                           sf::Vector2f(25.0 * cos(angle / 180.0 * 3.14159),
                                        25.0 * sin(angle / 180.0 * 3.14159)));
  angleButton->setOutlineColor(sf::Color(0, 0, 0, 255));
  angleButton->setOutlineThickness(1);
  angleButton->setOrigin(
      sf::Vector2f(smallButtonSize / 2.0, smallButtonSize / 2.0));

  std::shared_ptr<sf::Shape> line =
      std::make_shared<sf::RectangleShape>(sf::Vector2f(26, 1));
  line->setPosition(position +
                    sf::Vector2f(3.0 * cos(angle / 180.0 * 3.14159),
                                 3.0 * sin(angle / 180.0 * 3.14159)));
  ...
  guiElement.area = wxRect(angleButton->getPosition().x - smallButtonSize / 2.0,
                           angleButton->getPosition().y - smallButtonSize / 2.0,
                           smallButtonSize,
                           smallButtonSize);
  guiElements.push_back(guiElement);
  if (!guiElement.area.Contains(wxPoint(sf::Mouse::getPosition(*this).x,
  ...
  angleButtonCenter =
      position;  // Save the position of the center to calculate the new angle
                 // when we'll be dragging the button ( See OnMoving method )
...
void LayoutEditorCanvas::DrawHighlightRectangleGuiElement(
    std::vector<std::shared_ptr<sf::Shape> >& target,
    const sf::FloatRect& rectangle) {
  std::shared_ptr<sf::Shape> highlight = std::make_shared<sf::RectangleShape>(
      sf::Vector2f(rectangle.width, rectangle.height));
...
void LayoutEditorCanvas::AddSmallButtonGuiElement(
    std::vector<std::shared_ptr<sf::Shape> >& target,
    const sf::Vector2f& position,
    const gd::String& buttonName) {
  // Declare the button as a gui element
  gd::LayoutEditorCanvasGuiElement guiElement;
  guiElement.name = buttonName;
  guiElement.area =
      wxRect(position.x, position.y, smallButtonSize, smallButtonSize);
  guiElements.push_back(guiElement);

  // Draw button
  std::shared_ptr<sf::Shape> button = std::make_shared<sf::RectangleShape>(
      sf::Vector2f(smallButtonSize, smallButtonSize));
...
class InstancesRenderer : public gd::InitialInstanceFunctor {
 public:
  InstancesRenderer(
      LayoutEditorCanvas& editor_,
      gd::InitialInstance* highlightedInstance_,
      std::vector<std::shared_ptr<sf::Shape> >& guiElementsShapes_)
  ...
  virtual void operator()(gd::InitialInstance& instance) {
    gd::Object* associatedObject =
        editor.GetObjectLinkedToInitialInstance(instance);
    if (!associatedObject) return;

    associatedObject->DrawInitialInstance(
        instance, editor, editor.project, editor.layout);
    sf::Vector2f origin = associatedObject->GetInitialInstanceOrigin(
        instance, editor.project, editor.layout);
    sf::Vector2f size =
        sf::Vector2f(instance.GetCustomWidth(), instance.GetCustomHeight());
    if (!instance.HasCustomSize())
      size = associatedObject->GetInitialInstanceDefaultSize(
          instance, editor.project, editor.layout);

    // Selection rectangle
    if (editor.selectedInstances.find(&instance) !=
        editor.selectedInstances.end()) {
      sf::Vector2f rectangleOrigin =
          editor.ConvertToWindowCoordinates(instance.GetX() - origin.x,
                                            instance.GetY() - origin.y,
                                            editor.editionView);
      sf::Vector2f rectangleEnd =
          editor.ConvertToWindowCoordinates(instance.GetX() - origin.x + size.x,
                                            instance.GetY() - origin.y + size.y,
                                            editor.editionView);

      editor.DrawSelectionRectangleGuiElement(
          guiElementsShapes,
          sf::FloatRect(rectangleOrigin, rectangleEnd - rectangleOrigin));
      ...
      editor.DrawHighlightRectangleGuiElement(
          guiElementsShapes,
          sf::FloatRect(rectangleOrigin, rectangleEnd - rectangleOrigin));
    }
  }
...
void LayoutEditorCanvas::RenderEdittime() {
  clear(sf::Color(layout.GetBackgroundColorRed(),
  ...
  std::vector<std::shared_ptr<sf::Shape> > guiElementsShapes;
  guiElements.clear();
  InstancesRenderer renderer(
      *this, GetInitialInstanceUnderCursor(), guiElementsShapes);
  ...
  for (std::size_t layerIndex = 0; layerIndex < layout.GetLayersCount();
    ...
    if (layout.GetLayer(layerIndex).GetVisibility()) {
      ...
      glLoadIdentity();
      OpenGLTools::PerspectiveGL(
          layout.GetOpenGLFOV(),
          static_cast<double>(getSize().x) / static_cast<double>(getSize().y),
          layout.GetOpenGLZNear(),
          layout.GetOpenGLZFar());
      ...
      instances.IterateOverInstancesWithZOrdering(
          renderer, layout.GetLayer(layerIndex).GetName());
    }
  }

  // Go back to "window" view before drawing GUI elements
  setView(sf::View(sf::Vector2f(getSize().x / 2, getSize().y / 2),
                   sf::Vector2f(getSize().x, getSize().y)));
  ...
  if (renderer.drawResizeButtons) {
    ...
    AddSmallButtonGuiElement(
        guiElementsShapes,
        sf::Vector2f(
            resizeButtonsMinX - gapBetweenButtonsAndRectangle - smallButtonSize,
            resizeButtonsMinY - gapBetweenButtonsAndRectangle -
                smallButtonSize),
        "resizeLeftUp");
    AddSmallButtonGuiElement(
        guiElementsShapes,
        sf::Vector2f(
            0.5 * (resizeButtonsMinX + resizeButtonsMaxX - smallButtonSize),
            resizeButtonsMinY - gapBetweenButtonsAndRectangle -
                smallButtonSize),
        "resizeUp");
    AddSmallButtonGuiElement(
        guiElementsShapes,
        sf::Vector2f(resizeButtonsMaxX + gapBetweenButtonsAndRectangle,
                     resizeButtonsMinY - gapBetweenButtonsAndRectangle -
                         smallButtonSize),
        "resizeRightUp");
    AddSmallButtonGuiElement(
        guiElementsShapes,
        sf::Vector2f(
            resizeButtonsMaxX + gapBetweenButtonsAndRectangle,
            0.5 * (resizeButtonsMinY + resizeButtonsMaxY - smallButtonSize)),
        "resizeRight");
    AddSmallButtonGuiElement(
        guiElementsShapes,
        sf::Vector2f(resizeButtonsMaxX + gapBetweenButtonsAndRectangle,
                     resizeButtonsMaxY + gapBetweenButtonsAndRectangle),
        "resizeRightDown");
    AddSmallButtonGuiElement(
        guiElementsShapes,
        sf::Vector2f(
    ...
    AddSmallButtonGuiElement(
        ...
        sf::Vector2f(
            ...
            0.5 * (resizeButtonsMinY + resizeButtonsMaxY - smallButtonSize)),
        "resizeLeft");
    DrawAngleButtonGuiElement(
        guiElementsShapes,
        sf::Vector2f(
            0.5 * (resizeButtonsMinX + resizeButtonsMaxX - smallButtonSize),
            0.5 * (resizeButtonsMinY + resizeButtonsMaxY - smallButtonSize)),
        renderer.selectionAngle);
  }

  if (isSelecting) {
    sf::Vector2f rectangleOrigin = ConvertToWindowCoordinates(
        selectionRectangle.GetX(), selectionRectangle.GetY(), editionView);

    sf::Vector2f rectangleEnd =
        ConvertToWindowCoordinates(selectionRectangle.GetBottomRight().x,
                                   selectionRectangle.GetBottomRight().y,
                                   editionView);

    DrawSelectionRectangleGuiElement(
        guiElementsShapes,
        sf::FloatRect(rectangleOrigin, rectangleEnd - rectangleOrigin));
  }
...
void LayoutEditorCanvas::RenderWindowMask() {
  sf::Vector2f rectangleOrigin = ConvertToWindowCoordinates(
      editionView.getCenter().x - project.GetMainWindowDefaultWidth() / 2,
      editionView.getCenter().y - project.GetMainWindowDefaultHeight() / 2,
      editionView);

  sf::Vector2f rectangleEnd = ConvertToWindowCoordinates(
      editionView.getCenter().x + project.GetMainWindowDefaultWidth() / 2,
      editionView.getCenter().y + project.GetMainWindowDefaultHeight() / 2,
      editionView);
  ...
  sf::Color maskColor((layout.GetBackgroundColorRed() + 128) % 255,
                      ...
                      (layout.GetBackgroundColorBlue() + 128) % 255,
                      128);
  ...
  }
  {
    sf::RectangleShape mask(
        sf::Vector2f(rectangleOrigin.x, getSize().y - rectangleOrigin.y));
    mask.setPosition(0, rectangleOrigin.y);
    mask.setFillColor(maskColor);
    draw(mask);
  }
  {
    sf::RectangleShape mask(sf::Vector2f(getSize().x - rectangleEnd.x,
                                         getSize().y - rectangleOrigin.y));
    mask.setPosition(rectangleEnd.x, rectangleOrigin.y);
    mask.setFillColor(maskColor);
    draw(mask);
  }
  {
    sf::RectangleShape mask(sf::Vector2f(rectangleEnd.x - rectangleOrigin.x,
                                         getSize().y - rectangleEnd.y));
...
void LayoutEditorCanvas::RenderInitialWindowBorder() {
  ...
  sf::Vertex line[8] = {
      ...
      sf::Vertex(ConvertToWindowCoordinates(
                     project.GetMainWindowDefaultWidth(), 0, editionView),
                 color),
      sf::Vertex(ConvertToWindowCoordinates(
                     project.GetMainWindowDefaultWidth(), 0, editionView),
                 color),
      sf::Vertex(
          ConvertToWindowCoordinates(project.GetMainWindowDefaultWidth(),
                                     project.GetMainWindowDefaultHeight(),
                                     editionView),
          color),
      sf::Vertex(
          ConvertToWindowCoordinates(project.GetMainWindowDefaultWidth(),
          ...
          color),
      sf::Vertex(ConvertToWindowCoordinates(
                     0, project.GetMainWindowDefaultHeight(), editionView),
                 color),
      sf::Vertex(ConvertToWindowCoordinates(
                     0, project.GetMainWindowDefaultHeight(), editionView),
                 color),
...
void LayoutEditorCanvas::RenderGrid() {
  int initialXPos =
      floor((editionView.getCenter().x - editionView.getSize().x / 2) /
            options.gridWidth) -
      options.gridWidth;
  initialXPos *= options.gridWidth;
  int initialYPos =
      floor((editionView.getCenter().y - editionView.getSize().y / 2) /
            options.gridHeight) -
      options.gridHeight;
  initialYPos *= options.gridHeight;

  for (int Xpos = initialXPos + options.gridOffsetX;
       Xpos < (editionView.getCenter().x + editionView.getSize().x / 2);
       Xpos += options.gridWidth) {
    sf::Vertex line[2] = {
        sf::Vertex(ConvertToWindowCoordinates(Xpos, initialYPos, editionView),
                   sf::Color(options.gridR, options.gridG, options.gridB)),
        sf::Vertex(ConvertToWindowCoordinates(
                       Xpos,
                       editionView.getCenter().y + editionView.getSize().y / 2,
                       editionView),
                   sf::Color(options.gridR, options.gridG, options.gridB))};
    draw(line, 2, sf::Lines);
  }

  for (int Ypos = initialYPos + options.gridOffsetY;
       Ypos < (editionView.getCenter().y + editionView.getSize().y / 2);
       Ypos += options.gridHeight) {
    sf::Vertex line[2] = {
        sf::Vertex(ConvertToWindowCoordinates(initialXPos, Ypos, editionView),
                   sf::Color(options.gridR, options.gridG, options.gridB)),
        sf::Vertex(ConvertToWindowCoordinates(
                       editionView.getCenter().x + editionView.getSize().x / 2,
                       Ypos,
                       editionView),
                   sf::Color(options.gridR, options.gridG, options.gridB))};
    draw(line, 2, sf::Lines);
...
sf::Vector2f LayoutEditorCanvas::ConvertToWindowCoordinates(
    float x, float y, const sf::View& view) {
  ...
  return sf::Vector2f(
      (hCoords.x + 1.f) / 2.f * viewport.width + viewport.left,
...
void LayoutEditorCanvas::OnCreateObjectSelected(wxCommandEvent& event) {
  ...
  if (chooseTypeDialog.ShowModal() == 0) return;
  ...
  layout.InsertNewObject(project,
                         chooseTypeDialog.GetSelectedObjectType(),
                         name,
                         layout.GetObjectsCount());

  for (auto& it : associatedEditors) it->ObjectsUpdated();

  // Add it on the layout ( Use oldMouseX/Y as the cursor has moved since the
  // right click )
  ...
  project.GetCurrentPlatform().GetChangesNotifier().OnObjectEdited(
      project, &layout, layout.GetObject(name));
...
void LayoutEditorCanvas::OnUnLockSelected(wxCommandEvent& event) {
  gd::InitialInstance* instance = GetInitialInstanceAtPosition(
      oldMouseX, oldMouseY, /*pickOnlyLockedInstances=*/true);
  if (instance) {
...
void LayoutEditorCanvas::OnCopySelected(wxCommandEvent& event) {
  ...
  for (auto& it : selectedInstances) {
    ...
    copiedPositions.push_back(
        std::shared_ptr<gd::InitialInstance>(it.first->Clone()));
...
void LayoutEditorCanvas::OnPasteSelected(wxCommandEvent& event) {
  ...
  vector<std::shared_ptr<gd::InitialInstance> > pastedInstances =
      gd::Clipboard::Get()->Gets();

  for (auto& it : pastedInstances) {
    gd::InitialInstance& instance =
        instances.InsertInitialInstance(*it->Clone());
...
void LayoutEditorCanvas::OnPasteSpecialSelected(wxCommandEvent& event) {
  ...
  if (pastedInstances.empty() ||
      pastedInstances[0] == std::shared_ptr<gd::InitialInstance>())
    return;
  ...
  std::shared_ptr<gd::InitialInstance> instance =
      std::shared_ptr<gd::InitialInstance>(pastedInstances[0]->Clone());
  ...
  for (std::size_t i = 0; i < dialog.GetYCount(); ++i) {
    for (std::size_t j = 0; j < dialog.GetXCount(); ++j) {
      gd::InitialInstance& insertedInstance =
          instances.InsertInitialInstance(*instance);
...
void LayoutEditorCanvas::UpdateSize() {
  ...
  if (editing) {
    if (parentControl->GetSize().GetWidth() <= 0 ||
        parentControl->GetSize().GetHeight() <= 0)
      return;

    unsigned int width = parentControl->GetSize().GetWidth() -
                         (vScrollbar ? vScrollbar->GetSize().GetWidth() : 0);
    unsigned int height = parentControl->GetSize().GetHeight() -
                          (hScrollbar ? hScrollbar->GetSize().GetHeight() : 0);
    ...
    Window::setSize(sf::Vector2u(project.GetMainWindowDefaultWidth(),
                                 project.GetMainWindowDefaultHeight()));
    wxWindowBase::SetClientSize(project.GetMainWindowDefaultWidth(),
                                project.GetMainWindowDefaultHeight());

    // Scene is centered in preview mode
    wxWindowBase::SetPosition(wxPoint(
        (parentControl->GetSize().GetWidth() - wxWindowBase::GetSize().GetX()) /
            2,
        (parentControl->GetSize().GetHeight() -
         wxWindowBase::GetSize().GetY()) /
            2));
  }
}
...
void LayoutEditorCanvas::OnvScrollbarScroll(wxScrollEvent& event) {
  ...
  int newY =
      event.GetPosition() - (vScrollbar->GetRange() / 2) + (getSize().y / 2);
...
void LayoutEditorCanvas::OnhScrollbarScroll(wxScrollEvent& event) {
  ...
  int newX =
      event.GetPosition() - (hScrollbar->GetRange() / 2) + (getSize().x / 2);
...
void LayoutEditorCanvas::UpdateScrollbars() {
  ...
  int thumbY =
      editionView.getCenter().y + vScrollbar->GetRange() / 2 - getSize().y / 2;
  vScrollbar->SetScrollbar(
      thumbY, getSize().y, vScrollbar->GetRange(), getSize().y);

  int thumbX =
      editionView.getCenter().x + hScrollbar->GetRange() / 2 - getSize().x / 2;
  hScrollbar->SetScrollbar(
      thumbX, getSize().x, hScrollbar->GetRange(), getSize().x);

  // Update the size if needed
  if (hScrollbar->HasFocus() || vScrollbar->HasFocus()) return;

  if (thumbY <= 0 ||
      static_cast<int>(thumbY + getSize().y) >= vScrollbar->GetRange()) {
    int offset = getSize().y;
    vScrollbar->SetScrollbar(thumbY + offset / 2,
                             getSize().y,
                             vScrollbar->GetRange() + offset,
                             getSize().y);
  }

  if (thumbX <= 0 ||
      static_cast<int>(thumbX + getSize().x) >= hScrollbar->GetRange()) {
    int offset = getSize().x;
    hScrollbar->SetScrollbar(thumbX + offset / 2,
                             getSize().x,
                             hScrollbar->GetRange() + offset,
                             getSize().x);
  }
}
void LayoutEditorCanvas::UpdateViewAccordingToZoomFactor() {
  editionView.setSize(getSize().x / options.zoomFactor,
                      getSize().y / options.zoomFactor);
}

void LayoutEditorCanvas::OnHelpBtClick(wxCommandEvent& event) {
  gd::HelpFileAccess::Get()->OpenPage(
      "game_develop/documentation/manual/edit_layout");
}
