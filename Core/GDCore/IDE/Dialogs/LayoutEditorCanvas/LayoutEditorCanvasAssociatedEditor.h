...
/**
 * \brief Base class meant to be used by the IDE for its editors that are
 * associated with a gd::LayoutEditorCanvas.
 *
 * When the IDE create a LayoutEditorCanvas and some others editors ( like an
 * objects editor, also a layers editor probably ), the latter must be aware of
 * changes happening in the LayoutEditorCanvas ( and reciprocally, the
 * LayoutEditorCanvas sometimes must be updated after changes inside these
 * editors. ). This class provides a simple interface to allow these
...
class LayoutEditorCanvasAssociatedEditor {
 public:
  ...
  /**
   * Called when the objects of the layout have been changed inside the
   * gd::LayoutEditorCanvas. ( Typically, LayoutEditorCanvas can allow the user
   * to create a new object by choosing an option in a contextual menu )
   */
  virtual void ObjectsUpdated(){};

  /**
   * Ask the editor to go into a normal ( non greyed ) or disabled ( greyed )
   * state.
   */
