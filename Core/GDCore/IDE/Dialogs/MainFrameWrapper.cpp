...
void gd::MainFrameWrapper::DisableControlsForScenePreviewing() {
  unsigned int notebookCurrentPage =
      editorsNotebook ? editorsNotebook->GetSelection() : 0;
  for (std::size_t i = 0; i < disableOnPreview.size(); ++i)
    disableOnPreview[i]->Disable();
  if (editorsNotebook) editorsNotebook->SetSelection(notebookCurrentPage);
}

void gd::MainFrameWrapper::RemoveControlToBeDisabledOnPreview(
    wxWindow* control) {
  for (std::size_t i = 0; i < disableOnPreview.size(); ++i) {
