...
class GD_CORE_API MainFrameWrapper {
 public:
  /**
   * Constructor, only called by the IDE itself to construct the
   * MainFrameWrapper.
   */
  ...
  /**
   * Return the pointer to the Scene ribbon's wxRibbonButtonBar of the main
   * editor.
   */
  wxRibbonButtonBar* GetRibbonSceneEditorButtonBar() const {
    return ribbonSceneEditorButtonBar;
  };
   ...
   */
  void LockShortcuts(wxWindow* window) {
    scenesLockingShortcuts->push_back(window);
  }
  ...
  void UnLockShortcuts(wxWindow* window) {
    scenesLockingShortcuts->erase(std::remove(scenesLockingShortcuts->begin(),
                                              scenesLockingShortcuts->end(),
                                              window),
                                  scenesLockingShortcuts->end());
  }
  ...
  /**
   * Call this method when a scene is going to be previewed and others controls
   * than the scene editor must be deactivated
   */
   ...
   */
  void EnableControlsAfterScenePreviewing() {
    for (std::size_t i = 0; i < disableOnPreview.size(); ++i)
      disableOnPreview[i]->Enable();
  };

  /**
   * Used by the IDE to setup controls to be disabled when a scene is previewed.
   */
  void AddControlToBeDisabledOnPreview(wxWindow* control) {
    if (find(disableOnPreview.begin(), disableOnPreview.end(), control) ==
        disableOnPreview.end())
      disableOnPreview.push_back(control);
  }
 ...
 private:
  ...
  wxInfoBar* infoBar;
  std::vector<wxWindow*>*
      scenesLockingShortcuts;  ///< When a scene is being previewed, it is added
                               ///< to this list to disable shortcuts.
  std::vector<wxWindow*>
      disableOnPreview;  ///< To be filled with controls to be deactivated when
                         ///< a scene is previewed.
  wxString workingDirectory;  ///< Contains the working directory of the IDE. (
                              ///< i.e. IDE executable directory )
};
