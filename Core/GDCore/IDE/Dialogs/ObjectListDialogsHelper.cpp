...
std::vector<gd::String> ObjectListDialogsHelper::GetMatchingObjects() const {
  ...
  for (std::size_t i = 0; i < project.GetObjectsCount(); i++) {
    ...
    if ((objectTypeAllowed.empty() ||
         project.GetObject(i).GetType() == objectTypeAllowed) &&
        (!searching ||
         (searching &&
          name.CaseFold().find(searchText.CaseFold()) != gd::String::npos))) {
  ...
  for (std::size_t i = 0; i < project.GetObjectGroups().size(); i++) {
    ...
    if ((objectTypeAllowed.empty() ||
         gd::GetTypeOfObject(
             project, layout, project.GetObjectGroups()[i].GetName()) ==
             objectTypeAllowed) &&
        (!searching ||
         (searching &&
          name.CaseFold().find(searchText.CaseFold()) != gd::String::npos))) {
  ...
  for (std::size_t i = 0; i < layout.GetObjectsCount(); i++) {
    ...
    // Only add the object if it has the correct type
    if ((objectTypeAllowed.empty() ||
         layout.GetObject(i).GetType() == objectTypeAllowed) &&
        (!searching ||
         (searching &&
          name.CaseFold().find(searchText.CaseFold()) != gd::String::npos))) {
      results.push_back(name);
  ...
  for (std::size_t i = 0; i < layout.GetObjectGroups().size(); i++) {
    ...
    if ((objectTypeAllowed.empty() ||
         gd::GetTypeOfObject(
             project, layout, layout.GetObjectGroups()[i].GetName()) ==
             objectTypeAllowed) &&
        (!searching ||
         (searching &&
          name.CaseFold().find(searchText.CaseFold()) != gd::String::npos))) {
...
void ObjectListDialogsHelper::RefreshList(wxTreeCtrl* objectsList,
                                          wxTreeItemId* objectsRootItem_,
                                          wxTreeItemId* groupsRootItem_) {
  ...
  wxTreeItemId objectsRootItem =
      objectsList->AppendItem(objectsList->GetRootItem(), _("Objects"), 0);
  wxTreeItemId groupsRootItem =
      objectsList->AppendItem(objectsList->GetRootItem(), _("Groups"), 1);

  AddObjectsToList(objectsList, objectsRootItem, layout, false);
  if (groupsAllowed)
    AddGroupsToList(
        objectsList, groupsRootItem, layout.GetObjectGroups(), false);
  AddObjectsToList(objectsList, objectsRootItem, project, true);
  if (groupsAllowed)
    AddGroupsToList(
        objectsList, groupsRootItem, project.GetObjectGroups(), true);
...
wxTreeItemId ObjectListDialogsHelper::AddObjectsToList(
  ...
  for (std::size_t i = 0; i < objects.GetObjectsCount(); i++) {
    ...
    if ((objectTypeAllowed.empty() ||
         objects.GetObject(i).GetType() == objectTypeAllowed) &&
        (!searching ||
         (searching &&
          name.CaseFold().find(searchText.CaseFold()) != gd::String::npos))) {
...
wxTreeItemId ObjectListDialogsHelper::AddGroupsToList(
    wxTreeCtrl* objectsList,
    wxTreeItemId rootItem,
    const gd::ObjectGroupsContainer& groups,
    bool globalGroups) {
  bool searching = searchText.empty() ? false : true;
  ...
  for (std::size_t i = 0; i < groups.size(); ++i) {
    if ((objectTypeAllowed.empty() ||
         gd::GetTypeOfObject(project, layout, groups[i].GetName()) ==
             objectTypeAllowed) &&
        (!searching ||
         (searching && groups[i].GetName().CaseFold().find(
                           searchText.CaseFold()) != gd::String::npos))) {
...
void ObjectListDialogsHelper::MakeGroupItem(wxTreeCtrl* objectsList,
                                            wxTreeItemId item,
                                            const gd::ObjectGroup& group,
                                            bool globalGroup) {
  objectsList->SetItemText(item, group.GetName());
  objectsList->SetItemImage(item, 1);
  objectsList->SetItemData(
      item,
      new gd::TreeItemStringData(globalGroup ? "GlobalGroup" : "LayoutGroup"));
  if (globalGroup) objectsList->SetItemBold(item, true);

  if (hasGroupExtraRendering) groupExtraRendering(item);
}

void ObjectListDialogsHelper::MakeObjectItem(wxTreeCtrl* objectsList,
                                             wxTreeItemId item,
                                             const gd::Object& object,
                                             bool globalObject) {
  objectsList->SetItemText(item, object.GetName());
  objectsList->SetItemImage(item, MakeObjectItemThumbnail(objectsList, object));
  objectsList->SetItemData(item,
                           new gd::TreeItemStringData(
                               globalObject ? "GlobalObject" : "LayoutObject"));
  if (globalObject) objectsList->SetItemBold(item, true);
}

int ObjectListDialogsHelper::MakeObjectItemThumbnail(wxTreeCtrl* objectsList,
                                                     const gd::Object& object) {
  ...
  wxBitmap thumbnail;
  if (objectsList->GetImageList() &&
      object.GenerateThumbnail(project, thumbnail) && thumbnail.IsOk()) {
