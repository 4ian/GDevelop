...
void ObjectsPropgridHelper::RefreshFrom(const gd::Object* object,
                                        bool displayedAfterInstanceProperties) {
  if (grid == NULL) return;
  if (!object) return;
  auto metadata = gd::MetadataProvider::GetObjectMetadata(
  ...
  grid->EnableProperty(grid->Append(new wxStringProperty(
                           _("Object name"), wxPG_LABEL, object->GetName())),
                       false);
  grid->EnableProperty(grid->Append(new wxStringProperty(
                           _("Kind"), wxPG_LABEL, metadata.GetFullName())),
                       false);
  if (!metadata.GetHelpUrl().empty()) {
    grid->Append(
        new wxStringProperty(_("Help"), wxPG_LABEL, _("Click to see help...")));
    grid->SetPropertyCell(_("Help"),
                          1,
                          _("Click to see help..."),
                          wxNullBitmap,
  ...
  if (properties.empty() ||
      properties.find("PLEASE_ALSO_SHOW_EDIT_BUTTON_THANKS") !=
          properties.end()) {
    //"Click to edit" is not shown if properties are not empty, except if the
    // magic property is set.
    grid->Append(
        new wxStringProperty(_("Edit"), wxPG_LABEL, _("Click to edit...")));
    grid->SetPropertyCell(_("Edit"),
                          1,
                          _("Click to edit..."),
                          wxNullBitmap,
                          wxSystemSettings::GetColour(wxSYS_COLOUR_HOTLIGHT));
  ...
  if (!displayedAfterInstanceProperties) {
    grid->Append(new wxPropertyCategory(
        _("Object variables") + " (" +
            gd::String::From(object->GetVariables().Count()) + ")",
        "OBJECT_VARIABLES_CATEGORY"));
    grid->Append(new wxStringProperty(
        _("Variables"), wxPG_LABEL, _("Click to edit...")));
    grid->SetPropertyCell(_("Variables"),
                          1,
                          _("Click to edit..."),
                          wxNullBitmap,
                          wxSystemSettings::GetColour(wxSYS_COLOUR_HOTLIGHT));
  ...
  grid->Append(new wxPropertyCategory(
      _("Behaviors") + " (" +
          gd::String::From(object->GetAllBehaviors().size()) + ")",
      "AUTO"));
  grid->Append(
      new wxStringProperty(_("Add a behavior"), "AUTO_ADD", _("Add...")));
  grid->SetPropertyCell("AUTO_ADD",
                        1,
                        _("Add..."),
                        wxNullBitmap,
                        wxSystemSettings::GetColour(wxSYS_COLOUR_HOTLIGHT));
  ...
  if (!behaviors.empty()) {
    grid->AppendIn("AUTO",
                   new wxStringProperty("", "AUTO_REMOVE", _("Remove...")));
    grid->SetPropertyCell("AUTO_REMOVE",
                          1,
                          _("Remove..."),
                          wxNullBitmap,
                          wxSystemSettings::GetColour(wxSYS_COLOUR_HOTLIGHT));
  ...
  for (std::size_t i = 0; i < behaviors.size(); ++i) {
    ...
    if (properties.empty() ||
      ...
      // magic property is set.
      grid->Append(new wxStringProperty(
          _("Edit"), wxString("AUTO:" + behaviors[i]), _("Click to edit...")));
      grid->SetPropertyCell(wxString("AUTO:" + behaviors[i]),
                            1,
                            _("Click to edit..."),
                            wxNullBitmap,
                            wxSystemSettings::GetColour(wxSYS_COLOUR_HOTLIGHT));
    ...
    grid->Append(new wxStringProperty(
        "", "AUTO_RENAME:" + behaviors[i], _("Rename...")));
    grid->SetPropertyCell(wxString("AUTO_RENAME:" + behaviors[i]),
                          1,
                          _("Rename..."),
                          wxNullBitmap,
                          wxSystemSettings::GetColour(wxSYS_COLOUR_HOTLIGHT));
...
void ObjectsPropgridHelper::RefreshFrom(
    const std::map<gd::String, gd::PropertyDescriptor>& properties,
    gd::String propertiesNames) {
  for (auto it = properties.begin(); it != properties.end(); ++it) {
    if ((*it).first == "PLEASE_ALSO_SHOW_EDIT_BUTTON_THANKS")
      continue;  // Skip the magic property.
    ...
    if (type == "Choice") {
      ...
      wxEnumProperty* prop =
          new wxEnumProperty(name, propertiesNames, choicesArray);
...
bool ObjectsPropgridHelper::OnPropertySelected(gd::Object* object,
                                               gd::Layout* layout,
  ...
  if (event.GetColumn() == 1)  // Manage button-like properties
  {
    if (event.GetPropertyName() == _("Edit")) {
      ...
      for (std::size_t j = 0; j < project.GetUsedPlatforms().size(); ++j)
        project.GetUsedPlatforms()[j]->GetChangesNotifier().OnObjectEdited(
            project, globalObject ? NULL : layout, *object);
      ...
      if (dialog.ShowModal() == 1) {
        for (std::size_t j = 0; j < project.GetUsedPlatforms().size(); ++j)
          project.GetUsedPlatforms()[j]
              ->GetChangesNotifier()
              .OnObjectVariablesChanged(
                  project, globalObject ? NULL : layout, *object);

        // Update the grid:
        if (grid->GetProperty("OBJECT_VARIABLES_CATEGORY") != NULL)
          grid->SetPropertyLabel(
              "OBJECT_VARIABLES_CATEGORY",
              _("Object variables") + " (" +
                  gd::String::From(object->GetVariables().Count()) + ")");
      }
    } else if (event.GetPropertyName() == "AUTO_ADD") {
      return gd::ChooseBehaviorTypeDialog::ChooseAndAddBehaviorToObject(
          grid, project, object, layout, globalObject);
      ...
      int selection = wxGetSingleChoiceIndex(_("Choose the behavior to delete"),
                                             _("Choose the behavior to delete"),
                                             behaviorsStr);
      ...
      for (std::size_t j = 0; j < project.GetUsedPlatforms().size(); ++j)
        project.GetUsedPlatforms()[j]->GetChangesNotifier().OnBehaviorDeleted(
            project,
            globalObject ? NULL : layout,
            *object,
            behaviors[selection]);
      ...
      gd::String newName =
          wxGetTextFromUser(_("Enter a new name for the behavior"),
                            _("Rename a behavior"),
                            behavior.GetName());
      if (newName == behavior.GetName() || object->HasBehaviorNamed(newName) ||
          newName.empty())
        return false;
      ...
      for (std::size_t j = 0; j < project.GetUsedPlatforms().size(); ++j)
        project.GetUsedPlatforms()[j]->GetChangesNotifier().OnBehaviorRenamed(
            project, globalObject ? NULL : layout, *object, behavior, oldName);
      ...
      behavior.EditBehavior(
          ...
          project,
          layout,
          mainFrameWrapper);  // EditBehavior always need a valid layout!
      for (std::size_t j = 0; j < project.GetUsedPlatforms().size(); ++j)
        project.GetUsedPlatforms()[j]->GetChangesNotifier().OnBehaviorEdited(
            project, globalObject ? NULL : layout, *object, behavior);
    }
  }
...
bool ObjectsPropgridHelper::OnPropertyChanged(gd::Object* object,
                                              gd::Layout* layout,
                                              wxPropertyGridEvent& event) {
  if (!grid || !object) return false;

  auto readEnumPropertyString =
      [&event](std::map<gd::String, gd::PropertyDescriptor> properties) {
        const std::vector<gd::String>& choices =
            properties[event.GetProperty()->GetLabel()].GetExtraInfo();
  ...
  if (event.GetPropertyName().substr(0, 10) == "AUTO_PROP:") {
    ...
    if (wxEnumProperty* enumProperty =
            dynamic_cast<wxEnumProperty*>(event.GetProperty()))
      value = readEnumPropertyString(behavior.GetProperties(project));

    if (!behavior.UpdateProperty(
            event.GetProperty()->GetLabel(), value, project)) {
      event.Veto();
      return false;
    }
...
void ObjectsPropgridHelper::UpdateBehaviorsSharedData(gd::Project& project,
