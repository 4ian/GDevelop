...
void ParameterControlsHelper::UpdateParameterContent(unsigned int i, bool show, bool isOptional,
    std::string description, std::string type, std::string content)
{
    ...
    paramCheckboxes.at(i)->Show(isOptional);
    ...
    paramTexts.at(i)->SetLabel( description + _(":") );
    ...
    bool disable = isOptional && !paramCheckboxes.at(i)->GetValue() && paramEdits.at(i)->GetValue().empty();
...
void ParameterControlsHelper::UpdateControls(unsigned int size)
{
    if (!sizer) return;

    while ( paramEdits.size() < size )
    {
        ...
        //Placing controls
        sizer->Add( paramCheckboxes.back(), 1, wxALL | wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 5 );
        ...
        sizer->Add( paramBmpBts.back(), 1, wxALL | wxALIGN_LEFT | wxEXPAND | wxALIGN_CENTER_VERTICAL, 5 );
