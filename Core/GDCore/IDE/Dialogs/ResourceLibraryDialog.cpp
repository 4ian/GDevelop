...
ResourceLibraryDialog::ResourceLibraryDialog(wxWindow* parent,
                                             wxWindowID id,
                                             const wxPoint& pos,
                                             const wxSize& size) {
  ...
  StaticText1 = new wxStaticText(
      Panel1,
      ID_STATICTEXT1,
      _("Drag images to add them to your project images bank."),
      wxDefaultPosition,
      wxDefaultSize,
      0,
      _T("ID_STATICTEXT1"));
  ...
  StaticText3 = new wxStaticText(
      this,
      ID_STATICTEXT3,
      _("Images are copied into the project folder when you drag and drop them "
        "in the\nimage bank. You can choose a specific subfolder:"),
      wxDefaultPosition,
      wxDefaultSize,
      0,
      _T("ID_STATICTEXT3"));
  ...
  StaticText2 = new wxStaticText(this,
                                 ID_STATICTEXT2,
                                 _("Sub folder where copy images:"),
                                 wxDefaultPosition,
                                 wxDefaultSize,
                                 0,
                                 _T("ID_STATICTEXT2"));
  FlexGridSizer3->Add(
      ...
      wxALL | wxALIGN_CENTER_HORIZONTAL | wxALIGN_CENTER_VERTICAL,
      5);
  insertionFolderEdit = new wxTextCtrl(this,
                                       ID_TEXTCTRL1,
                                       wxEmptyString,
                                       wxDefaultPosition,
                                       wxDefaultSize,
                                       ...
                                       wxDefaultValidator,
                                       _T("ID_TEXTCTRL1"));
  insertionFolderEdit->SetToolTip(
      _("The file is relative to the folder of the project"));
  FlexGridSizer3->Add(
      insertionFolderEdit,
      1,
      wxALL | wxEXPAND | wxALIGN_CENTER_HORIZONTAL | wxALIGN_CENTER_VERTICAL,
  ...
  FlexGridSizer2->Add(
      ...
      5);
  FlexGridSizer1->Add(
      FlexGridSizer2, 1, wxALL | wxALIGN_RIGHT | wxALIGN_CENTER_VERTICAL, 0);
  ...
  Connect(ID_LISTCTRL1,
          ...
          (wxObjectEventFunction)&ResourceLibraryDialog::OnlistCtrlBeginDrag);
  Connect(
      ID_LISTCTRL1,
      wxEVT_COMMAND_LIST_ITEM_ACTIVATED,
      (wxObjectEventFunction)&ResourceLibraryDialog::OnlistCtrlItemActivated);
  Connect(ID_BUTTON1,
          wxEVT_COMMAND_BUTTON_CLICKED,
          (wxObjectEventFunction)&ResourceLibraryDialog::OncloseBtClick);
  //*)
...
wxBitmap Rescale(wxBitmap bmp, int max_width, int max_height) {
  ...
  wxBitmap result = wxBitmap(img.Rescale((int)(bmp.GetWidth() * factor),
    ...
    if (wxDirExists(currentDir + "/" + filename)) {
      ...
      if (wxFileExists(currentDir + "/" + filename + "/GDLibrary.txt")) {
        ...
        if (wxFileExists(currentDir + "/" + filename + "/GDLibraryIcon.png"))
          PasteBitmap(
              folderBmp,
              wxBitmap(currentDir + "/" + filename + "/GDLibraryIcon.png",
                       wxBITMAP_TYPE_ANY),
              20,
              20);
...
void ResourceLibraryDialog::OnlistCtrlItemActivated(wxListEvent& event) {
  if (event.GetLabel() == _("Parent folder") && event.GetIndex() == 0) {
    ...
    wxFileName filename =
        wxFileName::FileName(currentDir + "/" + event.GetLabel());
    filename.Normalize();
...
void ResourceLibraryDialog::OncloseBtClick(wxCommandEvent& event) { Hide(); }

void ResourceLibraryDialog::OnlistCtrlBeginDrag(wxListEvent& event) {
  wxString fullList =
      "COPYANDADDRESOURCES;" + insertionFolderEdit->GetValue() +
      ";";  // The resource editor is expecting a specifically formatted string
  long itemIndex = -1;

  for (;;) {
    itemIndex = listCtrl->GetNextItem(
        itemIndex, wxLIST_NEXT_ALL, wxLIST_STATE_SELECTED);
    if (itemIndex == -1) break;

    if ((listCtrl->GetItemText(itemIndex) == _("Parent folder") &&
         itemIndex == 0) ||
        wxDirExists(currentDir + "/" + listCtrl->GetItemText(itemIndex))) {
      // Folder: Do nothing
    } else
