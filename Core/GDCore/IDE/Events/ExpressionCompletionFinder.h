...
class GD_CORE_API ExpressionCompletionFinder
 ...
 protected:
  void OnVisitSubExpressionNode(SubExpressionNode& node) override {
    auto type = (ignoreNodeType)
                    ? "number|string"
                    : gd::ExpressionTypeFinder::GetType(platform,
                                                        globalObjectsContainer,
                                                        objectsContainer,
                                                        rootType,
                                                        node);
  ...
  void OnVisitTextNode(TextNode& node) override {
    ...
    if (functionCall != nullptr) {
      ...
      const gd::String& type =
          (ignoreNodeType) ? "number|string" : parameterMetadata->GetType();
      if (type == "string") {
 ...
 private:
  ...
  ExpressionCompletionFinder(const gd::Platform &platform_,
                             ...
                             bool ignoreNodeType_ = false)
      : platform(platform_),
        globalObjectsContainer(globalObjectsContainer_),
        objectsContainer(objectsContainer_),
        rootType(rootType_),
        ignoreNodeType(ignoreNodeType_),
