...
/**
 * \brief Tools functions that must be used by the IDE (especially the events
 * editors) when changes occurred in the events of a layout or in external
 * events.
 */
class GD_CORE_API EventsChangesNotifier {
 public:
  static void NotifyChangesInEventsOfScene(gd::Project& project,
                                           gd::Layout& layout);
  static void NotifyChangesInEventsOfExternalEvents(
      gd::Project& project, gd::ExternalEvents& externalEvents);

 private:
  /**
   * Fill layouts and externalEvents vector with pointers to layouts and
   * external events linked (even indirectly) by the events.
   *
   * \see EventsChangesNotifier::NotifyChangesInEventsOfScene
   * \see EventsChangesNotifier::NotifyChangesInEventsOfExternalEvents
   */
  static void GetScenesAndExternalEventsLinkedTo(
