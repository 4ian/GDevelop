...
class CallbacksForSearchingGlobalVariable : public gd::ParserCallbacks
{
    ...
    virtual bool OnSubMathExpression(const gd::Project & project, const gd::Layout & layout, gd::Expression & expression)
    {
        ...
        parser.ParseMathExpression(project, layout, callbacks);
    ...
    virtual bool OnSubTextExpression(const gd::Project & project, const gd::Layout & layout, gd::Expression & expression)
    {
        ...
        gd::ExpressionParser parser(expression.GetPlainString());
        parser.ParseStringExpression(project, layout, callbacks);
...
std::set < std::string > EventsVariablesFinder::FindAllGlobalVariables(const gd::Project & project)
{
    std::set < std::string > results;

    for (unsigned int i = 0;i<project.GetLayoutCount();++i)
    {
        std::set < std::string > results2 = FindArgumentsInEvents(project, project.GetLayout(i), project.GetLayout(i).GetEvents(), "globalvar");
...
std::set < std::string > EventsVariablesFinder::FindAllLayoutVariables(const gd::Project & project, const gd::Layout & layout)
{
    std::set < std::string > results;

    std::set < std::string > results2 = FindArgumentsInEvents(project, layout, layout.GetEvents(), "scenevar");
    results.insert(results2.begin(), results2.end());

    return results;
}

std::set < std::string > EventsVariablesFinder::FindArgumentsInInstructions(const gd::Project & project, const gd::Layout & layout, const vector < gd::Instruction > & instructions, bool instructionsAreConditions, const std::string & parameterType)
{
    std::set < std::string > results;
    ...
    for (unsigned int aId = 0;aId < instructions.size();++aId)
    {
        ...
        gd::InstructionMetadata instrInfos = instructionsAreConditions ? metadataHolder.GetConditionMetadata(instructions[aId].GetType()) : metadataHolder.GetActionMetadata(instructions[aId].GetType());
        ...
        if ( !instructions[aId].GetSubInstructions().empty() ) FindArgumentsInInstructions(project, layout, instructions[aId].GetSubInstructions(), instructionsAreConditions, parameterType);
    }

    return results;
}

std::set < std::string > EventsVariablesFinder::FindArgumentsInEvents(const gd::Project & project, const gd::Layout & layout, const vector < gd::BaseEventSPtr > & events, const std::string & parameterType)
{
    std::set < std::string > results;
    for (unsigned int i = 0;i<events.size();++i)
    {
        ...
        {
            std::set < std::string > results2 = FindArgumentsInInstructions(project, layout, *conditionsVectors[j], /*conditions=*/true, parameterType);
            results.insert(results2.begin(), results2.end());
        }
        ...
        {
            std::set < std::string > results2 = FindArgumentsInInstructions(project, layout, *actionsVectors[j], /*conditions=*/false, parameterType);
            results.insert(results2.begin(), results2.end());
        }

        if ( events[i]->CanHaveSubEvents() )
        {
            std::set < std::string > results2 = FindArgumentsInEvents(project, layout, events[i]->GetSubEvents(), parameterType);
            results.insert(results2.begin(), results2.end());
        }
    }
