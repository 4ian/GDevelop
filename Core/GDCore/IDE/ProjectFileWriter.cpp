...
bool ProjectFileWriter::SaveToFile(const gd::Project& project,
  ...
  if (project.IsFolderProject() &&
      !forceSingleFile)  // Optionally split the project
  {
    ...
    for (auto& element : splitElements) {
      ...
      gd::String filename =
          projectPath + element.path + "-" + MakeFileNameSafe(element.name);
      gd::RecursiveMkDir::MkDir(wxFileName::FileName(filename).GetPath());
      if (!gd::SaveXmlToFile(doc, filename)) {
        gd::LogError(
            _("Unable to save file ") + filename +
            _("!\nCheck that the drive has enough free space, is not "
              "write-protected and that you have read/write permissions."));
        return false;
      }
    }
  }
...
bool ProjectFileWriter::SaveToJSONFile(const gd::Project& project,
                                       const gd::String& filename) {
  // Serialize the whole project
  gd::SerializerElement rootElement;
  project.SerializeTo(rootElement);
...
bool ProjectFileWriter::LoadFromJSONFile(gd::Project& project,
                                         const gd::String& filename) {
  gd::FileStream ifs(filename, std::ios_base::in);
  if (!ifs.is_open()) {
    gd::String error = _("Unable to open the file.") +
                       _("Make sure the file exists and that you have the "
...
bool ProjectFileWriter::LoadFromFile(gd::Project& project,
                                     const gd::String& filename) {
  ...
  if (!rootXmlElement)
    rootXmlElement = hdl.FirstChildElement("Project").ToElement();
  if (!rootXmlElement)
    rootXmlElement = hdl.FirstChildElement("Game").ToElement();
...
// Unsplit the project
  ...
  gd::Splitter splitter;
  splitter.Unsplit(
      rootElement, [&projectPath](gd::String path, gd::String name) {
        TiXmlDocument doc;
        gd::SerializerElement rootElement;
        ...
        if (!gd::LoadXmlFromFile(doc, filename)) {
          ...
          gd::String error = _("Error while loading :") + "\n" +
                             errorTinyXmlDesc + "\n\n" +
                             _("Make sure the file exists and that you have "
                               "the right to open the file.");
        ...
        TiXmlHandle hdl(&doc);
        gd::Serializer::FromXML(rootElement,
                                hdl.FirstChildElement().ToElement());
        return rootElement;
      });
#endif
...
void ProjectFileWriter::ConvertANSIXMLFile(TiXmlHandle& hdl,
                                           TiXmlDocument& doc,
                                           const gd::String& filename) {
// COMPATIBILITY CODE WITH ANSI GDEVELOP ( <= 3.6.83 )
#if defined(GD_IDE_ONLY) && \
    !defined(GD_NO_WX_GUI)  // There should not be any problem with encoding in
                            // compiled games
  ...
  if (strcmp(declXmlElement->Encoding(), "UTF-8") != 0) {
    std::cout << "This is a legacy GDevelop project, checking if it is already "
                 "encoded in UTF8..."
              << std::endl;

    // The document has not been converted for/saved by GDevelop UTF8, now, try
    // to determine if the project was saved on Linux and is already in UTF8 or
    // on Windows and still in the locale encoding.
    ...
    while (!docStream.eof()) {
      ...
      if (!gd::String::FromUTF8(docLine).IsValid()) {
        ...
        // the file has been saved by the legacy ANSI Windows version of
        // GDevelop
    ...
    if (isNotInUTF8) {
      std::cout
          << "The project file is not encoded in UTF8, conversion started... ";
...
// Create a temporary file
      ...
      while (!docStream.eof()) {
        ...
        std::getline(docStream, docLine);
        sf::Utf8::fromAnsi(
            docLine.begin(), docLine.end(), std::back_inserter(convLine));
      ...
      wxString tmpFileName = wxStandardPaths::Get().GetUserConfigDir() +
                             "/gdevelop_converted_project";
      gd::String iconvCall = gd::String("iconv -f LATIN1 -t UTF-8 \"") +
                             filename.ToLocale() + "\" ";
#if defined(MACOS)
      ...
      // Reload the converted file, forcing UTF8 encoding as the XML header is
      // false (still written ISO-8859-1)
      doc.LoadFile(std::string(tmpFileName).c_str(), TIXML_ENCODING_UTF8);

      std::cout << "Finished." << std::endl;
      gd::LogMessage(
          _("Your project has been upgraded to be used with GDevelop 4.\nIf "
            "you save it, you won't be able to open it with an older version: "
            "please do a backup of your project file if you want to go back to "
            "GDevelop 3."));
    }
  }
#endif
