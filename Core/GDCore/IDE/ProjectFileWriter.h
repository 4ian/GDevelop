...
class GD_CORE_API ProjectFileWriter {
 public:
   ...
   */
  static bool SaveToFile(const gd::Project& project,
   ...
   * "Dirty" flag is set to false when save is done.
   */
  static bool SaveToJSONFile(const gd::Project& project,
                             const gd::String& filename);

  /**
   * \brief Load the project from a JSON file.
   */
  static bool LoadFromJSONFile(gd::Project& project,
                               const gd::String& filename);
#endif
 ...
 private:
  /**
   * \brief Check if the current TinyXML file (represented by hdl and doc) is in
   * UTF8. If not, the file is converted to UTF8 and reopened.
   */
  static void ConvertANSIXMLFile(TiXmlHandle& hdl,
                                 TiXmlDocument& doc,
                                 const gd::String& filename);
};
