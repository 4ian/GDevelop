...
static unsigned char gdAuiBlendColour(unsigned char fg,
                                      unsigned char bg,
                                      double alpha) {
...
static wxBitmap gdAuiBitmapFromBits(const unsigned char bits[],
                                    int w,
                                    int h,
...
AuiTabArt::AuiTabArt() {
  ...
  m_disabled_close_bmp =
      gdAuiBitmapFromBits(GDclose_bits, 16, 16, wxColour(128, 128, 128));

  m_active_left_bmp = gdAuiBitmapFromBits(GDleft_bits, 16, 16, *wxBLACK);
  m_disabled_left_bmp =
      gdAuiBitmapFromBits(GDleft_bits, 16, 16, wxColour(128, 128, 128));

  m_active_right_bmp = gdAuiBitmapFromBits(GDright_bits, 16, 16, *wxBLACK);
  m_disabled_right_bmp =
      gdAuiBitmapFromBits(GDright_bits, 16, 16, wxColour(128, 128, 128));

  m_active_windowlist_bmp = gdAuiBitmapFromBits(GDlist_bits, 16, 16, *wxBLACK);
  m_disabled_windowlist_bmp =
      gdAuiBitmapFromBits(GDlist_bits, 16, 16, wxColour(128, 128, 128));
...
void AuiTabArt::SetFlags(unsigned int flags) { m_flags = flags; }

void AuiTabArt::SetSizingInfo(const wxSize& tab_ctrl_size, size_t tab_count) {
  m_fixed_tab_width = 100;
  ...
  if (m_fixed_tab_width < 100) m_fixed_tab_width = 100;

  if (m_fixed_tab_width > tot_width / 2) m_fixed_tab_width = tot_width / 2;

  if (m_fixed_tab_width > 220) m_fixed_tab_width = 220;
...
void AuiTabArt::DrawBackground(wxDC& dc,
  ...
  wxColor top_color =
      noBgGradient ? *wxWHITE : gdAuiStepColour(m_base_colour, 120);
...
void AuiTabArt::DrawTab(wxDC& dc,
  ...
  */
  ...
  if (page.active) {
    ...
    // Adapt text color
    int average = (m_active_colour.Red() + m_active_colour.Green() +
                   m_active_colour.Blue()) /
                  3;
    if (average < 127)
  ...
  wxString draw_text = gdAuiChopText(
...
int gd::AuiTabArt::GetIndentSize() { return 5; }
...
void AuiTabArt::SetNormalFont(const wxFont& font) { m_normal_font = font; }

void AuiTabArt::SetSelectedFont(const wxFont& font) { m_selected_font = font; }
