 ...
 *
 * The bitmaps are loaded when the CommonBitmapProvider is constructed for the
 * first time ( usually by the IDE ).
 *
 * \ingroup IDE
 */
class GD_CORE_API CommonBitmapProvider {
 public:
  ...
  static CommonBitmapProvider *Get() {
    if (NULL == _singleton) _singleton = new CommonBitmapProvider;
