...
void FlatAuiTabArt::SetFlags(unsigned int flags) { m_flags = flags; }
...
void FlatAuiTabArt::DrawTab(wxDC& dc,
  ...
  */
  ...
  if (page.active) {
    ...
    // Adapt text color
    int average =
        (m_base_colour.Red() + m_base_colour.Green() + m_base_colour.Blue()) /
        3;
    if (average < 127)
      dc.SetTextForeground(*wxWHITE);
    else
...
int gd::FlatAuiTabArt::GetIndentSize() { return 5; }
...
void FlatAuiTabArt::SetNormalFont(const wxFont& font) { m_normal_font = font; }
