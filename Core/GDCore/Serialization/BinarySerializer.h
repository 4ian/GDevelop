...
extern "C" {
  ...
  /**
   * \brief Serialize a SerializerElement to JSON.
   * \return Pointer to JSON string (caller must free with free)
   */
  EMSCRIPTEN_KEEPALIVE
  char* serializeElementToJSON(SerializerElement* element);

  /**
   * \brief Delete a SerializerElement created by deserializeBinarySnapshot.
   */
  EMSCRIPTEN_KEEPALIVE
