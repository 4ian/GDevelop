...
wxString HelpFileAccess::GetHelpURL(wxString page) const {
  ...
  wxString languageName =
      wxLocale::GetLanguageInfo(gd::LocaleManager::Get()->GetLanguage())
          ->CanonicalName.substr(0, 2);

  // Add the language two letter code if the language is supported by the wiki.
  // Note: for "en", we don't need to add "en" to the URL.
  if (languageName == "fr" || languageName == "de" || languageName == "es" ||
      languageName == "pt" || languageName == "ru" || languageName == "zh")
