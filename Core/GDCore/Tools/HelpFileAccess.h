 ...
 *
 * IDEs must create a child of this class and declares it to gd::HelpFileAccess
 * thanks to gd::HelpFileAccess::SetHelpProvider.
 *
 * \ingroup Tools
 */
...
class GD_CORE_API HelpFileAccess {
 public:
   ...
   * \warning The caller is responsible of \a newHelpProvider and must deleted
   * it if needed ( when application is closed notably or if a new help provider
   * is set ) \see gd::HelpProvider
   */
   ...
   * This assumes that the page is hosted on the GDevelop's wiki :
   * http://wiki.compilgames.net/doku.php/language/the_page_name The "language"
   * from the URL is deduced from the user language. For the languages that are
   * not available on the wiki, the user is redirected to the english page.
   *
   * \param page the address to the page relative to the wiki
   * (http://wiki.compilgames.net/doku.php/language/)
   */
  ...
  static HelpFileAccess *Get() {
    if (NULL == _singleton) _singleton = new HelpFileAccess;
