...
TEST_CASE( "DependenciesAnalyzer", "[common]" ) {
    SECTION("Can detect a simple scene dependency") {
        ...
        REQUIRE(analyzer.GetScenesDependencies().find("Layout2") != analyzer.GetScenesDependencies().end());
    ...
    SECTION("Can detect a simple external events dependency") {
        ...
        auto & externalEvents2 = project.InsertNewExternalEvents("ExternalEvents2", 0);
        ...
        REQUIRE(analyzer.GetExternalEventsDependencies().find("ExternalEvents1") != analyzer.GetExternalEventsDependencies().end());
        REQUIRE(analyzer.GetSourceFilesDependencies().size() == 0);
    }
    ...
    SECTION("Can detect a transitive scene and external events dependency") {
        ...
        REQUIRE(analyzer.GetScenesDependencies().find("Layout2") != analyzer.GetScenesDependencies().end());
        REQUIRE(analyzer.GetScenesDependencies().find("Layout3") != analyzer.GetScenesDependencies().end());
        REQUIRE(analyzer.GetExternalEventsDependencies().size() == 1);
        REQUIRE(analyzer.GetExternalEventsDependencies().find("ExternalEvents1") != analyzer.GetExternalEventsDependencies().end());
        REQUIRE(analyzer.GetSourceFilesDependencies().size() == 0);
    }
    ...
    SECTION("Can detect a (nested) circular dependency with scenes and external events") {
        gd::Project project;
        auto & layout1 = project.InsertNewLayout("Layout1", 0);
        auto & layout2 = project.InsertNewLayout("Layout2", 0);
        auto & layout3 = project.InsertNewLayout("Layout3", 0);
