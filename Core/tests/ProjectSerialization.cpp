...
void SetupProject(gd::Project &project, gd::Platform& platform) {
  ...
  gd::Object &object = layout.InsertNewObject(project, "MyExtension::Sprite", "MyObject", 0);
  gd::Behavior *behavior = object.AddNewBehavior(project, "MyEventsExtension::MyEventsBasedBehavior", "MyEventsBasedBehavior");
...
void CheckBehaviorPropertyInElement(SerializerElement &projectElement) {
  ...
  REQUIRE(behaviorElement.GetStringAttribute("name") == "MyEventsBasedBehavior");
  REQUIRE(behaviorElement.GetStringAttribute("type") == "MyEventsExtension::MyEventsBasedBehavior");
  REQUIRE(behaviorElement.GetStringAttribute("MyProperty") == "481516");
};

void CheckBehaviorPropertyInProject(Project &project) {
  auto &object = project.GetLayout("Scene").GetObject("MyObject");
...
TEST_CASE("ProjectSerialization", "[common]") {
  ...
  SECTION("Save and load a project with a custom behavior property value") {
    ...
    CheckBehaviorPropertyInProject(readProject);
  }
