...
TEST_CASE("Variable", "[common][variables]") {
  ...
  SECTION("Serialization keeps the \"mixed values\" marker") {
    // The editor marks the variables of a temporary container (merging the
    // variables of several objects) as having mixed values, and serializes it
    // to compute changesets from it: the marker must survive a serialization.
    ...
    // Marking a structure as having mixed values clears its children.
    ...
    REQUIRE(unserializedNumberVariable.GetType() ==
