...
gdjs.AnchorRuntimeBehavior.prototype = Object.create(
  gdjs.RuntimeBehavior.prototype
);
gdjs.AnchorRuntimeBehavior.thisIsARuntimeBehaviorConstructor =
...
gdjs.AnchorRuntimeBehavior.prototype.doStepPreEvents = function(runtimeScene) {
  ...
  if (this._invalidDistances) {
    ...
    if (
      ...
      gdjs.AnchorRuntimeBehavior.HorizontalAnchor.WINDOW_LEFT
    )
      this._leftEdgeDistance = topLeftPixel[0];
    else if (
      this._leftEdgeAnchor ===
      gdjs.AnchorRuntimeBehavior.HorizontalAnchor.WINDOW_RIGHT
    )
      this._leftEdgeDistance = rendererWidth - topLeftPixel[0];
    else if (
      this._leftEdgeAnchor ===
      gdjs.AnchorRuntimeBehavior.HorizontalAnchor.PROPORTIONAL
    )
    ...
    if (
      this._topEdgeAnchor ===
      gdjs.AnchorRuntimeBehavior.VerticalAnchor.WINDOW_TOP
    )
      this._topEdgeDistance = topLeftPixel[1];
    else if (
      this._topEdgeAnchor ===
      gdjs.AnchorRuntimeBehavior.VerticalAnchor.WINDOW_BOTTOM
    )
      this._topEdgeDistance = rendererHeight - topLeftPixel[1];
    else if (
      this._topEdgeAnchor ===
      gdjs.AnchorRuntimeBehavior.VerticalAnchor.PROPORTIONAL
    )
    ...
    if (
      this._rightEdgeAnchor != gdjs.AnchorRuntimeBehavior.HorizontalAnchor.NONE
    )
      this.owner.setWidth(bottomRightCoord[0] - topLeftCoord[0]);
    if (
      this._bottomEdgeAnchor != gdjs.AnchorRuntimeBehavior.VerticalAnchor.NONE
    )
      this.owner.setHeight(bottomRightCoord[1] - topLeftCoord[1]);
    if (
      this._leftEdgeAnchor != gdjs.AnchorRuntimeBehavior.HorizontalAnchor.NONE
    )
      this.owner.setX(
...
gdjs.AnchorRuntimeBehavior.prototype.doStepPostEvents = function(
  runtimeScene
) {};
