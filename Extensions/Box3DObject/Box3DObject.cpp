...
void Box3DObject::DoUnserializeFrom(gd::Project& project,
 ...
 */
bool RuntimeBox3DObject::ExtraInitializationFromInitialInstance(
...
bool RuntimeBox3DObject::Draw(sf::RenderTarget& window) {
  ...
  glTranslatef(
      GetX() * 0.25f - xView,
      -GetY() * 0.25f - yView,
      zPosition * 0.25f - 75.0f * (window.getView().getSize().y / 600.0f));
  ...
  glVertex3f(0, 0, 0);
  glTexCoord2f(0, 1);
  glVertex3f(0, sizeHeight, 0);
  glTexCoord2f(1, 1);
  glVertex3f(sizeWidth, sizeHeight, 0);
  glTexCoord2f(1, 0);
  glVertex3f(sizeWidth, 0, 0);
  glEnd();

  sf::Texture::bind(&frontTexture->texture);
  glBegin(GL_QUADS);
  glTexCoord2f(0, 0);
  glVertex3f(0, 0, sizeDepth);
  glTexCoord2f(0, 1);
  glVertex3f(0, sizeHeight, sizeDepth);
  glTexCoord2f(1, 1);
  glVertex3f(sizeWidth, sizeHeight, sizeDepth);
  glTexCoord2f(1, 0);
  glVertex3f(sizeWidth, 0, sizeDepth);
  glEnd();
...
RuntimeBox3DObject::RuntimeBox3DObject(RuntimeScene& scene,
                                       const Box3DObject& box3DObject)
 ...
 */
void Box3DObject::DrawInitialInstance(gd::InitialInstance& instance,
                                      sf::RenderTarget& renderTarget,
                                      gd::Project& project,
                                      gd::Layout& layout) {
  if (!topTexture || !bottomTexture || !rightTexture || !leftTexture ||
  ...
  glTranslatef(instance.GetX() * 0.25f - xView,
               -instance.GetY() * 0.25f - yView,
               zPosition * 0.25f -
                   75.0f * (renderTarget.getView().getSize().y / 600.0f));
...
sf::Vector2f Box3DObject::GetInitialInstanceDefaultSize(
...
bool Box3DObject::GenerateThumbnail(const gd::Project& project,
                                    wxBitmap& thumbnail) const {
#if !defined(GD_NO_WX_GUI)
  thumbnail =
      wxBitmap("CppPlatform/Extensions/Box3Dicon24.png", wxBITMAP_TYPE_ANY);
#endif

  return true;
}

void Box3DObject::EditObject(wxWindow* parent,
                             gd::Project& game,
...
Box3DObject::GetInitialInstanceProperties(const gd::InitialInstance& position,
                                          gd::Project& game,
                                          gd::Layout& scene) {
  ...
  properties[_("Pitch")] =
      position.floatInfos.find("pitch") != position.floatInfos.end()
          ? gd::String::From(position.floatInfos.find("pitch")->second)
          : "0";

  properties[_("Roll")] =
      position.floatInfos.find("roll") != position.floatInfos.end()
...
bool Box3DObject::UpdateInitialInstanceProperty(gd::InitialInstance& position,
...
void RuntimeBox3DObject::GetPropertyForDebugger(std::size_t propertyNb,
                                                gd::String& name,
  ...
  if (propertyNb == 0) {
    ...
    name = _("Depth");
    value = gd::String::From(depth);
  } else if (propertyNb == 3) {
    name = _("Z Coordinate");
    value = gd::String::From(zPosition);
  } else if (propertyNb == 4) {
    name = _("Yaw");
    value = gd::String::From(yaw);
  } else if (propertyNb == 5) {
    name = _("Pitch");
    value = gd::String::From(pitch);
  } else if (propertyNb == 6) {
    name = _("Roll");
    value = gd::String::From(roll);
  }
}

bool RuntimeBox3DObject::ChangeProperty(std::size_t propertyNb,
                                        gd::String newValue) {
  if (propertyNb == 0) {
    width = newValue.To<int>();
  } else if (propertyNb == 1) {
    height = newValue.To<int>();
  } else if (propertyNb == 2) {
    depth = newValue.To<int>();
  } else if (propertyNb == 3) {
    zPosition = newValue.To<int>();
  } else if (propertyNb == 4) {
    yaw = newValue.To<int>();
  } else if (propertyNb == 5) {
    pitch = newValue.To<int>();
  } else if (propertyNb == 6) {
    roll = newValue.To<int>();
  }

  return true;
}

std::size_t RuntimeBox3DObject::GetNumberOfProperties() const { return 7; }
#endif
