...
namespace gdjs {
  ...
  gdjs.dialogueTree.getText = function () {
    ...
    if(!gdjs.dialogueTree.dialogueIsRunning || dialogueText.length === 0) return '';
   ...
   */
  gdjs.dialogueTree.createNewActor = function (actorId: string, actorName: string, actorColor: string) {
   ...
   */
  gdjs.dialogueTree.getActiveLineActorParameterViaIndex = function(paramIndex: number){
  const activeLineActorParameters = gdjs.dialogueTree.activeLineActorParameters;
    if (paramIndex > -1 && activeLineActorParameters.length > 0 && paramIndex < activeLineActorParameters.length) {
   ...
   */
  gdjs.dialogueTree.setActorInfo = function(actorId: string, infoKey: string, newValue: string){
   ...
   */
  gdjs.dialogueTree.getVariableExists = function (key: string) { // todo expose this to gd, replace where used
   ...
   */
  gdjs.dialogueTree.getChildKeyViaIndex = function (targetKey: string, targetIndex: index) {
  ...
  gdjs.dialogueTree.getCommandParameterViaKey = function (paramKey: string) {
    ...
    if (this.commandParameters.length > 0) {
      const parameterWithKey = this.commandParameters.find(parameter => parameter.startsWith(`${paramKey}=`));
      if(parameterWithKey){
  ...
  gdjs.dialogueTree.getTagViaKey = function (tagKey: string) {
    ...
    if (this.dialogueIsRunning && this.dialogueBranchTags.length > 0) {
      const parameterWithKey = this.dialogueBranchTags.find(tag => tag.startsWith(`${tagKey}:`));
      if(parameterWithKey){
        const [_, returnedParam] = parameterWithKey.split(":");
        return returnedParam ? returnedParam : '';
      }
