...
Light::~Light() { shapes.clear(); }

void Light::Draw(sf::RenderTarget *App) { App->draw(shapes, sf::BlendAdd); }
...
void Light::AddTriangle(sf::Vector2f pt1,
                        sf::Vector2f pt2,
                        int minimum_wall,
  ...
  for (std::vector<Wall *>::iterator IterWall = m_wall.begin() + minimum_wall;
       IterWall != m_wall.end();
       ++IterWall, ++w) {
    // l1 et l2 sont les positions relatives au centre de la lumière des deux
    // extrémités du mur
    sf::Vector2f l1((*IterWall)->pt1.x - m_position.x,
                    (*IterWall)->pt1.y - m_position.y);
    sf::Vector2f l2((*IterWall)->pt2.x - m_position.x,
                    (*IterWall)->pt2.y - m_position.y);

    // Discard walls that are too far.
    if ((l1.x * l1.x + l1.y * l1.y + l2.x * l2.x + l2.y * l2.y) / 2 >
        (m_radius * m_radius) * 1.5)
      continue;
  ...
  // On ajoute un shape
  // On lui donne comme point de départ (0,0), le centre de la lumière, avec la
  // couleur et intensité maximal
  shapes.append(sf::Vertex(m_position,
                           sf::Color((int)(m_intensity * m_color.r / 255),
                                     (int)(m_intensity * m_color.g / 255),
                                     (int)(m_intensity * m_color.b / 255))));

  // On calcul ou l'on se trouve par rapport au centre, pour savoir à quel
  // intensité on est
  intensity = m_intensity -
              sqrt(pt1.x * pt1.x + pt1.y * pt1.y) * m_intensity / m_radius;
  ...
  // Idem
  intensity = m_intensity -
              sqrt(pt2.x * pt2.x + pt2.y * pt2.y) * m_intensity / m_radius;
...
void Light::Generate(std::vector<Wall *> &m_wall) {
  ...
  // buf est l'angle de chaque triangle, c'est donc 2pi divisé par le nombre de
  // triangles
  ...
  for (int i = 0; i < m_quality; i++) {
    AddTriangle(
        sf::Vector2f((float)((float)m_radius * cos((float)i * buf)),
