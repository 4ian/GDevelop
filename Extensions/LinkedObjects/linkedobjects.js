...
gdjs.LinksManager.prototype.getObjectsLinkedWith = function(objA) {
  if (!this.links.hasOwnProperty(objA.id)) this.links[objA.id] = [];
...
gdjs.LinksManager.prototype.linkObjects = function(objA, objB) {
  ...
  if (objALinkedObjects.indexOf(objB) === -1) objALinkedObjects.push(objB);

  var objBLinkedObjects = this.getObjectsLinkedWith(objB);
  if (objBLinkedObjects.indexOf(objA) === -1) objBLinkedObjects.push(objA);
};
...
gdjs.LinksManager.prototype.removeAllLinksOf = function(obj) {
  ...
  if (this.links.hasOwnProperty(obj.id)) delete this.links[obj.id];
};
...
gdjs.evtTools.linkedObjects.gdjsCallbackRuntimeSceneLoaded = function(
  runtimeScene
) {
...
gdjs.evtTools.linkedObjects.gdjsCallbackObjectDeletedFromScene = function(
  runtimeScene,
  obj
) {
...
gdjs.evtTools.linkedObjects.pickObjectsLinkedTo = function(
  ...
  var linkedObjects = gdjs.LinksManager.getManager(
    runtimeScene
