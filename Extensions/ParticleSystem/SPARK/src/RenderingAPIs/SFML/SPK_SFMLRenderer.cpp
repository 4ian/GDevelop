...
namespace SFML
{
	...
	float SFMLRenderer::getViewZoom(ResizeMode mode)
	{
		...
		sf::Vector2f halfSize(currentTarget->GetView().GetSize());

		if (mode == RESIZE_ZOOM_X)
			return currentTarget->GetWidth() / (halfSize.x);	// target width / view width

		if (mode == RESIZE_ZOOM_Y)
			return currentTarget->GetHeight() / (halfSize.y); // target height / view height
