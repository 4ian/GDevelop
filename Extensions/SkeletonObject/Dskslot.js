...
gdjs.sk.SharedSlot.prototype.loadDragonBonesSlotData = function(slotData) {
  ...
  this.defaultAlpha = slotData.color.hasOwnProperty("aM")
    ...
    : 1.0;
  this.defaultVisible = slotData.hasOwnProperty("displayIndex")
...
gdjs.sk.SharedSlot.prototype.loadDragonBonesSkinData = function(
  ...
  if (skinData.display[0].type === "image") {
    ...
    if (skinData.display[0].hasOwnProperty("weights")) {
      ...
      var worldMatrixInverse = new gdjs.sk.Matrix(
        slotPose[0],
        -slotPose[1],
        slotPose[4],
        -slotPose[2],
      ...
      for (
        ...
        var boneWorldMatrix = new gdjs.sk.Matrix(
          bonePose[i + 1],
          -bonePose[i + 2],
          bonePose[i + 5],
          -bonePose[i + 3],
...
gdjs.sk.Slot.prototype.updateSkinning = function() {
  ...
  for (var i = 0; i < this.shared.vertexWeights.length; i++) {
    ...
    for (var j = 0; j < this.shared.vertexWeights[i].length; j++) {
      var v = boneMatrices[this.shared.vertexBones[i][j]].mulVec(
        this.vertices[i]
      );
