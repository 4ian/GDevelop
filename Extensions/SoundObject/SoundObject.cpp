...
RuntimeSoundObject::RuntimeSoundObject(RuntimeScene &scene_,
                                       const SoundObject &soundObject)
    : RuntimeObject(scene_, soundObject), m_sound(NULL) {
...
RuntimeSoundObject &RuntimeSoundObject::operator=(
    const RuntimeSoundObject &other) {
...
RuntimeSoundObject::RuntimeSoundObject(const RuntimeSoundObject &other)
    : RuntimeObject(other), m_sound(NULL) {
  Init(other);
}
...
void SoundObject::DoUnserializeFrom(gd::Project &project,
                                    const gd::SerializerElement &element) {
  SetSoundType(element.GetChild("type", 0, "Type").GetValue().GetString());
  SetSoundFileName(
      element.GetChild("filename", 0, "Filename").GetValue().GetString());
  SetPitch(element.GetChild("volume", 0, "Volume").GetValue().GetDouble());
  SetPitch(element.GetChild("pitch", 0, "Pitch").GetValue().GetDouble());
  SetMinDistance(
      element.GetChild("minDistance", 0, "MinDistance").GetValue().GetDouble());
  SetAttenuation(
      element.GetChild("attenuation", 0, "Attenuation").GetValue().GetDouble());
...
#if defined(GD_IDE_ONLY)
void SoundObject::DrawInitialInstance(gd::InitialInstance &instance,
...
SoundObject::GetInitialInstanceProperties(const gd::InitialInstance &position,
                                          gd::Project &game,
                                          gd::Layout &scene) {
  std::map<gd::String, gd::PropertyDescriptor> properties;
  properties[_("Z")] =
      position.floatInfos.find("z") != position.floatInfos.end()
...
bool SoundObject::UpdateInitialInstanceProperty(gd::InitialInstance &position,
...
bool SoundObject::GenerateThumbnail(const gd::Project &project,
                                    wxBitmap &thumbnail) const {
#if !defined(GD_NO_WX_GUI)
  thumbnail =
      wxBitmap("CppPlatform/Extensions/soundicon24.png", wxBITMAP_TYPE_ANY);
#endif

  return true;
}

void SoundObject::EditObject(wxWindow *parent,
                             gd::Project &game,
...
void RuntimeSoundObject::GetPropertyForDebugger(std::size_t propertyNb,
                                                gd::String &name,
                                                gd::String &value) const {
  if (propertyNb == 0) {
    name = _("Sound level");
    value = gd::String::From(GetVolume());
  } else if (propertyNb == 1) {
    name = _("Minimal distance");
    value = gd::String::From(GetMinDistance());
  } else if (propertyNb == 2) {
    name = _("Attenuation");
    value = gd::String::From(GetAttenuation());
  } else if (propertyNb == 3) {
    name = _("Loop");
    value = IsLooping() ? _("Yes") : _("No");
  } else if (propertyNb == 4) {
    name = _("Z Position");
    value = gd::String::From(GetZPos());
  }
}

bool RuntimeSoundObject::ChangeProperty(std::size_t propertyNb,
                                        gd::String newValue) {
  if (propertyNb == 0) {
    SetVolume(newValue.To<float>());
  } else if (propertyNb == 1) {
    SetMinDistance(newValue.To<float>());
  } else if (propertyNb == 2) {
    SetAttenuation(newValue.To<float>());
  } else if (propertyNb == 3) {
    SetLooping(!(newValue == _("No")));
  } else if (propertyNb == 4) {
    SetZPos(newValue.To<float>());
  }
  return true;
}

std::size_t RuntimeSoundObject::GetNumberOfProperties() const { return 5; }
#endif

bool RuntimeSoundObject::ExtraInitializationFromInitialInstance(
    const gd::InitialInstance &instance) {
  SetZPos(instance.floatInfos.find("z") != instance.floatInfos.end()
...
void RuntimeSoundObject::SetSoundFileName(const gd::String &soundfilename) {
  fileName = soundfilename;
  if (m_sound) m_sound->SetFileName(fileName);
}

float RuntimeSoundObject::GetZPos() const { return m_sound->GetPosition().z; }

void RuntimeSoundObject::SetZPos(float zpos) {
  if (m_sound)
    m_sound->SetPosition(
        sf::Vector3f(m_sound->GetPosition().x, m_sound->GetPosition().y, zpos));
}
...
const gd::String &RuntimeSoundObject::GetSoundType() const { return m_type; }

bool RuntimeSoundObject::ReloadSound(const RuntimeScene &scene) {
  return m_sound->LoadFromFile(m_sound->GetFileName(), scene);
}

void RuntimeSoundObject::Play() { m_sound->Play(); }

void RuntimeSoundObject::Pause() { m_sound->Pause(); }

void RuntimeSoundObject::Stop() { m_sound->Stop(); }

bool RuntimeSoundObject::IsPlaying() const { return m_sound->IsPlaying(); }

bool RuntimeSoundObject::IsPaused() const { return m_sound->IsPausing(); }

bool RuntimeSoundObject::IsStopped() const { return m_sound->IsStopped(); }

void RuntimeSoundObject::SetVolume(float volume) { m_sound->SetVolume(volume); }

float RuntimeSoundObject::GetVolume() const { return m_sound->GetVolume(); }
...
void RuntimeSoundObject::SetLooping(bool is) { m_sound->SetLooping(is); }

bool RuntimeSoundObject::IsLooping() const { return m_sound->IsLooping(); }

void RuntimeSoundObject::SetPitch(float pitch) { m_sound->SetPitch(pitch); }

float RuntimeSoundObject::GetPitch() const { return m_sound->GetPitch(); }
