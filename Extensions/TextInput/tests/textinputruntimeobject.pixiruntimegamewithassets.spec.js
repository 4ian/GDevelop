...
describe('gdjs.ShapePainterRuntimeObject (using a PIXI RuntimeGame with assets)', function () {
  ...
  it('creates the DOM element', async () => {
    ...
    const gameDomElementContainer = runtimeGame.getRenderer().getDomElementContainer();
    if (!gameDomElementContainer) throw new Error("Expected getDomElementContainer to return a valid container.");

    expect(gameDomElementContainer.hasChildNodes()).to.be(true);

    const inputElement = gameDomElementContainer.querySelector('input');
    if (!inputElement) throw new Error("Expected an input element to have been created by the object.");
