...
TextObjectEditor::TextObjectEditor(wxWindow* parent,
                                   gd::Project& game,
                                   TextObject& object,
  ...
  // Auto adjust toolbar size to the combobox default size (because of the size
  // difference between Windows and GTK+)
...
void TextObjectEditor::UpdateColorBt() {
  ...
  colorImage.SetRGB(wxRect(0, 0, 16, 16),
                    textColor.Red(),
                    textColor.Green(),
                    textColor.Blue());
...
void TextObjectEditor::UpdatePreview() {
  ...
  textFont.SetPointSize(gd::String(m_sizeCombobox->GetValue()).To<float>() *
                        0.75f);
  ...
  // Change font style
  textFont.SetWeight(m_toolbar->GetToolToggled(BOLD_TOOL_ID)
                         ? wxFONTWEIGHT_BOLD
                         : wxFONTWEIGHT_NORMAL);
  textFont.SetStyle(m_toolbar->GetToolToggled(ITALIC_TOOL_ID)
                        ? wxFONTSTYLE_ITALIC
                        : wxFONTSTYLE_NORMAL);
  textFont.SetUnderlined(m_toolbar->GetToolToggled(UNDER_TOOL_ID));

#if defined(LINUX)  // Need to force the background color for already written
                    // characters on GTK+
...
void TextObjectEditor::OnChangeFontButton(wxCommandEvent& event) {
  ...
  wxFileDialog fileDialog(this,
                          _("Choose a font ( ttf/ttc files )"),
                          gameDirectory,
                          "",
                          "Polices (*.ttf, *.ttc)|*.ttf;*.ttc");
