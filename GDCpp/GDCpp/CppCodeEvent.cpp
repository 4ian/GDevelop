...
{
#if !defined(GD_NO_WX_GUI)
    gd::String outputFile(CodeCompiler::Get()->GetOutputDirectory()+"GD"+ToString(this)+"SourceFile.cpp");
...
void CppCodeEvent::AutogenerateFunctionName() const
{
    ...
    if( event != std::shared_ptr<gd::BaseEvent>() )
        functionToCall = "GDCppCode"+gd::String::FromLocale(ToString(event.get())); //The name refers to the original event being in the original scene.
    else
        functionToCall = "GDCppCode"+gd::String::FromLocale(ToString(this)); //We are the original event: Create the name using our address.
}
