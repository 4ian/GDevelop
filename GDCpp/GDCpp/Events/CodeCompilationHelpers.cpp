...
namespace
{
    ...
    {
        if ( !wxFileExists(gd::String(CodeCompiler::Get()->GetOutputDirectory()+"GD"+gd::String::FromLocale(ToString(&sourceFile))+"ObjectFile.o") ))
            return true;
        else
        {
            ...
            {
                wxFileName bitcodeFileInfo(gd::String(CodeCompiler::Get()->GetOutputDirectory()+"GD"+gd::String::FromLocale(ToString(&sourceFile))+"ObjectFile.o"));
    ...
    bool ExternalEventsNeedRecompilation(gd::Project & game, gd::ExternalEvents & events)
    {
        ...
        if ( !wxFileExists(gd::String(CodeCompiler::Get()->GetOutputDirectory()+"GD"+gd::String::FromLocale(ToString(&events))+"ObjectFile.o") ))
            return true;
        else
        {
            wxFileName bitcodeFileInfo(gd::String(CodeCompiler::Get()->GetOutputDirectory()+"GD"+gd::String::FromLocale(ToString(&events))+"ObjectFile.o"));
    ...
    bool EnsureDependenciesAreOrWillBeCompiledForRuntime(gd::Project & game, const DependenciesAnalyzer & analyzer, gd::Layout * optionalScene, gd::ArbitraryResourceWorker & resourceWorker)
    {
        ...
        for (std::set<gd::String>::const_iterator i = analyzer.GetSourceFilesDependencies().begin();i!=analyzer.GetSourceFilesDependencies().end();++i)
        {
            ...
            task.compilerCall.outputFile = gd::String(CodeCompiler::Get()->GetOutputDirectory()+"GD"+gd::String::FromLocale(ToString(&sourceFile))+"RuntimeObjectFile.o");
        ...
        for (std::set<gd::String>::const_iterator i = analyzer.GetExternalEventsDependencies().begin();i!=analyzer.GetExternalEventsDependencies().end();++i)
        {
            if (game.HasExternalEventsNamed(*i))
            {
                ...
                if ( !analyzer.ExternalEventsCanBeCompiledForAScene().empty() )
                {
                    ...
                    task.compilerCall.inputFile = CodeCompiler::Get()->GetOutputDirectory()+"GD"+gd::String::FromLocale(ToString(&events))+"RuntimeEventsSource.cpp";
                    task.compilerCall.outputFile = CodeCompiler::Get()->GetOutputDirectory()+"GD"+gd::String::FromLocale(ToString(&events))+"RuntimeObjectFile.o";
    ...
    void CreateSceneEventsLinkingTask(gd::Project & game, gd::Layout & scene)
    {
        ...
        task.compilerCall.optimize = false;
        task.compilerCall.eventsGeneratedCode = true;
        task.compilerCall.inputFile = CodeCompiler::Get()->GetOutputDirectory()+"GD"+gd::String::FromLocale(ToString(&scene))+"ObjectFile.o";
        task.compilerCall.outputFile = CodeCompiler::Get()->GetOutputDirectory()+"GD"+gd::String::FromLocale(ToString(&scene))+"Code.dll";
        ...
        for (std::set<gd::String>::const_iterator i = analyzer.GetSourceFilesDependencies().begin();i!=analyzer.GetSourceFilesDependencies().end();++i)
        {
            ...
            task.compilerCall.extraObjectFiles.push_back(CodeCompiler::Get()->GetOutputDirectory()+"GD"+gd::String::FromLocale(ToString(&sourceFile))+"ObjectFile.o");
        }
        for (std::set<gd::String>::const_iterator i = analyzer.GetExternalEventsDependencies().begin();i!=analyzer.GetExternalEventsDependencies().end();++i)
        {
            ...
            if (externalEventsAnalyzer.ExternalEventsCanBeCompiledForAScene() == scene.GetName())
            {
                ...
                task.compilerCall.extraObjectFiles.push_back(CodeCompiler::Get()->GetOutputDirectory()+"GD"+gd::String::FromLocale(ToString(&externalEvents))+"ObjectFile.o");
            }
        }
...
bool EventsCodeCompilerPreWork::Execute()
{
    ...
    std::ofstream myfile;
    myfile.open ( (CodeCompiler::Get()->GetOutputDirectory()+"GD"+gd::String::FromLocale(ToString(scene))+"EventsSource.cpp").ToLocale().c_str() );
...
bool EventsCodeCompilerRuntimePreWork::Execute()
{
    ...
    std::ofstream myfile;
    myfile.open ( gd::String(CodeCompiler::Get()->GetOutputDirectory()+"GD"+gd::String::FromLocale(ToString(scene))+"RuntimeEventsSource.cpp").ToLocale().c_str() );
    myfile << eventsOutput.c_str();
    myfile.close();
...
bool EventsCodeCompilerPostWork::Execute()
{
    ...
    if ( CodeCompiler::Get()->MustDeleteTemporaries() )
        wxRemoveFile(CodeCompiler::Get()->GetOutputDirectory()+"GD"+gd::String::FromLocale(ToString(scene))+"EventsSource.cpp");
...
bool EventsCodeCompilerLinkingPostWork::Execute()
{
    ...
    if ( !compilationSucceeded || !wxFileExists(CodeCompiler::Get()->GetOutputDirectory()+"GD"+gd::String::FromLocale(ToString(scene))+"Code.dll") )
    {
        std::cout << "Scene linking failed." << std::endl;
        return false;
    }

    scene->SetCompiledEventsFile(CodeCompiler::Get()->GetOutputDirectory()+"GD"+gd::String::FromLocale(ToString(scene))+"Code.dll");
...
bool ExternalEventsCodeCompilerPreWork::Execute()
{
    ...
    std::ofstream myfile;
    myfile.open ( gd::String(CodeCompiler::Get()->GetOutputDirectory()+"GD"+gd::String::FromLocale(ToString(externalEvents))+"EventsSource.cpp").ToLocale().c_str() );
    myfile << eventsOutput.c_str();
    myfile.close();
...
bool ExternalEventsCodeCompilerRuntimePreWork::Execute()
{
    ...
    std::ofstream myfile;
    myfile.open ( gd::String(CodeCompiler::Get()->GetOutputDirectory()+"GD"+gd::String::FromLocale(ToString(externalEvents))+"RuntimeEventsSource.cpp").ToLocale().c_str() );
    myfile << eventsOutput.c_str();
    myfile.close();
...
void GD_API CodeCompilationHelpers::CreateSceneEventsCompilationTask(gd::Project & game, gd::Layout & scene)
{
    ...
    task.compilerCall.optimize = false;
    task.compilerCall.eventsGeneratedCode = true;
    task.compilerCall.inputFile = gd::String(CodeCompiler::Get()->GetOutputDirectory()+"GD"+gd::String::FromLocale(ToString(&scene))+"EventsSource.cpp");
    task.compilerCall.outputFile = gd::String(CodeCompiler::Get()->GetOutputDirectory()+"GD"+gd::String::FromLocale(ToString(&scene))+"ObjectFile.o");
    task.scene = &scene;
...
void GD_API CodeCompilationHelpers::CreateExternalSourceFileCompilationTask(gd::Project & game, SourceFile & file, gd::Layout * scene)
{
    ...
    inputFile.MakeAbsolute(wxFileName::FileName(game.GetProjectFile()).GetPath());
    task.compilerCall.inputFile = inputFile.GetFullPath();
    task.compilerCall.outputFile = gd::String(CodeCompiler::Get()->GetOutputDirectory()+"GD"+gd::String::FromLocale(ToString(&file))+"ObjectFile.o");
    task.compilerCall.compilationForRuntime = false;
    task.compilerCall.optimize = false;
    task.compilerCall.eventsGeneratedCode = false;
    task.compilerCall.extraHeaderDirectories.push_back(wxFileName::FileName(game.GetProjectFile()).GetPath());
...
void  GD_API CodeCompilationHelpers::CreateExternalEventsCompilationTask(gd::Project & game, gd::ExternalEvents & events)
{
    ...
    task.compilerCall.optimize = false;
    task.compilerCall.eventsGeneratedCode = true;
    task.compilerCall.inputFile = gd::String(CodeCompiler::Get()->GetOutputDirectory()+"GD"+gd::String::FromLocale(ToString(&events))+"EventsSource.cpp");
    task.compilerCall.outputFile = gd::String(CodeCompiler::Get()->GetOutputDirectory()+"GD"+gd::String::FromLocale(ToString(&events))+"ObjectFile.o");
