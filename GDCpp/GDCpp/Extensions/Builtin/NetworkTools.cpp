...
void GD_API SendHttpRequest(const gd::String& host,
                            ...
                            gd::Variable& responseVar) {
  // Separate the host and the port number
  auto hostInfo = host.Split(U':');

  if (hostInfo.size() < 2)
    return;  // Invalid address (there should be two elements: "http" and
             // "//the.domain.com")

  // Create Http
  sf::Http http;
  http.setHost(hostInfo[0].ToUTF8() + ":" + hostInfo[1].ToUTF8(),
               hostInfo.size() > 2 ? hostInfo[2].To<unsigned short>() : 0);

  // Create request
  sf::Http::Request request;
  request.setMethod(method == "POST" ? sf::Http::Request::Post
                                     : sf::Http::Request::Get);
  request.setField("Content-Type",
                   contentType.empty() ? "application/x-www-form-urlencoded"
                                       : contentType.ToUTF8());
...
size_t ParseJSONObject(const std::string& jsonStr,
  ...
  if (jsonStr[pos] == '{')  // Object
  {
    ...
    while (firstChild || jsonStr[pos] == ',') {
      ...
      pos++;
      pos = ::ParseJSONObject(
          jsonStr, pos, variable.GetChild(gd::String::FromUTF8(childName)));
    ...
    return pos + 1;
             ...
             '[')  // Array are translated into child named 0,1,2...
  {
    std::size_t index = 0;
    while (index == 0 || jsonStr[pos] == ',') {
      ...
      if (pos < jsonStr.length() && jsonStr[pos] == ']') break;
      pos = ::ParseJSONObject(
          jsonStr, pos, variable.GetChild(gd::String::From(index)));
...
void GD_API JSONToVariableStructure(const gd::String& jsonStr,
                                    gd::Variable& variable) {
