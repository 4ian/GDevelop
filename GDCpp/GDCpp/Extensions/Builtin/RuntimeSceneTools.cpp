...
gd::String GD_API GetSceneName(RuntimeScene &scene) { return scene.GetName(); }
...
void GD_API ReplaceScene(RuntimeScene &scene,
                         gd::String newSceneName,
                         bool clearOthers) {
  if (!scene.game->HasLayoutNamed(newSceneName)) return;
  scene.RequestChange(clearOthers ? RuntimeScene::SceneChange::CLEAR_SCENES
                                  : RuntimeScene::SceneChange::REPLACE_SCENE,
                      newSceneName);
}
...
void GD_API MoveObjects(RuntimeScene &scene) {
  ...
  for (std::size_t id = 0; id < allObjects.size(); ++id) {
    double elapsedTime =
        static_cast<double>(allObjects[id]->GetElapsedTime(scene)) / 1000000.0;
    allObjects[id]->SetX(allObjects[id]->GetX() +
                         allObjects[id]->TotalForceX() * elapsedTime);
    allObjects[id]->SetY(allObjects[id]->GetY() +
                         allObjects[id]->TotalForceY() * elapsedTime);
...
void DoCreateObjectOnScene(
  ...
  std::vector<ObjSPtr>::const_iterator sceneObject =
      std::find_if(scene.GetObjects().begin(),
                   ...
                   std::bind2nd(ObjectHasName(), objectName));
  ...
  pickedObjectLists[objectName]->push_back(
...
void GD_API CreateObjectOnScene(
  ...
  ::DoCreateObjectOnScene(scene,
                          pickedObjectLists.begin()->first,
                          pickedObjectLists,
                          positionX,
                          positionY,
                          layer);
...
void GD_API CreateObjectFromGroupOnScene(
    RuntimeScene &scene,
    std::map<gd::String, std::vector<RuntimeObject *> *> pickedObjectLists,
    const gd::String &objectWanted,
    float positionX,
    float positionY,
    const gd::String &layer) {
  if (pickedObjectLists[objectWanted] == nullptr)
    return;  // Bail out if the object is not present in the specified group

  ::DoCreateObjectOnScene(
      scene, objectWanted, pickedObjectLists, positionX, positionY, layer);
}

bool GD_API PickAllObjects(
    RuntimeScene &scene,
    std::map<gd::String, std::vector<RuntimeObject *> *> pickedObjectLists) {
  for (auto it = pickedObjectLists.begin(); it != pickedObjectLists.end();
       ++it) {
    if (it->second != nullptr) {
      std::vector<RuntimeObject *> objectsOnScene =
          scene.objectsInstances.GetObjectsRawPointers(it->first);

      for (std::size_t j = 0; j < objectsOnScene.size(); ++j) {
        if (find(it->second->begin(), it->second->end(), objectsOnScene[j]) ==
            it->second->end())
...
bool GD_API PickRandomObject(
  ...
  if (allObjects.empty()) return false;
...
bool GD_API SceneVariableExists(RuntimeScene &scene,
                                const gd::String &variable) {
  return scene.GetVariables().Has(variable);
}

bool GD_API GlobalVariableExists(RuntimeScene &scene,
                                 const gd::String &variable) {
...
bool GD_API VariableChildExists(const gd::Variable &variable,
                                const gd::String &childName) {
  return variable.HasChild(childName);
}

void GD_API VariableRemoveChild(gd::Variable &variable,
                                const gd::String &childName) {
...
unsigned int GD_API GetVariableChildCount(gd::Variable &variable) {
  if (variable.IsPrimitive()) return 0;
...
void GD_API SetWindowIcon(RuntimeScene &scene, const gd::String &imageName) {
  // Retrieve the image
  std::shared_ptr<SFMLTextureWrapper> image =
      scene.GetImageManager()->GetSFMLTexture(imageName);
  if (image == std::shared_ptr<SFMLTextureWrapper>()) return;

  scene.renderWindow->setIcon(image->image.getSize().x,
                              image->image.getSize().y,
                              image->image.getPixelsPtr());
}
...
void GD_API SetWindowTitle(RuntimeScene &scene, const gd::String &newName) {
  ...
  if (scene.renderWindow != NULL)
    scene.renderWindow->setTitle(scene.GetWindowDefaultTitle());
}
...
void GD_API SetWindowSize(RuntimeScene &scene,
                          int windowWidth,
                          int windowHeight,
  ...
  if (windowWidth == scene.renderWindow->getSize().x &&
      windowHeight == scene.renderWindow->getSize().y)
    return;
  ...
  scene.renderWindow->create(
      ...
      sf::Style::Close |
          (scene.RenderWindowIsFullScreen() ? sf::Style::Fullscreen : 0));
