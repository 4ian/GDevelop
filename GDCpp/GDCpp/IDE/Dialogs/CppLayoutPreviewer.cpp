...
CppLayoutPreviewer::CppLayoutPreviewer(gd::LayoutEditorCanvas& editor_)
    : previewScene(&editor_, &previewGame),
      ...
      isReloading(false),
      playing(false) {
  // The external preview window is created only when necessary to prevent it to
  // be shown at creation on Linux. Additional editors are created in
  // SetParentAuiManager method.
  ...
  if (editor.GetLayout().CompilationNeeded() &&
      !CodeCompiler::Get()->HasTaskRelatedTo(editor.GetLayout()))
    CodeCompilationHelpers::CreateSceneEventsCompilationTask(
        editor.GetProject(), editor.GetLayout());
}
...
bool CppLayoutPreviewer::LaunchPreview() {
  ...
  if (wxDirExists(
          wxFileName::FileName(editor.GetProject().GetProjectFile()).GetPath()))
  ...
  mainFrameWrapper.GetRibbonSceneEditorButtonBar()->EnableButton(idRibbonPlay,
                                                                 true);
  mainFrameWrapper.GetRibbonSceneEditorButtonBar()->EnableButton(idRibbonPause,
                                                                 false);
  mainFrameWrapper.GetRibbonSceneEditorButtonBar()->EnableButton(
      idRibbonPlayWin, true);
...
void CppLayoutPreviewer::OnUpdate() {
  if (isReloading) {
    if (CodeCompiler::Get()->CompilationInProcess())  // We're still waiting for
  ...
  if (playing) {
    ...
    if (externalPreviewWindow &&
        externalPreviewWindow
            ->IsShown())  // Be sure that the editor is updated.
    {
    ...
    if (changeRequested) {
      ...
      if (request.change == RuntimeScene::SceneChange::STOP_GAME)
        mainFrameWrapper.GetInfoBar()->ShowMessage(
            _("In the compiled game, the game will be stopped."));
      else if (request.change == RuntimeScene::SceneChange::REPLACE_SCENE)
        mainFrameWrapper.GetInfoBar()->ShowMessage(
            _("In the compiled game, the scene will be stopped and replaced "
              "by ") +
            "\"" + request.requestedScene + "\"");
      else if (request.change == RuntimeScene::SceneChange::PUSH_SCENE)
        mainFrameWrapper.GetInfoBar()->ShowMessage(
            _("In the compiled game, the scene will be paused and replaced "
              "by ") +
            "\"" + request.requestedScene + "\"");
      else if (request.change == RuntimeScene::SceneChange::POP_SCENE)
        mainFrameWrapper.GetInfoBar()->ShowMessage(
            _("In the compiled game, the scene will be stopped and the game "
              "will go to the previous paused one"));
    }
  } else  // Paused
...
void CppLayoutPreviewer::RefreshFromLayout() {
  ...
  previewGame.GetSoundManager().ClearAllSoundsAndMusics();
  if (editor.GetProject().GetImageManager())
    editor.GetProject()
        .GetImageManager()
        ->PreventImagesUnloading();  // Images are normally unloaded and loaded
                                     // again when reloading the scene. We can
                                     // prevent this to happen as it is time
                                     // wasting.
  ...
  previewGame.SetImageManager(
      editor.GetProject().GetImageManager());  // Use same image manager.
  ...
  // Launch now events compilation if it has not been launched by another way
  // (i.e: by the events editor).
  if (editor.GetLayout().CompilationNeeded() &&
      !CodeCompiler::Get()->HasTaskRelatedTo(editor.GetLayout())) {
    CodeCompilationHelpers::CreateSceneEventsCompilationTask(
        editor.GetProject(), editor.GetLayout());
    mainFrameWrapper.GetInfoBar()->ShowMessage(
        _("Changes made to events will be taken into account when you switch "
          "to Editing mode"));
  }

  return;  // RefreshFromLayoutSecondPart() will be called by OnUpdate() when
           // appropriate
}
...
void CppLayoutPreviewer::RefreshFromLayoutSecondPart() {
  ...
  if (!previewScene.GetCodeExecutionEngine()->LoadFromDynamicLibrary(
          editor.GetLayout().GetCompiledEventsFile(),
          "GDSceneEvents" + gd::SceneNameMangler::GetMangledSceneName(
                                editor.GetLayout().GetName()))) {
    gd::LogError(
        _("Compilation of events failed, and scene cannot be previewed. Please "
          "report this problem to GDevelop's developer, joining this file:\n") +
        CodeCompiler::Get()->GetOutputDirectory() +
  ...
  // We were preventing images unloading so as to be sure not to waste time
  // unloading and reloading just after scenes images.
  if (editor.GetProject().GetImageManager())
    editor.GetProject().GetImageManager()->EnableImagesUnloading();
...
void CppLayoutPreviewer::OnPreviewPlayWindowBtClick(wxCommandEvent& event) {
  ...
  mainFrameWrapper.GetRibbonSceneEditorButtonBar()->EnableButton(idRibbonPause,
                                                                 true);
  mainFrameWrapper.GetRibbonSceneEditorButtonBar()->EnableButton(
      idRibbonPlayWin, false);

  // Create now the window if necessary (not done in the constructor because, on
  // linux, the window was not hidden).
  if (!externalPreviewWindow)
    externalPreviewWindow =
        std::make_shared<RenderDialog>(editor.GetParentControl(), this);

  externalPreviewWindow->Show(true);

  externalPreviewWindow->SetSizeOfRenderingZone(
      editor.GetProject().GetMainWindowDefaultWidth(),
      editor.GetProject().GetMainWindowDefaultHeight());
  previewScene.ChangeRenderWindow(externalPreviewWindow->renderCanvas);
}
void CppLayoutPreviewer::ExternalWindowClosed() {
  if (playing) PlayPreview();  // Go back to the internal preview
}
...
void CppLayoutPreviewer::RenderCompilationScreen() {
  ...
  editor.pushGLStates();
  editor.setView(
      sf::View(sf::Vector2f(editor.getSize().x / 2, editor.getSize().y / 2),
               sf::Vector2f(editor.getSize().x, editor.getSize().y)));

  reloadingIconSprite.setTexture(reloadingIconImage);
  reloadingIconSprite.setColor(sf::Color(255, 255, 255, 128));
  reloadingIconSprite.setPosition(
      editor.getSize().x / 2 - reloadingIconSprite.getLocalBounds().width / 2,
      editor.getSize().y / 2 - reloadingIconSprite.getLocalBounds().height / 2);
  reloadingText.setPosition(
      editor.getSize().x / 2 - reloadingText.getLocalBounds().width / 2,
      reloadingIconSprite.getPosition().y +
          reloadingIconSprite.getLocalBounds().height + 10);
...
void CppLayoutPreviewer::SetParentAuiManager(wxAuiManager* manager) {
  ...
  if (manager && editor.GetParentControl()) {
    if (!debugger) {
      ...
      debugger = std::shared_ptr<DebuggerGUI>(new DebuggerGUI(
          editor.GetParentControl(), previewScene, [this](bool newState) {
            playing = newState;
          }));
      if (!parentAuiManager->GetPane("DBG").IsOk())
        parentAuiManager->AddPane(debugger.get(),
                                  wxAuiPaneInfo()
                                      .Name(wxT("DBG"))
                                      .Float()
                                      .CloseButton(true)
    ...
    if (!profiler) {
      ...
      if (!parentAuiManager->GetPane("PROFILER").IsOk())
        parentAuiManager->AddPane(profiler.get(),
                                  wxAuiPaneInfo()
                                      .Name(wxT("PROFILER"))
                                      .Float()
                                      .CloseButton(true)
...
void CppLayoutPreviewer::CreatePreviewRibbonTools(
    wxRibbonButtonBar& buttonBar) {
  bool hideLabels = false;
  wxConfigBase::Get()->Read(_T( "/Skin/HideLabels" ), &hideLabels);

  buttonBar.AddButton(idRibbonPlay,
                      !hideLabels ? _("Play") : gd::String(),
                      gd::SkinHelper::GetRibbonIcon("start"),
                      _("Start the preview"));
  buttonBar.AddButton(idRibbonPlayWin,
                      !hideLabels ? _("Play in a window") : gd::String(),
                      gd::SkinHelper::GetRibbonIcon("startwindow"),
                      _("Start the preview in an external window"));
  buttonBar.AddButton(idRibbonPause,
                      !hideLabels ? _("Pause") : gd::String(),
                      gd::SkinHelper::GetRibbonIcon("pause"),
                      _("Pause the preview"));
  buttonBar.AddButton(idRibbonDebugger,
                      !hideLabels ? _("Debugger") : gd::String(),
                      gd::SkinHelper::GetRibbonIcon("bug"),
                      _("Show the debugger to view the state of the game"));
  buttonBar.AddButton(idRibbonProfiler,
                      !hideLabels ? _("Profiling") : gd::String(),
                      gd::SkinHelper::GetRibbonIcon("profiler"),
                      _("Show the profiler to monitor performances"));
}
...
void CppLayoutPreviewer::ConnectPreviewRibbonToolsEvents() {
  mainFrameWrapper.GetMainEditor()->Connect(
      ...
      (wxObjectEventFunction)&CppLayoutPreviewer::OnPreviewPlayBtClick,
      NULL,
      this);
  mainFrameWrapper.GetMainEditor()->Connect(
      idRibbonPlayWin,
      wxEVT_COMMAND_RIBBONBUTTON_CLICKED,
      (wxObjectEventFunction)&CppLayoutPreviewer::OnPreviewPlayWindowBtClick,
      NULL,
      this);
  mainFrameWrapper.GetMainEditor()->Connect(
      idRibbonPause,
      wxEVT_COMMAND_RIBBONBUTTON_CLICKED,
      (wxObjectEventFunction)&CppLayoutPreviewer::OnPreviewPauseBtClick,
      NULL,
      this);
  mainFrameWrapper.GetMainEditor()->Connect(
      idRibbonDebugger,
      wxEVT_COMMAND_RIBBONBUTTON_CLICKED,
      (wxObjectEventFunction)&CppLayoutPreviewer::OnPreviewDebugBtClick,
      NULL,
      this);
  mainFrameWrapper.GetMainEditor()->Connect(
      idRibbonProfiler,
      wxEVT_COMMAND_RIBBONBUTTON_CLICKED,
      (wxObjectEventFunction)&CppLayoutPreviewer::OnPreviewProfilerBtClick,
      NULL,
      this);
}

gd::Layout& CppLayoutPreviewer::GetLayout() { return editor.GetLayout(); }

gd::Project& CppLayoutPreviewer::GetProject() { return editor.GetProject(); }
