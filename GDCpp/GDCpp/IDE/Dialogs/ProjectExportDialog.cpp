...
ProjectExportDialog::ProjectExportDialog(wxWindow* parent,
                                         gd::Project& gameToCompile_)
  ...
  AvancementGauge = new wxGauge(this,
                                ID_GAUGE1,
                                100,
                                wxDefaultPosition,
                                wxSize(238, 16),
  ...
  FlexGridSizer7->Add(
      ...
      wxALL | wxEXPAND | wxALIGN_CENTER_HORIZONTAL | wxALIGN_CENTER_VERTICAL,
      0);
  statusTxt = new wxStaticText(this,
                               ID_STATICTEXT2,
                               wxEmptyString,
                               wxDefaultPosition,
                               wxDefaultSize,
  ...
  status2Txt = new wxStaticText(this,
                                ID_STATICTEXT1,
                                wxEmptyString,
                                wxDefaultPosition,
                                wxDefaultSize,
  ...
  StaticBitmap1 = new wxStaticBitmap(this,
                                     ...
                                     wxNO_BORDER,
                                     _T("ID_STATICBITMAP2"));
  FlexGridSizer2->Add(StaticBitmap1,
                      1,
  ...
  FlexGridSizer2->Add(
      ...
      5);
  FlexGridSizer3->Add(
      FlexGridSizer2, 1, wxALL | wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 0);
  FermerBt = new wxButton(this,
                          ...
                          _T("ID_BUTTON2"));
  FlexGridSizer3->Add(
      FermerBt, 1, wxALL | wxALIGN_RIGHT | wxALIGN_CENTER_VERTICAL, 5);
  FlexGridSizer1->Add(
      FlexGridSizer3,
      1,
      wxALL | wxEXPAND | wxALIGN_CENTER_HORIZONTAL | wxALIGN_CENTER_VERTICAL,
  ...
  Connect(ID_BUTTON1,
          ...
          (wxObjectEventFunction)&ProjectExportDialog::OnCompilBtClick);
  Connect(ID_BUTTON5,
          wxEVT_COMMAND_BUTTON_CLICKED,
          (wxObjectEventFunction)&ProjectExportDialog::OnbrowseBtClick);
  Connect(ID_HYPERLINKCTRL1,
          wxEVT_COMMAND_HYPERLINK,
          (wxObjectEventFunction)&ProjectExportDialog::OnAideBtClick);
  Connect(ID_BUTTON2,
          wxEVT_COMMAND_BUTTON_CLICKED,
          (wxObjectEventFunction)&ProjectExportDialog::OnFermerBtClick);
  //*)
  ...
  if (wxDirExists(gameToCompile.GetLastCompilationDirectory()))
    ...
    dirEdit->SetValue(wxFileName::GetHomeDir() +
                      wxFileName::GetPathSeparator() +
                      DeleteInvalidCharacters(gameToCompile.GetName()));
  }

  if (gameToCompile.GetLayoutsCount() == 0) {
    CompilBt->Enable(false);
    statusTxt->SetLabel(
        _("The game is empty: add a scene using the project manager."));
  }
}

wxString ProjectExportDialog::DeleteInvalidCharacters(
    const wxString& directoryName) const {
 ...
 */
class FullProjectCompilerDialogDiagnosticManager
    : public GDpriv::FullProjectCompilerDiagnosticManager {
 public:
  FullProjectCompilerDialogDiagnosticManager(wxStaticText* staticText1_,
                                             wxStaticText* staticText2_,
                                             wxGauge* gauge_,
                                             wxString destinationDirectory_)
      : staticText1(staticText1_),
        staticText2(staticText2_),
        gauge(gauge_),
  ...
  virtual void OnCompilationSucceeded() {
    staticText1->SetLabel(_("Compilation finished"));
    staticText2->SetLabel(
        _("Compiled project is now available in the export folder."));

    // Display the errors if there are some
    if (!GetErrors().empty()) {
      wxMessageBox(_("Some errors happened during the compilation, but the "
                     "game was still compiled successfully:") +
                       gd::String("\n") + GetErrors(),
...
void ProjectExportDialog::OnCompilBtClick(wxCommandEvent& event) {
  if (dirEdit->GetValue().empty()) {
    wxMessageBox(
        _("You must choose a directory where the project must be exported."),
        _("Compilation canceled"));
    return;
  }
  if (!wxDirExists(dirEdit->GetValue()) && !wxMkdir(dirEdit->GetValue())) {
    wxMessageBox(_("Unable to create the folder where the project must be "
  ...
  FullProjectCompilerDialogDiagnosticManager diagnosticManager(
      statusTxt, status2Txt, AvancementGauge, dirEdit->GetValue());
  GDpriv::FullProjectCompiler compilationManager(
      gameToCompile, diagnosticManager, dirEdit->GetValue());
...
void ProjectExportDialog::OnAideBtClick(wxCommandEvent& event) {
  gd::HelpFileAccess::Get()->OpenPage(
      "game_develop/documentation/manual/distribution/compilation");
}
...
void ProjectExportDialog::OnbrowseBtClick(wxCommandEvent& event) {
  ...
  if (dialog.ShowModal() == wxID_OK) dirEdit->SetValue(dialog.GetPath());
}
#endif
