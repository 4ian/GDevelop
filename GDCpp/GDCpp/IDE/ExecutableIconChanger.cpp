...
#if defined(WINDOWS)
bool ExecutableIconChanger::LoadIcon(gd::String iconFile,
                                     ICONHEADER*& pHeader,
                                     ICONIMAGE**& pIcons,
                                     GRPICONHEADER*& pGrpHeader) {
  HANDLE hFile = CreateFile(iconFile.ToLocale().c_str(),
                            GENERIC_READ,
                            FILE_SHARE_READ,
                            NULL,
                            OPEN_EXISTING,
  ...
  pHeader = (ICONHEADER*)realloc(
      pHeader, sizeof(WORD) * 3 + sizeof(ICONENTRY) * pHeader->count);
  ReadFile(hFile,
           pHeader->entries,
           pHeader->count * sizeof(ICONENTRY),
           &bytesRead,
           NULL);
  ...
  for (int i = 0; i < pHeader->count; i++) {
    ...
    ReadFile(
        hFile, pIcons[i], pHeader->entries[i].bytesInRes, &bytesRead, NULL);
  }

  // Convert to resource format
  pGrpHeader = (GRPICONHEADER*)malloc(sizeof(WORD) * 3 +
                                      pHeader->count * sizeof(GRPICONENTRY));
...
bool ExecutableIconChanger::ChangeWindowsExecutableIcon(gd::String exeFile,
                                                        gd::String iconFile) {
#if defined(WINDOWS)
  // Read icon file
  ICONHEADER* pHeader;
