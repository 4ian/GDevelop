...
bool CodeExecutionEngine::LoadFromDynamicLibrary(
    const gd::String &filename, const gd::String &mainFunctionName) {
  if (loaded) Unload();
  ...
  if (dynamicLibrary == NULL) {
    std::cout << "ERROR: Unable to load " << dynamicLibraryFilename
              << std::endl;
    std::cout << "Full error message: " << gd::DynamicLibraryLastError()
              << std::endl;
    return false;
  }
  ...
  if (function == NULL) {
    std::cout << "ERROR: Unable to find " << functionName << " in "
              << dynamicLibraryFilename << std::endl;
...
bool CodeExecutionEngine::LoadFunction(functionType fn) {
  ...
  function = (void *)fn;
  if (!function) {
    std::cout << "ERROR: Unable to use the specified function for a code "
                 "execution engine."
              << std::endl;
    return false;
  }
...
void CodeExecutionEngine::Init(const CodeExecutionEngine &other) {
  ...
  if (loaded) Unload();
  if (other.Ready())
    LoadFromDynamicLibrary(other.dynamicLibraryFilename, other.functionName);
}
