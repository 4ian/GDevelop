...
RuntimeSpriteObject::RuntimeSpriteObject(RuntimeScene& scene,
                                         const gd::SpriteObject& spriteObject)
  ...
  for (std::size_t j = 0; j < animations.size(); j++) {
    ...
    for (std::size_t k = 0; k < anim.GetDirectionsCount(); k++) {
      for (std::size_t l = 0; l < anim.GetDirection(k).GetSpritesCount(); l++) {
        ...
        sprite.LoadImage(
            scene.GetImageManager()->GetSFMLTexture(sprite.GetImageName()));
      }
    }
  }
}
...
bool RuntimeSpriteObject::Draw(sf::RenderTarget& renderTarget) {
  ...
  renderTarget.draw(
      GetCurrentSFMLSprite(),
      sf::RenderStates(
          blendMode == 0
              ? sf::BlendAlpha
              : (blendMode == 1
                     ? sf::BlendAdd
                     : (blendMode == 2 ? sf::BlendMultiply : sf::BlendNone))));
...
float RuntimeSpriteObject::GetWidth() const {
  return scaleX > 0 ? GetCurrentSFMLSprite().getLocalBounds().width * scaleX
                    : -GetCurrentSFMLSprite().getLocalBounds().width * scaleX;
}

float RuntimeSpriteObject::GetHeight() const {
  return scaleY > 0 ? GetCurrentSFMLSprite().getLocalBounds().height * scaleY
                    : -GetCurrentSFMLSprite().getLocalBounds().height * scaleY;
}
...
float RuntimeSpriteObject::GetPointX(const gd::String& name) const {
  if (!name.empty()) {
    ...
    return GetCurrentSFMLSprite()
        .getTransform()
        .transformPoint(point.GetX(), point.GetY())
        .x;
  }
...
float RuntimeSpriteObject::GetScaleX() const { return fabs(scaleX); }

float RuntimeSpriteObject::GetScaleY() const { return fabs(scaleY); }
...
void RuntimeSpriteObject::CopyImageOnImageOfCurrentSprite(
  ...
  ptrToCurrentSprite
      ->MakeSpriteOwnsItsImage();  // We want to modify only the image of the
                                   // object, not all objects which have the
                                   // same image.
  std::shared_ptr<SFMLTextureWrapper> dest =
      ptrToCurrentSprite->GetSFMLTexture();

  // Make sure the coordinates are correct.
  if (xPosition < 0 ||
      static_cast<unsigned>(xPosition) >= dest->texture.getSize().x)
    return;
  if (yPosition < 0 ||
      static_cast<unsigned>(yPosition) >= dest->texture.getSize().y)
    return;

  // Update texture and pixel perfect collision mask
  dest->image.copy(scene.GetImageManager()->GetSFMLTexture(imageName)->image,
                   xPosition,
                   yPosition,
                   sf::IntRect(0, 0, 0, 0),
                   useTransparency);
...
void RuntimeSpriteObject::MakeColorTransparent(const gd::String& colorStr) {
  ...
  // Update texture and pixel perfect collision mask
  dest->image.createMaskFromColor(
...
void RuntimeSpriteObject::UpdateCurrentSprite() const {
  ...
  ptrToCurrentSprite->GetSFMLSprite().setOrigin(
      ptrToCurrentSprite->GetCenter().GetX(),
      ptrToCurrentSprite->GetCenter().GetY());
  ;
  ptrToCurrentSprite->GetSFMLSprite().setRotation(
      multipleDirections ? 0 : currentAngle);
  ptrToCurrentSprite->GetSFMLSprite().setPosition(
      X + (ptrToCurrentSprite->GetCenter().GetX() -
           ptrToCurrentSprite->GetOrigin().GetX()) *
              fabs(scaleX),
      Y + (ptrToCurrentSprite->GetCenter().GetY() -
           ptrToCurrentSprite->GetOrigin().GetY()) *
              fabs(scaleY));
  if (isFlippedX)
    ptrToCurrentSprite->GetSFMLSprite().move(
        (ptrToCurrentSprite->GetSFMLSprite().getLocalBounds().width / 2 -
         ptrToCurrentSprite->GetCenter().GetX()) *
            fabs(scaleX) * 2,
        0);
  if (isFlippedY)
    ptrToCurrentSprite->GetSFMLSprite().move(
        0,
        (ptrToCurrentSprite->GetSFMLSprite().getLocalBounds().height / 2 -
         ptrToCurrentSprite->GetCenter().GetY()) *
            fabs(scaleY) * 2);
  ptrToCurrentSprite->GetSFMLSprite().setScale(scaleX, scaleY);
  ptrToCurrentSprite->GetSFMLSprite().setColor(
      sf::Color(colorR, colorV, colorB, opacity));
...
void RuntimeSpriteObject::Update(const RuntimeScene& scene) {
  ...
  if (timeElapsedOnCurrentSprite > delay) {
    if (delay != 0) {
      std::size_t frameCount =
          static_cast<std::size_t>(timeElapsedOnCurrentSprite / delay);
  ...
  if (currentSprite >= direction.GetSpritesCount()) {
    if (direction.IsLooping())
      ...
      currentSprite = direction.GetSpritesCount() - 1;
  }
    ...
    for (std::size_t j = 0; j < polygons[i].vertices.size(); ++j) {
      sf::Vector2f newVertice = currentSFMLSprite.getTransform().transformPoint(
          !isFlippedX
              ...
              : GetCurrentSprite().GetSFMLSprite().getLocalBounds().width -
                    polygons[i].vertices[j].x,
          !isFlippedY
              ? polygons[i].vertices[j].y
              : GetCurrentSprite().GetSFMLSprite().getLocalBounds().height -
                    polygons[i].vertices[j].y);
...
bool RuntimeSpriteObject::SetSprite(std::size_t nb) {
  if (currentAnimation >= GetAnimationsCount() ||
      currentDirection >=
          animations[currentAnimation].Get().GetDirectionsCount() ||
      nb >= animations[currentAnimation]
                .Get()
...
bool RuntimeSpriteObject::SetCurrentAnimation(
    const gd::String& newAnimationName) {
...
bool RuntimeSpriteObject::SetDirection(float nb) {
  ...
  if (!animations[currentAnimation].Get().useMultipleDirections) {
    ...
    if (nb >= animations[currentAnimation].Get().GetDirectionsCount() ||
        animations[currentAnimation].Get().GetDirection(nb).HasNoSprites())
      return false;
...
void RuntimeSpriteObject::SetColor(unsigned int r,
                                   unsigned int v,
                                   unsigned int b) {
  colorR = r;
  colorV = v;
...
#if defined( \
    ANDROID)  // TODO: Accurate test leads to strange result with touches.
  accurate = false;
#endif

  RuntimeLayer& theLayer = scene.GetRuntimeLayer(layer);
  auto insideObject = [this, accurate](const sf::Vector2f& pos) {
    if (GetDrawableX() <= pos.x && GetDrawableX() + GetWidth() >= pos.x &&
      ...
      return (!accurate || GetCurrentSprite()
                                   .GetSFMLTexture()
                                   ->image.getPixel(localX, localY)
                                   .a != 0);
    }
...
#if defined(GD_IDE_ONLY)
void RuntimeSpriteObject::GetPropertyForDebugger(std::size_t propertyNb,
                                                 gd::String& name,
                                                 gd::String& value) const {
  if (propertyNb == 0) {
    name = _("Animation");
    value = gd::String::From(GetCurrentAnimation());
  } else if (propertyNb == 1) {
    name = _("Direction");
    value = gd::String::From(GetCurrentDirection());
  } else if (propertyNb == 2) {
    name = _("Image");
    value = gd::String::From(GetSpriteNb());
    ...
    name = _("X Scale");
    value = gd::String::From(GetScaleX());
  } else if (propertyNb == 6) {
    name = _("Y Scale");
    value = gd::String::From(GetScaleY());
  }
}

bool RuntimeSpriteObject::ChangeProperty(std::size_t propertyNb,
                                         gd::String newValue) {
  if (propertyNb == 0) {
    return SetCurrentAnimation(newValue.To<int>());
  } else if (propertyNb == 1) {
    if (currentAnimation >= GetAnimationsCount()) return false;

    return animations[currentAnimation].Get().useMultipleDirections
               ? SetDirection(newValue.To<std::size_t>())
               : SetAngle(newValue.To<float>());
  } else if (propertyNb == 2) {
    return SetSprite(newValue.To<int>());
  } else if (propertyNb == 3) {
    SetOpacity(newValue.To<float>());
  } else if (propertyNb == 4) {
    SetBlendMode(newValue.To<int>());
  } else if (propertyNb == 5) {
    SetScaleX(newValue.To<float>());
  } else if (propertyNb == 6) {
    SetScaleY(newValue.To<float>());
  }

  return true;
}

std::size_t RuntimeSpriteObject::GetNumberOfProperties() const { return 7; }
#endif

AnimationProxy::AnimationProxy() : animation(new gd::Animation) {}
AnimationProxy::~AnimationProxy() { delete animation; }
