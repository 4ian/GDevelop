...
JsCodeEventDialog::JsCodeEventDialog(wxWindow* parent,
  ...
  for (unsigned int i = 0; i < 12; ++i)
    codeEdit->StyleSetFont(i, gd::EventsRenderingHelper::Get()->GetFont());
  ...
  codeEdit->SetKeyWords(
      0,
      wxT("break case catch continue default delete do else finally for "
  ...
  codeEdit->SetKeyWords(2,
                        wxT("push join slice splice Math exec match split "
                            "length toString alert window"));
...
void JsCodeEventDialog::onCancelBtClick(wxCommandEvent& event) { EndModal(0); }
#endif
