...
ProjectExportDialog::ProjectExportDialog(wxWindow* parent,
                                         gd::Project& project_)
    : BaseProjectExportDialog(parent), project(project_) {
  ...
  if (wxDirExists(project.GetLastCompilationDirectory())) {
    ...
    exportFolderEdit->SetValue(wxFileName::GetHomeDir() +
                               wxFileName::GetPathSeparator() +
                               DeleteInvalidCharacters(project.GetName()));
    cocosExportFolderEdit->SetValue(wxFileName::GetHomeDir() +
                                    wxFileName::GetPathSeparator() +
                                    DeleteInvalidCharacters(project.GetName()));
  }
  ...
  wxConfigBase::Get()->Read(
      "Export/JS platform/LatestExportType", &latestPage, 0);
  exportChoice->SetSelection(latestPage);

  // Check if we last used debug mode.
  bool debugMode = false;
  wxConfigBase::Get()->Read(
      "Export/JS platform/LatestDebugMode", &debugMode, false);
...
void ProjectExportDialog::OnExportBtClicked(wxCommandEvent& event) {
  ...
  if (dir.IsOpened() && dir.HasFiles()) {
    if (wxMessageBox(wxString::Format(
                         wxString("The export folder (%s) is not empty: all "
                                  "files in this folder will be erased.\nAre "
                                  "you sure you want to continue?"),
                         GetExportDir().c_str()),
                     _("The export folder is not empty"),
                     wxYES_NO | wxICON_WARNING) == wxNO)
      return;
  }

  project.SetLastCompilationDirectory(GetExportDir());
  wxConfigBase::Get()->Write("Export/JS platform/LatestExportType",
                             exportChoice->GetSelection());
  wxConfigBase::Get()->Write("Export/JS platform/LatestDebugMode",
                             IsDebugMode());
  EndModal(1);
}
...
void ProjectExportDialog::OnBrowseBtClick(wxCommandEvent& event) {
  wxDirDialog dialog(this,
                     _("Choose a folder, empty if possible, where the game "
                       "should be exported."));
...
gd::String ProjectExportDialog::GetExportDir() {
  if (GetExportType() == PixiCordova)
    return gd::String(wxFileName::GetHomeDir()) +
           wxString(wxFileName::GetPathSeparator()) +
           gd::String(DeleteInvalidCharacters(project.GetName()));
