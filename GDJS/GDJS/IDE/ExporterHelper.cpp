...
const std::vector<gd::String> ExporterHelper::ExportResources(
...
const gd::String ExporterHelper::GenerateServiceWorker(
    const std::vector<gd::String> includesFiles,
    const std::vector<gd::String> resourcesNames) {
  gd::String generatedIncludes = "[";
  gd::String staticIncludes = "['/','/index.html',";
  gd::String resources = "[";

  for (const auto &includeName : includesFiles) {
    if (fs.IsAbsolute(includeName)) {
      // By convention, absolute files are
      //   1. Not files bundled with GDevelop, but autogenerated code files that
      //   may change between exports
      //   2. Exported under their file name, ignoring the directory they
      //   originate from.
      generatedIncludes += "\"" + fs.FileNameFrom(includeName) + "\",";
    } else {
      // By convention, an include file that is relative is relative to
      // the "<GDJS Root>/Runtime" folder, and therefore is not expected to
      // change unless the GDevelop version is changed.
      staticIncludes += "\"" + includeName + "\",";
    }
  };

  for (const auto &resourceFile : resourcesNames)
    resources += "\"" + resourceFile + "\",";

  // Replace last "," with "]"
  generatedIncludes.pop_back();
  generatedIncludes += ']';
  staticIncludes.pop_back();
  staticIncludes += ']';
  resources.pop_back();
  resources += ']';

  return gd::String(R"js_code(
// While only one cache is present for now, this system is a best practice 
// that would allow in the future to have a different cache invalidated at different times
// e.g. GDJS Runtime code cache could be kept as long as the GDevelop version has not changed.
const CURRENT_CACHES = [
  {
    name: 'gdjs-generated-code-vGENERATED_CODE_CACHE_VERSION',
    files: /*GENERATED_CODE_FILES*/,
  },
  {
    name: 'gdjs-static-code-vSTATIC_CODE_CACHE_VERSION',
    files: /*STATIC_CODE_FILES*/,
  },
  {
    name: 'gdjs-resources-vRESOURCES_CACHE_VERSION',
...
self.addEventListener("install", function (event) {
  ...
  event.waitUntil(
    Promise.all(
      CURRENT_CACHES.map(async (cacheData) => {
        const cache = await caches.open(cacheData.name);
        const cachePromises = cacheData.files.map(async (urlToPrefetch) => {
...
self.addEventListener("activate", function (event) {
  // Delete all caches that aren't named in CURRENT_CACHES, as they must be an old outdated one.
  const expectedCacheNames = new Set(
    CURRENT_CACHES.map((cacheData) => cacheData.name)
  );
...
self.addEventListener("fetch", function (event) {
  ...
  event.respondWith(
    (async () => {
      ...
      // This cache should be refeteched each time a new export is made. A UUID
      // should be good enough.
      .FindAndReplace(
          "GENERATED_CODE_CACHE_VERSION", gd::UUID::MakeUuid4(), /*all=*/false)
      .FindAndReplace(
          "/*GENERATED_CODE_FILES*/", generatedIncludes, /*all=*/false)
      .FindAndReplace("STATIC_CODE_CACHE_VERSION",
                      gd::VersionWrapper::FullString(),
                      /*all=*/false)
      .FindAndReplace("/*STATIC_CODE_FILES*/", staticIncludes, /*all=*/false)
      .FindAndReplace(
          "RESOURCES_CACHE_VERSION", gd::UUID::MakeUuid4(), /*all=*/false)
      .FindAndReplace("/*RESOURCE_FILES*/", resources, /*all=*/false);
};
