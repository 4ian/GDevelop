...
export class HotReloader {
  ...
  /**
   * @param runtimeGame - The `RuntimeGame` to be hot-reloaded.
   */
      ...
      if (!newBehaviorConstructor) {
        this._logs.push({
          ...
          message:
            'Behavior with type ' +
            behaviorTypeName +
            ' was removed from the registered behaviors in ',
        });
      } else {
        if (oldBehaviorConstructor !== newBehaviorConstructor) {
          this._logs.push({
            ...
            message:
              'Behavior with type ' +
              behaviorTypeName +
              ' was changed, and will be re-instantiated in RuntimeObjects using it.',
          });
  ...
  _hotReloadVariablesContainer(
    ...
    newVariablesData.forEach((newVariableData) => {
      ...
      } else if (!gdjs.Variable.isPrimitive(newVariableData.type || 'number')) {
  ...
  _hotReloadRuntimeScene(
    ...
    // Re-instantiate any RuntimeBehavior that was changed.
