import { PixiImageManager, RuntimeGamePixiRenderer } from '.';
...
const hasFadedIn = (object: DisplayObject | null) => {
...
class LoadingScreenPixiRenderer {
  ...
  _loadingScreenContainer: Container;
  _backgroundSprite: Sprite | null = null;
  _gdevelopLogoSprite: Sprite | null = null;
  _progressBarGraphics: Graphics | null = null;
  ...
  constructor(
    ...
    if (backgroundTexture !== imageManager.getInvalidPIXITexture()) {
      this._backgroundSprite = Sprite.from(backgroundTexture);
    ...
    if (loadingScreenData.showGDevelopSplash) {
      this._gdevelopLogoSprite = Sprite.from(gdevelopLogo);
    ...
    if (loadingScreenData.showProgressBar) {
      this._progressBarGraphics = new Graphics();
