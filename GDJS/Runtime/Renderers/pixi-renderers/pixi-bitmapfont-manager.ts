...
const patchInstalledBitmapFont = (
  ...
  BitmapFont.available[bitmapFontInstallKey] = bitmapFont;
  delete BitmapFont.available[defaultName];
  return BitmapFont.available[bitmapFontInstallKey];
};

/**
 * PixiBitmapFontManager loads fnt/xml files (using `fetch`), from the "bitmapFont" resources of the game.
 *
 * It installs the "BitmapFont" with PixiJS to be used with BitmapText.
 */
export class PixiBitmapFontManager {
  private _resources: ResourceData[];
  private _imageManager: PixiImageManager;

  /** BitmapFont used, indexed by their BitmapFont name. */
  private _pixiBitmapFontsInUse: Record<
    string,
    { objectsUsingTheFont: number }
  > = {};

  /** BitmapFont not used anymore, but not yet uninstalled, indexed by their BitmapFont name. */
   ...
   */
  constructor(resources: ResourceData[], imageManager: PixiImageManager) {
    this._resources = resources;
    this._imageManager = imageManager;
  }

  /**
   * Get the instance of the default `BitmapFont`, always available.
   */
  getDefaultBitmapFont() {
    if (this._defaultSlugFontName !== null) {
      return BitmapFont.available[this._defaultSlugFontName];
    }

    // Default bitmap font style
    const fontFamily = 'Arial';
    const bitmapFontStyle = new TextStyle({
    ...
    // Generate default bitmapFont, and replace the name of BitmapFont by a unique name
    const defaultBitmapFont = patchInstalledBitmapFont(
      BitmapFont.from(fontFamily, bitmapFontStyle, {
  ...
  releaseBitmapFont(bitmapFontInstallKey: string) {
    ...
    if (
      this._pixiBitmapFontsInUse[bitmapFontInstallKey].objectsUsingTheFont === 0
    ) {
      ...
      if (this._pixiBitmapFontsToUninstall.length > uninstallCacheSize) {
        ...
        BitmapFont.uninstall(oldestUnloadedPixiBitmapFontName);
        console.log(
  ...
  /**
   * Given a bitmap font resource name and a texture atlas resource name, returns the BitmapFont
   * for it.
  ...
  obtainBitmapFont(
    ...
    if (BitmapFont.available[bitmapFontInstallKey]) {
    ...
    try {
      // Create and install the BitmapFont in memory:
      const bitmapFont = patchInstalledBitmapFont(
        BitmapFont.install(fontData, texture),
        bitmapFontInstallKey
      );
      this._markBitmapFontAsUsed(bitmapFontInstallKey);
      return bitmapFont;
...
export const BitmapFontManager = PixiBitmapFontManager;
export type BitmapFontManager = PixiBitmapFontManager;
