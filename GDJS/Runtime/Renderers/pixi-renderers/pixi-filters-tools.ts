...
import { rgbToHexNumber } from '../../gd';
import { Filter as PIXIFilter } from '@pixi/core';
...
export const registerFilterCreator = function (
  ...
  if (_filterCreators.hasOwnProperty(filterName)) {
    console.warn(
      'Filter "' +
        filterName +
        '" was already registered in PixiFiltersTools. Replacing it with the new one.'
    );
  }
...
export type FilterCreator = {
  ...
  makePIXIFilter: (layer: Layer, effectData: EffectData) => any;
  /** The function to be called to update the filter at every frame */
  update: (filter: PIXIFilter, layer: Layer) => any;
