import { RuntimeGamePixiRenderer, LayerPixiRenderer } from '.';
import type { RuntimeScene } from '../../runtimescene';
import type { RuntimeObject } from '../../runtimeobject';
import type { Layer } from '../../layer';
import { SpriteRuntimeObject } from '../../spriteruntimeobject';
...
/**
 * The renderer for a RuntimeScene using js.
 */
export class RuntimeScenePixiRenderer {
  ...
  _debugDrawContainer: Container | null = null;
  _profilerText: Text | null = null;
  _debugDrawRenderedObjectsPoints: Record<
    number,
    {
      wasRendered: boolean;
      points: Record<string, Text>;
    }
  >;

  constructor(
    runtimeScene: RuntimeScene,
    runtimeGameRenderer: RuntimeGamePixiRenderer | null
  ) {
  ...
  /**
   * Render graphics for debugging purpose. Activate this in `RuntimeScene`,
  ...
  renderDebugDraw(
    ...
    if (!this._debugDraw || !this._debugDrawContainer) {
      this._debugDrawContainer = new Container();
      this._debugDraw = new Graphics();
    ...
    for (let i = 0; i < instances.length; i++) {
      ...
      if (object instanceof SpriteRuntimeObject) {
      ...
      // Draw custom point
      if (showCustomPoints && object instanceof SpriteRuntimeObject) {
  ...
  setLayerIndex(layer: Layer, index: float): void {
    const layerPixiRenderer: LayerPixiRenderer = layer.getRenderer();
    let layerPixiObject:
      | Container
      | Sprite
...
export type RuntimeSceneRenderer = RuntimeScenePixiRenderer;
export const RuntimeSceneRenderer = RuntimeScenePixiRenderer;
