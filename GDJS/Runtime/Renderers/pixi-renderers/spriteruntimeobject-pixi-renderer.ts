import { rgbToHex, toRad } from '../../gd';
import type { RuntimeScene } from '../../runtimescene';
import type {
  SpriteRuntimeObject,
  SpriteAnimationFrame,
} from '../../spriteruntimeobject';
import { hex2rgb } from '@pixi/utils';
import { Sprite } from '@pixi/sprite';

/**
 * The renderer for a SpriteRuntimeObject using js.
 */
export class SpriteRuntimeObjectPixiRenderer {
  _object: SpriteRuntimeObject;
   ...
   */
  constructor(runtimeObject: SpriteRuntimeObject, runtimeScene: RuntimeScene) {
  ...
  /**
   * Update the internal Sprite position, angle...
   */
  _updatePIXISprite() {
    if (this._object._animationFrame !== null) {
      ...
      this._sprite.rotation = toRad(this._object.angle);
  ...
  updateX(): void {
    const animationFrame = this._object._animationFrame as SpriteAnimationFrame;
    this._sprite.position.x =
      this._object.x +
  ...
  getColor() {
    const rgb = hex2rgb(this._sprite.tint);
    return (
