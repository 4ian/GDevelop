...
namespace gdjs {
  ...
  class ResourcePack {
    ...
    /**
     * Download the archive, then index its content.
     */
    load(
      onProgress?: (loadedBytes: integer, totalBytes: integer) => void
    ): Promise<void> {
      return this._download(onProgress).then((blob) => this._readIndex(blob));
    }

    /**
     * `XMLHttpRequest` is used rather than `fetch`: it works on the `file:`
     * URLs of Electron and Cordova games, and reports progress natively.
     */
    private _download(
      onProgress?: (loadedBytes: integer, totalBytes: integer) => void
    ): Promise<Blob> {
      return new Promise<Blob>((resolve, reject) => {
        ...
        request.onload = () => {
          const blob: Blob | null = request.response;
          // A `file:` URL answers with a status of 0.
          const isSuccess =
            (request.status >= 200 && request.status < 300) ||
            (request.status === 0 && !!blob && blob.size > 0);
          if (!isSuccess || !blob) {
            reject(
              new Error(
                `Could not download the resource pack "${this._url}" (status ${request.status}).`
              )
            );
            return;
          }
        ...
        request.onerror = () =>
          reject(
            new Error(
              `Could not download the resource pack "${this._url}" (network error).`
            )
          );
        request.onabort = () =>
          reject(
            new Error(
              `The download of the resource pack "${this._url}" was aborted.`
            )
          );
    ...
    private async _readIndex(blob: Blob): Promise<void> {
      ...
      if (version !== SUPPORTED_PACK_VERSION) {
        throw new Error(
          `The resource pack "${this._url}" uses the unsupported version ${version}.`
        );
      }
  ...
  export class ResourcePackManager {
    ...
    /**
     * @returns true when the game was exported with packed resources.
     */
    hasPacks(): boolean {
