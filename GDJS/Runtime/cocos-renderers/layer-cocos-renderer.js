...
gdjs.LayerCocosRenderer = function(layer, runtimeSceneRenderer) {
  ...
  if (effects.length === 0) {
    ...
    var gameRenderer = runtimeSceneRenderer._runtimeScene
      .getGame()
      .getRenderer(); //TODO
    this._renderTexture = new cc.RenderTexture(
      gameRenderer.getCurrentWidth(),
      gameRenderer.getCurrentHeight()
    );
    this._renderTexture.setPosition(
      cc.p(
        gameRenderer.getCurrentWidth() / 2,
        gameRenderer.getCurrentHeight() / 2
      )
    );
...
gdjs.LayerCocosRenderer.prototype._makeShaders = function() {
  ...
  for (var i = 0; i < effect.uniformNames.length; ++i) {
    ...
    theShader.uniforms[name] = theShader.shader.getUniformLocationForName(name);
  }
...
gdjs.LayerCocosRenderer.prototype.changeRendererObjectZOrder = function(
  child,
  newZOrder
) {
...
gdjs.LayerCocosRenderer.prototype.setEffectParameter = function(
  name,
  parameterName,
  value
) {
