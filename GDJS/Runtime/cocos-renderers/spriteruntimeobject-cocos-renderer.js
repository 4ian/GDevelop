...
gdjs.SpriteRuntimeObjectCocosRenderer.prototype.updateFrame = function(
  animationFrame
) {
  this._spriteDirty = true;

  //TODO Perf: Avoid calling cc.SpriteFrame.createWithTexture each time updateFrame is called?
  var spriteFrame = cc.SpriteFrame.createWithTexture(
...
gdjs.SpriteRuntimeObjectCocosRenderer.prototype._updateCocosSprite = function() {
  if (this._object._animationFrame !== null) {
    ...
    if (this._object._flippedX)
      xPos +=
        (this._cachedTextureWidth / 2 - this._object._animationFrame.center.x) *
        Math.abs(this._object._scaleX) *
        2;
    if (this._object._flippedY)
      yPos +=
        (this._cachedTextureHeight / 2 -
          this._object._animationFrame.center.y) *
        Math.abs(this._object._scaleY) *
        2;
...
gdjs.SpriteRuntimeObjectCocosRenderer.prototype.updateY = function() {
  ...
  if (this._object._flippedY)
    yPos +=
      (this._cachedTextureHeight / 2 - this._object._animationFrame.center.y) *
      Math.abs(this._object._scaleY) *
      2;
  this._sprite.setPositionY(this._convertYPosition(yPos));
};
...
gdjs.SpriteRuntimeObjectCocosRenderer.getAnimationFrame = function(
  imageManager,
  imageName
) {
...
gdjs.SpriteRuntimeObjectCocosRenderer.getAnimationFrameWidth = function(
  texture
) {
  return texture.pixelsWidth;
};

gdjs.SpriteRuntimeObjectCocosRenderer.getAnimationFrameHeight = function(
  texture
) {
