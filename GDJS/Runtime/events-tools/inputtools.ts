import { pickObjectsIf } from './objecttools';
...
export const isKeyPressed = function (runtimeScene, key) {
  if (keysNameToCode.hasOwnProperty(key)) {
    return runtimeScene
      .getGame()
      .getInputManager()
      .isKeyPressed(keysNameToCode[key]);
  }
  return false;
};
...
export const wasKeyReleased = function (runtimeScene, key) {
  if (keysNameToCode.hasOwnProperty(key)) {
    return runtimeScene
      .getGame()
      .getInputManager()
      .wasKeyReleased(keysNameToCode[key]);
  }
  return false;
};
...
export const cursorOnObject = function (
  ...
  return pickObjectsIf(_cursorIsOnObject, objectsLists, inverted, runtimeScene);
};

export const getTouchX = function (runtimeScene, identifier, layer, camera) {
  return runtimeScene
    .getLayer(layer)
    .convertCoords(
...
export const getTouchY = function (runtimeScene, identifier, layer, camera) {
  return runtimeScene
    .getLayer(layer)
    .convertCoords(
      runtimeScene.getGame().getInputManager().getTouchX(identifier),
...
export const getLastTouchId = function () {
  return lastTouchId || 0;
};

export const getLastEndedTouchId = function () {
  return lastEndedTouchId || 0;
};
