import { staticArray, staticArray2, Hashtable, RuntimeObject } from '..';
import type { RuntimeScene } from '..';
import { angleDifference } from './commontools';
...
export const pickOnly = function (
  objectsLists: Hashtable<Array<RuntimeObject>>,
...
export const twoListsTest = function (
  predicate: (
    object1: RuntimeObject,
    object2: RuntimeObject,
    extraArg: any
  ) => boolean,
  objectsLists1: Hashtable<Array<RuntimeObject>>,
  objectsLists2: Hashtable<Array<RuntimeObject>>,
  inverted: boolean,
  extraArg: any
) {
  let isTrue = false;
  const objects1Lists = staticArray(twoListsTest);
  objectsLists1.values(objects1Lists);
  const objects2Lists = staticArray2(twoListsTest);
...
export const pickObjectsIf = function (
  ...
  let isTrue = false;
  const lists = staticArray(pickObjectsIf);
  ...
  // Trim not picked objects from lists.
  for (let i = 0, leni = lists.length; i < leni; ++i) {
    filterPickedObjectsList(lists[i]);
  }
  return isTrue;
};
 ...
 */
export const filterPickedObjectsList = function (arr: RuntimeObject[]) {
  let finalSize = 0;
  for (let k = 0, lenk = arr.length; k < lenk; ++k) {
    const obj = arr[k];
    if (obj.pick) {
...
export const _movesToward = function (obj1, obj2, tolerance) {
  ...
  return (
    Math.abs(angleDifference(obj1.getAverageForce().getAngle(), objAngle)) <=
    tolerance / 2
  );
};
...
export const _turnedToward = function (obj1, obj2, tolerance) {
  ...
  objAngle *= 180 / 3.14159;
  return Math.abs(angleDifference(obj1.getAngle(), objAngle)) <= tolerance / 2;
};
...
export const pickRandomObject = function (runtimeScene, objectsLists) {
  ...
  // Find the object
  let startIndex = 0;
  let theChosenOne: RuntimeObject | null = null;
  for (let listName in objectsLists.items) {
    if (objectsLists.items.hasOwnProperty(listName)) {
      let list = objectsLists.items[listName];
  ...
  // @ts-ignore
  pickOnly(objectsLists, theChosenOne);
  return true;
};
...
export const pickNearestObject = function (objectsLists, x, y, inverted) {
  ...
  let best = 0;
  let first = true;
  const lists = staticArray(pickNearestObject);
  objectsLists.values(lists);
  ...
  pickOnly(objectsLists, bestObject);
  return true;
};
...
export const raycastObject = function (
  ...
  return raycastObjectToPosition(
    objectsLists,
    x,
    y,
...
export const raycastObjectToPosition = function (
  ...
  let resultX = 0;
  let resultY = 0;
  const lists = staticArray(raycastObjectToPosition);
  objectsLists.values(lists);
  ...
  pickOnly(objectsLists, matchObject);
...
export const doCreateObjectOnScene = function (
  objectsContext: EventsFunctionContext | RuntimeScene,
  objectName: string,
  objectsLists: Hashtable<Array<RuntimeObject>>,
  x,
  y,
  ...
  // objectsContext will either be the RuntimeScene or, in an events function, the
...
export const createObjectFromGroupOnScene = function (
  ...
  doCreateObjectOnScene(objectsContext, objectName, objectsLists, x, y, layer);
};
...
export const pickedObjectsCount = function (objectsLists) {
  let size = 0;
  const lists = staticArray(pickedObjectsCount);
  objectsLists.values(lists);
  for (let i = 0, len = lists.length; i < len; ++i) {
    size += lists[i].length;
  }
