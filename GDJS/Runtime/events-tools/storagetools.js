...
gdjs.evtTools.storage.loadJSONFileFromStorage = function(filename) {
  if (gdjs.evtTools.storage.loadedFiles.containsKey(filename)) return; //Already loaded.
...
gdjs.evtTools.storage.unloadJSONFile = function(filename) {
  if (!gdjs.evtTools.storage.loadedFiles.containsKey(filename)) return; //Not loaded.
  ...
  if (gdjs.evtTools.storage.fileUtils) {
    ...
    try {
      ...
      console.log(
        'Unable to save data to localStorage for "' + filename + '"!'
      );
    }
  }
...
gdjs.evtTools.storage.readNumberFromJSONFile = function(
  filename,
  element,
  runtimeScene,
  variable
  ...
  for (var i = 0; i < elemArray.length; ++i) {
    ...
    if (
      ...
      typeof currentElem[elemArray[i]].value !== 'undefined'
    )
...
gdjs.evtTools.storage.readStringFromJSONFile = function(
  filename,
  element,
  runtimeScene,
  variable
  ...
  for (var i = 0; i < elemArray.length; ++i) {
    ...
    if (
      i == elemArray.length - 1 &&
      typeof currentElem[elemArray[i]].str !== 'undefined'
    )
