import { Howl, Howler, HowlEvent, HowlOptions, HowlCallback } from 'howler';
...
export class HowlerSound {
  ...
  setRate(rate: float): this {
    if (this._id !== null) {
      rate = HowlerSoundManager.clampRate(rate);
...
export class HowlerSoundManager {
  ...
  createHowlerSound(soundName: string, isMusic: boolean): HowlerSound {
    ...
    return new HowlerSound(cacheContainer[soundFile]);
  }
  ...
  playSoundOnChannel(
    ...
    var sound = this.createHowlerSound(soundName, /* isMusic= */ false).play();
  ...
  playMusicOnChannel(
    ...
    const music = this.createHowlerSound(soundName, /* isMusic= */ true).play();
