...
(function(root, undefined) {
  ...
  HSHG.prototype.queryForCollisionWith = function(theObject, result) {
    ...
    var theObjectHashInItsGrid = theObject.HSHG.grid.toHash(
      theObjectAABB.min[0],
      theObjectAABB.min[1]
    );
    var theObjectCellInItsGrid =
    ...
    for (i = 0; i < this._grids.length; i++) {
      ...
      if (grid.cellSize === theObject.HSHG.grid.cellSize) {
        ...
        for (k = i + 1; k < this._grids.length; k++) {
          ...
          var objectHashInBiggerGrid = biggerGrid.toHash(
            theObjectAABB.min[0],
            theObjectAABB.min[1]
          );
        ...
        //We're in a grid with smaller objects
