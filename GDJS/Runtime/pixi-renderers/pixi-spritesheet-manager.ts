...
namespace gdjs {
   ...
   * - "meta": Object containing the texture atlas image reference and optional metadata
   */
  export class PixiSpritesheetManager implements gdjs.ResourceManager {
     ...
     */
    private _loadedSpritesheetData =
      new gdjs.ResourceCache<SpritesheetJsonData>();

    /**
     * PIXI.Spritesheet objects that have been created and parsed.
     */
    private _loadedPixiSpritesheets =
      new gdjs.ResourceCache<PIXI.Spritesheet>();
    ...
    private async _loadSpritesheet(
      ...
      try {
        ...
        const baseTexture =
          this._imageManager.getPIXITexture(imageResourceName);

        if (!baseTexture || !baseTexture.valid) {
          throw new Error(
            `Failed to load atlas texture '${imageResourceName}' for spritesheet '${resource.name}'.`
          );
        }

        // Create the PIXI.Spritesheet and parse it to generate frame `PIXI.Texture`s.
        ...
        logger.error(`Error loading spritesheet '${resource.name}':`, error);
        throw error;
      }
    }
     ...
     */
    getSpritesheetFramePixiTexture(
      resourceName: string,
      frameName: string
    ): PIXI.Texture {
      const spritesheet =
        this._loadedPixiSpritesheets.getFromName(resourceName);
     ...
     */
    getPixiSpritesheet(resourceName: string): PIXI.Spritesheet | null {
      return this._loadedPixiSpritesheets.getFromName(resourceName);
    }
     ...
     * Clear all loaded spritesheets (both the JSON data and the associated PIXI.Spritesheet objects).
     */
    dispose(): void {
