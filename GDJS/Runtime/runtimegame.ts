...
export class RuntimeGame {
   ...
   */
  _debuggerClient: IDebuggerClient | null;
  ...
  constructor(data: ProjectData, options?: RuntimeGameOptions) {
    ...
    this._variables = new VariablesContainer(data.variables);
    this._data = data;
    this._imageManager = new ImageManager(this._data.resources.resources);
    this._soundManager = new SoundManager(this._data.resources.resources);
    this._fontManager = new FontManager(this._data.resources.resources);
    this._jsonManager = new JsonManager(this._data.resources.resources);
    this._bitmapFontManager = new BitmapFontManager(
    ...
    this._renderer = new RuntimeGameRenderer(
      this,
      this._options.forceFullscreen || false
    );
    this._sceneStack = new SceneStack(this);
    this._inputManager = new InputManager();
    this._injectExternalLayout = this._options.injectExternalLayout || '';
    this._debuggerClient = DebuggerClient ? new DebuggerClient(this) : null;
  ...
  getRenderer(): RuntimeGameRenderer {
  ...
  /**
   * Get the SoundManager of the RuntimeGame.
  ...
  /**
   * Get the ImageManager of the RuntimeGame.
  ...
  /**
   * Get the BitmapFontManager of the RuntimeGame.
   * @return The bitmap font manager.
   */
  getBitmapFontManager(): BitmapFontManager {
    // @ts-ignore
  ...
  setGameResolutionSize(width: float, height: float): void {
    ...
    if (this._adaptGameResolutionAtRuntime) {
      if (
        RuntimeGameRenderer &&
        RuntimeGameRenderer.getWindowInnerWidth &&
        RuntimeGameRenderer.getWindowInnerHeight
      ) {
        const windowInnerWidth = RuntimeGameRenderer.getWindowInnerWidth();
        const windowInnerHeight = RuntimeGameRenderer.getWindowInnerHeight();
  ...
  loadAllAssets(callback: () => void, progressCallback?: (float) => void) {
    const loadingScreen = new LoadingScreenRenderer(
      this.getRenderer(),
      this._imageManager,
  ...
  /**
   * Return the stack of {@link RuntimeScene} being played.
   */
  getSceneStack(): SceneStack {
