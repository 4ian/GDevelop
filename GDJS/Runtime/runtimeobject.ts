 ...
 *
 * A `RuntimeObject` should not be instantiated directly, always a child class
 * (because RuntimeObject don't call onCreated at the end of its constructor).
 */
export class RuntimeObject {
  ...
  //Hit boxes:
  protected _defaultHitBoxes: Polygon[] = [];
  protected hitBoxes: Polygon[];
  protected hitBoxesDirty: boolean = true;
  protected aabb: AABB = { min: [0, 0], max: [0, 0] };

  //Variables:
  protected _variables: VariablesContainer;

  //Forces:
  protected _forces: Force[] = [];
  _averageForce: Force;

  /**
   * Contains the behaviors of the object.
   */
  protected _behaviors: RuntimeBehavior[] = [];
  protected _behaviorsTable: Hashtable<RuntimeBehavior>;
  protected _timers: Hashtable<Timer>;

  /**
   * @param runtimeScene The scene the object belongs to..
   * @param objectData The initial properties of the object.
   */
  constructor(runtimeScene: RuntimeScene, objectData: ObjectData) {
    this.name = objectData.name || '';
    this.type = objectData.type || '';
    this._nameId = RuntimeObject.getNameIdentifier(this.name);
    this.id = runtimeScene.createNewUniqueId();
    this._runtimeScene = runtimeScene;
    this._defaultHitBoxes.push(Polygon.createRectangle(0, 0));
    this.hitBoxes = this._defaultHitBoxes;
    this._variables = new VariablesContainer(
      objectData ? objectData.variables : undefined
    );
    this._averageForce = new Force(0, 0, 0);
    ...
    for (let i = 0, len = objectData.behaviors.length; i < len; ++i) {
      ...
      const Ctor = getBehaviorConstructor(autoData.type);
   ...
   * * Set `YourRuntimeObject.supportsReinitialization = true;` to declare support for recycling.
   * * Implement `reinitialize`. It **must** call the `reinitialize` of `RuntimeObject`, and call `this.onCreated();`
  ...
  reinitialize(objectData: ObjectData): void {
    ...
    for (const len = objectData.behaviors.length; i < len; ++i) {
      ...
      const Ctor = getBehaviorConstructor(behaviorData.type);
   ...
   * @param runtimeScene The RuntimeScene the object belongs to.
   */
  update(runtimeScene: RuntimeScene): void {}
   ...
   */
  deleteFromScene(runtimeScene: RuntimeScene): void {
   ...
   */
  onDestroyFromScene(runtimeScene: RuntimeScene): void {
    const theLayer = runtimeScene.getLayer(this.layer);
  ...
  rotateTowardAngle(
    ...
    const angularDiff = angleDifference(this.getAngle(), angle);
    ...
    if (
      // @ts-ignore
      (angleDifference(newAngle, angle) > 0) ^
      diffWasPositive
    ) {
      newAngle = angle;
    }
   ...
   */
  rotate(speed: float, runtimeScene: RuntimeScene): void {
    this.setAngle(
   ...
   * @static
   */
  static getVariableNumber(variable: Variable): number {
   ...
   * @static
   */
  static returnVariable(variable: Variable): Variable {
    return variable;
  }
   ...
   * @static
   */
  static getVariableString(variable: Variable): string {
   ...
   */
  static setVariableBoolean = function (variable: Variable, newValue: boolean) {
  ...
  static getVariableBoolean = function (
    ...
    return getVariableBoolean(variable, compareWith);
  };
   ...
   * @private
   * @param {Variable} variable
   * @see {toggleVariableBoolean}
   */
  static toggleVariableBoolean = function (variable: Variable) {
   ...
   * @static
   */
  static getVariableChildCount(variable: Variable): integer {
   ...
   */
  static setVariableNumber(variable: Variable, newValue: float): void {
   ...
   */
  static setVariableString(variable: Variable, newValue: string) {
   ...
   */
  private static variableClearChildren(variable: Variable): void {
   ...
   * @private
   */
  static variablePushCopy = function (array: Variable, variable: Variable) {
   ...
   * @private
   */
  static variableRemoveAt = function (array: Variable, index: number) {
   ...
   */
  private _getRecycledForce(x: float, y: float, multiplier: integer): Force {
    if (RuntimeObject.forcesGarbage.length === 0) {
      return new Force(x, y, multiplier);
    } else {
      const recycledForce = RuntimeObject.forcesGarbage.pop() as Force;
  ...
  addPolarForce(angle: float, len: float, multiplier: integer): void {
    const angleInRadians = toRad(angle);
  ...
  getHitBoxes(): Polygon[] {
    ...
    //(var rectangle = Polygon.createRectangle(this.getWidth(), this.getHeight());
    //...)
  ...
  updateHitBoxes(): void {
    ...
    this.hitBoxes[0].rotate(toRad(this.getAngle()));
   ...
   * is visible on screen. The RuntimeScene will hide the renderer object if
  ...
  timerElapsedTime(timerName: string, timeInSeconds: float): boolean {
    if (!this._timers.containsKey(timerName)) {
      this._timers.put(timerName, new Timer(timerName));
      return false;
    }
      ...
      if (objects[i].id != this.id) {
        ...
        for (let k = 0, lenk = hitBoxes.length; k < lenk; ++k) {
          for (let l = 0, lenl = otherHitBoxes.length; l < lenl; ++l) {
            const result = Polygon.collisionTest(
              hitBoxes[k],
              otherHitBoxes[l],
              ignoreTouchingEdges
            );
   ...
   */
  getDistanceToObject(otherObject: RuntimeObject): float {
   ...
   * @param otherObject The other object
   */
  getSqDistanceToObject(otherObject: RuntimeObject): float {
    if (otherObject === null) {
      return 0;
    }
    const x =
   ...
   * @param otherObject The other object
   */
  getAngleToObject(otherObject: RuntimeObject): float {
    if (otherObject === null) {
      return 0;
    }
    const x =
  ...
  static collisionTest(
    ...
    for (let k = 0, lenBoxes1 = hitBoxes1.length; k < lenBoxes1; ++k) {
      for (let l = 0, lenBoxes2 = hitBoxes2.length; l < lenBoxes2; ++l) {
        if (
          Polygon.collisionTest(hitBoxes1[k], hitBoxes2[l], ignoreTouchingEdges)
            .collision
        ) {
          return true;
        }
  ...
  raycastTest(
    ...
    // @ts-ignore
    let result = Polygon.raycastTestStatics.result;
    ...
    for (let i = 0; i < hitBoxes.length; i++) {
      const res = Polygon.raycastTest(hitBoxes[i], x, y, endX, endY);
      if (res.collision) {
   ...
   *
   * The position should be in "world" coordinates, i.e use Layer.convertCoords
   * if you need to pass the mouse or a touch position that you get from InputManager.
   *
   */
  ...
  isCollidingWithPoint(pointX: float, pointY: float): boolean {
    const hitBoxes = this.getHitBoxes();
    for (let i = 0; i < this.hitBoxes.length; ++i) {
      if (Polygon.isPointInside(hitBoxes[i], pointX, pointY)) {
        return true;
      }
    }
    return false;
   ...
   * @static
   */
  static forcesGarbage: Array<Force> = [];
