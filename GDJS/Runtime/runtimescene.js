  ...
  for (var i = 0, len = data.length; i < len; ++i) {
    ...
    if (newObject !== null) {
      newObject.setPosition(
        parseFloat(instanceData.x) + xPos,
        parseFloat(instanceData.y) + yPos
      );
  ...
  if (this._timeManager.isFirstFrame()) {
    ...
    for (var i = 0, len = this._allInstancesList.length; i < len; ++i) {
      ...
      if (object.isHidden()) {
        ...
        if (
          aabb && // If no AABB is returned, the object should always be visible
...
gdjs.RuntimeScene.prototype.addObject = function(obj) {
  if (!this._instances.containsKey(obj.name)) {
    console.log(
      'RuntimeScene.addObject: No objects called "' + obj.name + '"! Adding it.'
    );
...
gdjs.RuntimeScene.prototype.getObjects = function(name) {
  if (!this._instances.containsKey(name)) {
    console.log(
      'RuntimeScene.getObjects: No instances called "' + name + '"! Adding it.'
    );
