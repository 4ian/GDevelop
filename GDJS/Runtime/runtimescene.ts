...
export class RuntimeScene {
  ...
  // Set to `new Profiler()` to have profiling done on the scene.
  _onProfilerStopped: null | ((oldProfiler: Profiler) => void) = null;
  ...
  constructor(runtimeGame: RuntimeGame) {
    ...
    this._renderer = new RuntimeSceneRenderer(
      this,
      // @ts-ignore
   ...
   * See RuntimeGame.startGameLoop in particular.
   */
  onGameResolutionResized() {
    for (const name in this._layers.items) {
      if (this._layers.items.hasOwnProperty(name)) {
        /** @type Layer */
        const theLayer: Layer = this._layers.items[name];
  ...
  loadFromScene(sceneData: LayoutData | null) {
    ...
    //Load variables
    this._variables = new VariablesContainer(sceneData.variables);
    ...
    if (this._runtimeGame && !this._runtimeGame.wasFirstSceneLoaded()) {
      for (let i = 0; i < callbacksFirstRuntimeSceneLoaded.length; ++i) {
        callbacksFirstRuntimeSceneLoaded[i](this);
      }
    }
    for (let i = 0; i < callbacksRuntimeSceneLoaded.length; ++i) {
      callbacksRuntimeSceneLoaded[i](this);
    }
   ...
   * @see RuntimeScene#registerObject
   */
  ...
  /**
   * Register a {@link RuntimeObject} so that instances of it can be used in the scene.
  ...
  registerObject(objectData: ObjectData) {
    ...
    const Ctor = getObjectConstructor(objectData.type);
  ...
  /**
   * Update the data of a {@link RuntimeObject} so that instances use this when constructed.
   * @param objectData The data for the object to register.
   */
  ...
  /**
   * Unregister a {@link RuntimeObject}. Instances will be destroyed.
   ...
   */
  onPause() {
    for (let i = 0; i < callbacksRuntimeScenePaused.length; ++i) {
      callbacksRuntimeScenePaused[i](this);
    }
  }
  ...
  onResume() {
    ...
    for (let i = 0; i < callbacksRuntimeSceneResumed.length; ++i) {
      callbacksRuntimeSceneResumed[i](this);
    }
  }
  ...
  unloadScene() {
    ...
    for (let i = 0; i < callbacksRuntimeSceneUnloading.length; ++i) {
      callbacksRuntimeSceneUnloading[i](this);
    }
    ...
    for (let i = 0; i < callbacksRuntimeSceneUnloaded.length; ++i) {
      callbacksRuntimeSceneUnloaded[i](this);
    }
  ...
  renderAndStep(elapsedTime: float): boolean {
    ...
    for (let i = 0; i < callbacksRuntimeScenePreEvents.length; ++i) {
      callbacksRuntimeScenePreEvents[i](this);
    }
    if (this._profiler) {
    ...
    for (let i = 0; i < callbacksRuntimeScenePostEvents.length; ++i) {
      callbacksRuntimeScenePostEvents[i](this);
    }
    if (this._profiler) {
  ...
  setBackgroundColor(r: integer, g: integer, b: integer): void {
    this._backgroundColor = parseInt(rgbToHex(r, g, b), 16);
  }
   ...
   */
  createObject(objectName: string): RuntimeObject | null {
    if (
      !this._objectsCtor.containsKey(objectName) ||
      !this._objects.containsKey(objectName)
    ) {
   ...
   */
  markObjectForDeletion(obj: RuntimeObject) {
    ...
    // Notify the global callbacks
    for (let j = 0; j < callbacksObjectDeletedFromScene.length; ++j) {
      callbacksObjectDeletedFromScene[j](this, obj);
    }
    return;
  }
  ...
  addLayer(layerData: LayerData) {
    this._layers.put(layerData.name, new Layer(layerData, this));
  }

  /**
   * Remove a layer. All {@link RuntimeObject} on this layer will
  ...
  setLayerIndex(layerName: string, index: integer): void {
    const layer: Layer = this._layers.get(layerName);
    if (!layer) {
      return;
    }
   ...
   * @return The TimeManager of the scene.
   */
  ...
  /**
   * Request a scene change to be made. The change is handled externally (see SceneStack)
   ...
   */
  startProfiler(onProfilerStopped: (oldProfiler: Profiler) => void) {
    if (this._profiler) {
      return;
    }
  ...
  /**
   * Get a list of all RuntimeObject living on the scene.
   ...
   */
  getAdhocListOfAllInstances(): RuntimeObject[] {
    this._constructListOfAllInstances();
