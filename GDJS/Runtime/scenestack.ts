import { RuntimeScene, SceneChangeRequest } from './runtimescene';
import { RuntimeGame } from './runtimegame';

/**
 * Hold the stack of scenes ({@link RuntimeScene}) being played.
 */
export class SceneStack {
  ...
  constructor(runtimeGame: RuntimeGame) {
    if (!runtimeGame) {
      throw 'SceneStack must be constructed with a RuntimeGame.';
    }
  ...
  step(elapsedTime: float): boolean {
    ...
    if (currentScene.renderAndStep(elapsedTime)) {
      ...
      if (request === SceneChangeRequest.STOP_GAME) {
        this._runtimeGame.getRenderer().stopGame();
        return true;
      } else if (request === SceneChangeRequest.POP_SCENE) {
        this.pop();
      } else if (request === SceneChangeRequest.PUSH_SCENE) {
        this.push(currentScene.getRequestedScene());
      } else if (request === SceneChangeRequest.REPLACE_SCENE) {
        this.replace(currentScene.getRequestedScene());
      } else if (request === SceneChangeRequest.CLEAR_SCENES) {
  ...
  push(newSceneName: string, externalLayoutName?: string): RuntimeScene {
    ...
    // Load the new one
    const newScene = new RuntimeScene(this._runtimeGame);
  ...
  /**
   * Return the current RuntimeScene being played, or null if none is run.
   */
  getCurrentScene(): RuntimeScene | null {
    if (this._stack.length === 0) {
      return null;
    }
