...
namespace gdjs {
  ...
  export type SimEvent =
    ...
    | { frame: number; event: 'stuck'; object: string; x: number; y: number; z: number; duration: number };
  ...
  export class SimulationHarness {
    ...
    private _targetObjectName: string | null = null;
    private _targetCoords: { x: number; y: number; z: number } | null = null;
    ...
    async goToScene(name: string): Promise<void> {
      this._game
        .getSceneStack()
        .replace({ sceneName: name, clear: true });
    ...
    async stepUntil(
      ...
      options: {
        ...
        stuckDetection?: {
          ...
          onStuck: (state: { frame: number; x: number; y: number; z: number }) => void;
        };
      }
    ): Promise<boolean> {
          ...
          if (objs.length > 0) {
            ...
            if (posHistory.length >= stuck.windowFrames) {
              ...
              if (
                ...
                stuck.onStuck({ frame: this._framesExecuted, x: pos.x, y: pos.y, z: pos.z });
              }
            }
          }
        }
     ...
     *
     * @param referenceObjectName The object whose rotation to adjust (e.g. 'Player').
     * @param targetObjectName The object to look toward.
    ...
    async lookToward(
      ...
      options?: { tolerance?: number; maxFrames?: number }
    ): Promise<boolean> {
      ...
      const ref = this.getObjects(referenceObjectName)[0];
      if (!ref) return false;
      const layerName = ref.layer || '';

      // gdjs.scene3d.camera is the correct namespace for 3D camera tools.
      const scene3dCamera = (gdjs as any).scene3d && (gdjs as any).scene3d.camera;
      const targetObjs = scene.getObjects(targetObjectName);
      if (targetObjs.length === 0) return false;

      // Pick nearest target instance
      const pickNearest = (objs: gdjs.RuntimeObject[]): gdjs.RuntimeObject => {
        ...
        const refCz = ref.z;
        for (const t of objs) {
          const ddx = t.getX() + t.getWidth() / 2 - refCx;
          const ddy = t.getY() + t.getHeight() / 2 - refCy;
          const ddz = ((t as any).getZ ? (t as any).getZ() : 0) - refCz;
      ...
      const tgt = pickNearest(targetObjs);

      // --- 3D FPS path: use turnCameraTowardObject to compute the ideal angles,
      // then drive the FPS behavior there via mouse deltas so it stores the correct
      // internal state. We cannot rely on turnCameraTowardObject alone because the
      // FPS behavior overwrites the camera every frame from its own internal angles.
      const has3DAPI = scene3dCamera &&
        scene3dCamera.turnCameraTowardObject &&
        scene3dCamera.getCameraRotationY &&
        scene3dCamera.getCameraRotationX;
      ...
      if (has3DAPI) {
        ...
        // Step 1: Point camera at target to read the ideal yaw/pitch angles.
        // This is a read operation only — no frames stepped here.
        scene3dCamera.turnCameraTowardObject(scene, tgt, layerName, 0, false);
        const idealYaw: number = scene3dCamera.getCameraRotationY(scene, layerName, 0);
        const idealPitch: number = scene3dCamera.getCameraRotationX(scene, layerName, 0);

        // Step 2: Let the FPS behavior take one frame so it resets the camera from
        // its own internal state. Calibrate sensitivity from the resulting baseline.
        this._game.getSceneStack().step(1000 / 60);
        this._game.getInputManager().onFrameEnded();
        this._framesExecuted++;
        await new Promise<void>((r) => setTimeout(r, 1000 / 60));

        // Calibrate yaw (horizontal mouse → camera Y rotation)
        const preCalibYaw: number = scene3dCamera.getCameraRotationY(scene, layerName, 0);
        this.setMouseDelta(calibPx, 0);
        this._game.getSceneStack().step(1000 / 60);
        this._game.getInputManager().onFrameEnded();
        this._framesExecuted++;
        await new Promise<void>((r) => setTimeout(r, 1000 / 60));
        const postCalibYaw: number = scene3dCamera.getCameraRotationY(scene, layerName, 0);
        const yawRate = (postCalibYaw - preCalibYaw) / calibPx;

        // Calibrate pitch (vertical mouse → camera X rotation)
        const preCalibPitch: number = scene3dCamera.getCameraRotationX(scene, layerName, 0);
        this.setMouseDelta(0, calibPx);
        this._game.getSceneStack().step(1000 / 60);
        this._game.getInputManager().onFrameEnded();
        this._framesExecuted++;
        await new Promise<void>((r) => setTimeout(r, 1000 / 60));
        const postCalibPitch: number = scene3dCamera.getCameraRotationX(scene, layerName, 0);
        const pitchRate = (postCalibPitch - preCalibPitch) / calibPx;

        // If mouse has no effect at all, we cannot rotate the camera
        if (Math.abs(yawRate) < 0.001 && Math.abs(pitchRate) < 0.001) return false;
        ...
        for (let i = 0; i < maxFrames; i++) {
          ...
          const currentYaw: number = scene3dCamera.getCameraRotationY(scene, layerName, 0);
          const currentPitch: number = scene3dCamera.getCameraRotationX(scene, layerName, 0);

          const yawError = this._shortAngle(currentYaw, idealYaw);
          const pitchError = idealPitch - currentPitch;

          if (Math.abs(yawError) <= tolerance && Math.abs(pitchError) <= tolerance) {
            return true;
          }

          const dx = Math.abs(yawRate) > 0.001
            ? Math.max(-100, Math.min(100, yawError / yawRate)) : 0;
          const dy = Math.abs(pitchRate) > 0.001
            ? Math.max(-100, Math.min(100, pitchError / pitchRate)) : 0;

          this.setMouseDelta(dx, dy);
          this._game.getSceneStack().step(1000 / 60);
          this._game.getInputManager().onFrameEnded();
          this._framesExecuted++;
          this._trackObjectEvents();
        ...
        const finalYaw: number = scene3dCamera.getCameraRotationY(scene, layerName, 0);
        const finalPitch: number = scene3dCamera.getCameraRotationX(scene, layerName, 0);
        return Math.abs(this._shortAngle(finalYaw, idealYaw)) <= tolerance &&
    ...
    getNearby(
      ...
      const ref = refs[0];
      const refCx = ref.getX() + ref.getWidth() / 2;
      const refCy = ref.getY() + ref.getHeight() / 2;
      const refCz = (ref as any).getZ ? (ref as any).getZ() : 0;
    ...
    /**
     * Declare a navigation target for use with getNavigationHint.
     * Call this once (or whenever the target changes) before stepping frames.
     *
     * Overload 1 — target an object type by name (always picks the nearest instance):
     *   harness.setTarget('Coin');
     *   Useful for moving targets (enemies, collectibles) since the nearest instance
     *   is re-resolved each frame.
     *
     * Overload 2 — target specific world coordinates:
     *   harness.setTarget({ x: 928, y: -64 });
     *   harness.setTarget({ x: 928, y: -64, z: 0 });
     *   Use when you know the destination position (e.g. a checkpoint coordinate
     *   from getObjects() or hardcoded data).
     */
    setTarget(target: string | { x: number; y: number; z?: number }): void {
      if (typeof target === 'string') {
        this._targetObjectName = target;
        this._targetCoords = null;
      } else {
        this._targetCoords = { x: target.x, y: target.y, z: target.z || 0 };
        this._targetObjectName = null;
      }
    }

    /**
     * Returns directional movement hints toward the current target (set via setTarget).
     * Uses the first instance of `referenceObjectName` as the origin.
     * Returns null if no target has been set or no reference/target instance exists.
     *
    ...
    getNavigationHint(
      ...
      if (!this._targetObjectName && !this._targetCoords) return null;
      ...
      if (this._targetCoords) {
        targetCx = this._targetCoords.x;
        targetCy = this._targetCoords.y;
        targetCz = this._targetCoords.z;
      } else {
        const targets = scene.getObjects(this._targetObjectName!);
        if (targets.length === 0) return null;

        // Always pick the nearest instance
        let best = targets[0];
        let bestDist = Infinity;
        for (const t of targets) {
          const dx = t.getX() + t.getWidth() / 2 - refCx;
          const dy = t.getY() + t.getHeight() / 2 - refCy;
          const dz = ((t as any).getZ ? (t as any).getZ() : 0) - refCz;
          const d = Math.sqrt(dx * dx + dy * dy + dz * dz);
          if (d < bestDist) { bestDist = d; best = t; }
        }

        targetCx = best.getX() + best.getWidth() / 2;
        targetCy = best.getY() + best.getHeight() / 2;
        targetCz = (best as any).getZ ? (best as any).getZ() : 0;
      }
      ...
      const angleToTargetVertical = Math.atan2(dz, horizontalDist) * 180 / Math.PI;
      const refPitch = (ref as any).getRotationX ? -(ref as any).getRotationX() : 0;
    ...
    async execute(sceneName: string, scriptBody: string): Promise<void> {
      ...
      const capture = (level: string, original: (...args: any[]) => void) => (...args: any[]) => {
      ...
      try {
        ...
        inputManager.onMouseButtonReleased(gdjs.InputManager.MOUSE_RIGHT_BUTTON);
        inputManager.onMouseButtonReleased(gdjs.InputManager.MOUSE_MIDDLE_BUTTON);
      ...
      for (const name of names) {
        ...
        for (const id of prevIds) {
          if (!currentIds.has(id)) {
            this._eventLog.push({ frame: this._framesExecuted, event: 'removed', object: name });
          }
        }
        for (const id of currentIds) {
          if (!prevIds.has(id)) {
            this._eventLog.push({ frame: this._framesExecuted, event: 'spawned', object: name });
          }
        }
    ...
    getResult(): SimulationResult {
      ...
      if (scene) {
        ...
        sceneVariables = scene.getVariables().getNetworkSyncData({
          shouldExcludeVariableFromData: () => false,
        }) || [];
      }
    ...
    private static _keyNameToCode: { [name: string]: number } = {
      ...
      Shift: 16,    LShift: 1016,   RShift: 2016,
      Control: 17,  LControl: 1017, RControl: 2017,
      Alt: 18,      LAlt: 1018,     RAlt: 2018,
      LSystem: 1091, RSystem: 2091,
      // Function keys
      ...
      // Numpad arrows / nav
      NumpadLeft: 3037, NumpadUp: 3038, NumpadRight: 3039, NumpadDown: 3040,
      NumpadPageUp: 3033, NumpadPageDown: 3034, NumpadEnd: 3035, NumpadHome: 3036,
      NumpadReturn: 3013,
      // Numpad operators
      Add: 107, NumpadAdd: 3107,
      Subtract: 109, NumpadSubtract: 3109,
      Multiply: 106, NumpadMultiply: 3106,
      Divide: 111, NumpadDivide: 3111,
      // Punctuation
      SemiColon: 186, Comma: 188, Period: 190, Quote: 222,
      Slash: 191, BackSlash: 220, Equal: 187, Dash: 189,
      LBracket: 219, RBracket: 221, Tilde: 192,
    };
  }
}
