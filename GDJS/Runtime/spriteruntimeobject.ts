...
/** Represents a {@link SpriteAnimationFrame}. */
...
/** Represents the data of a {@link SpriteAnimationDirection}. */
...
/** Represents the data of a {@link SpriteAnimation}. */
export type SpriteAnimationData = {
  /** The name of the animation. */
  name: string;
  /** Does the animation use multiple {@link SpriteAnimationDirection}? */
  useMultipleDirections: boolean;
  /** The list of {@link SpriteDirectionData} representing {@link SpriteAnimationDirection} instances. */
  directions: Array<SpriteDirectionData>;
};

/** Represents the data of a {@link SpriteRuntimeObject}. */
export type SpriteObjectDataType = {
  /** Update the object even if he is not visible?. */
  updateIfNotVisible: boolean;
  /** The list of {@link SpriteAnimationData} representing {@link SpriteAnimation} instances. */
...
export class SpriteAnimationFrame {
   ...
   */
  constructor(imageManager: ImageManager, frameData: SpriteFrameData) {
    this.image = frameData ? frameData.image : '';
    this.texture = SpriteRuntimeObjectRenderer.getAnimationFrame(
      imageManager,
      this.image
    );
   ...
   * @param frameData The frame data used to initialize the frame
   */
  reinitialize(imageManager: ImageManager, frameData: SpriteFrameData) {
    this.points.clear();
    ...
    if (center.automatic !== true) {
      ...
      this.center.x =
        SpriteRuntimeObjectRenderer.getAnimationFrameWidth(this.texture) / 2;
      this.center.y =
        SpriteRuntimeObjectRenderer.getAnimationFrameHeight(this.texture) / 2;
    }
    ...
    if (frameData.hasCustomCollisionMask) {
      ...
      for (let len = frameData.customCollisionMask.length; i < len; ++i) {
        ...
        if (i >= this.customHitBoxes.length) {
          this.customHitBoxes.push(new Polygon());
        }
        let j = 0;
...
export class SpriteAnimationDirection {
   ...
   * @param directionData The direction data used to initialize the direction
   */
  reinitialize(imageManager: ImageManager, directionData: SpriteDirectionData) {
    this.timeBetweenFrames = directionData
      ? directionData.timeBetweenFrames
      : 1.0;
    this.loop = !!directionData.looping;
    ...
    for (const len = directionData.sprites.length; i < len; ++i) {
      ...
      if (i < this.frames.length) {
        ...
        this.frames.push(new SpriteAnimationFrame(imageManager, frameData));
      }
    }
...
export class SpriteAnimation {
  ...
  name: string;
  directions: SpriteAnimationDirection[] = [];
   ...
   * @param animData The animation data used to initialize the animation
   */
  reinitialize(imageManager: ImageManager, animData: SpriteAnimationData) {
    this.hasMultipleDirections = !!animData.useMultipleDirections;
    this.name = animData.name || '';
    let i = 0;
    for (const len = animData.directions.length; i < len; ++i) {
      ...
      if (i < this.directions.length) {
        ...
        this.directions.push(
          new SpriteAnimationDirection(imageManager, directionData)
        );
      }
    }
...
export class SpriteRuntimeObject extends RuntimeObject {
   ...
   *
   */
  _animationFrame: SpriteAnimationFrame | null = null;
  _renderer: SpriteRuntimeObjectRenderer;
  hitBoxesDirty: any;
  ...
  constructor(runtimeScene, spriteObjectData) {
    ...
    this._renderer = new SpriteRuntimeObjectRenderer(this, runtimeScene);
  ...
  updateHitBoxes(): void {
    ...
    for (let i = 0; i < this._animationFrame.customHitBoxes.length; ++i) {
      if (i >= this.hitBoxes.length) {
        this.hitBoxes.push(new Polygon());
      }
      for (
        let j = 0;
  ...
  getPointX(name: string): float {
    ...
    const pos = staticArray(SpriteRuntimeObject.prototype.getPointX);
  ...
  getPointY(name: string): float {
    ...
    const pt = this._animationFrame.getPoint(name);
    const pos = staticArray(SpriteRuntimeObject.prototype.getPointY);
    this._transformToGlobal(pt.x, pt.y, pos);
    return pos[1];
  }
  /**
