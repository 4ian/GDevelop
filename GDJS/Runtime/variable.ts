 ...
 */
type Children = Record<string, Variable>;
...
export class Variable {
      ...
      if (this._type === 'number') {
        ...
        if (varData.children !== undefined) {
          for (var i = 0, len = varData.children.length; i < len; ++i) {
            ...
            this._children[childData.name] = new Variable(childData);
          }
        }
      } else if (this._type === 'array' && varData.children) {
        for (const childData of varData.children)
          this._childrenArray.push(new Variable(childData));
      }
    }
  }
   ...
   */
  static copy(source: Variable, target: Variable, merge?: boolean): Variable {
   ...
   */
  isPrimitive() {
    return Variable.isPrimitive(this._type);
  }
   ...
   */
  getChild(childName: string): Variable {
   ...
   */
  addChild(childName: string, childVariable: Variable): this {
   ...
   */
  replaceChildrenArray(newChildren: Variable[]) {
    this._type = 'array';
   ...
   * @return true if the variable is a structure.
   * @deprecated Use {@link Variable.getType} instead.
   */
  isStructure(): boolean {
  ...
  getChildAt(index: integer) {
    ...
    if (
      ...
      this._childrenArray[index] = new Variable();
   ...
   */
  pushVariableCopy(variable: Variable) {
    this.castTo('array');
