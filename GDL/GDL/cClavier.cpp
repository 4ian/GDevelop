...
/**
 * Test if a key, retrieved from a text expression, is pressed
 */
bool CondKeyFromTextPressed( RuntimeScene & scene, ObjectsConcerned & objectsConcerned, const Instruction & condition )
{
    bool Ok = false;

    string Key = condition.GetParameter( 0 ).GetAsTextExpressionResult(scene, objectsConcerned);
...
{
    return scene.inputKeyPressed ^ condition.IsInverted();
}
