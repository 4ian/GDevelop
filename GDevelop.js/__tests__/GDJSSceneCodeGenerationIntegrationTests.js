...
describe('libGD.js - GDJS Scene Code Generation integration tests', function () {
  ...
  it('resolves global config placeholders in string action parameters for scene and external events', function () {
    const project = new gd.ProjectHelper.createNewGDJSProject();
  ...
  it('does not resolve global config placeholders in extension event actions', function () {
    const project = new gd.ProjectHelper.createNewGDJSProject();
    project.setGlobalConfigJson(
      JSON.stringify({
        labels: {
    ...
    const serializedEvents = gd.Serializer.fromJSObject([
      {
        ...
        actions: [
          {
            type: { value: 'SetStringVariable' },
            parameters: ['Result', '=', '"{{labels.scene}}"'],
          },
        ],
        events: [],
      },
