...
@Serializable
data class CapabilityVersionRange(val minimum: Int, val maximum: Int) {
	init {
	...
	companion object {
		fun exact(version: Int) = CapabilityVersionRange(version, version)
	}
}

@Serializable
enum class CapabilityUse { REQUIRED, OPTIONAL }
@Serializable
enum class CapabilityScopeKind { OPERATION, EXTENSION }
@Serializable
data class CapabilityScope(val kind: CapabilityScopeKind, val value: String)
...
@Serializable
data class ArtifactCapabilityReport(
...
fun RuntimeCapabilityId.requiredByOperation(operation: String, minimum: Int = 1, maximum: Int = minimum) =
	CapabilityRequirement(
		this, CapabilityVersionRange(minimum, maximum), CapabilityUse.REQUIRED,
