...
@Serializable data class ActionDescriptor(val type:String,val parameters:List<ParameterDescriptor>,val runtimeEntry:String,val requiredCapabilities:Set<RuntimeCapabilityId> = emptySet(),val memberPath:List<String> = type.split("::"),val contracts:ContractVersions=ContractVersions(),val capabilityRequirements:Set<CapabilityRequirement> = requiredCapabilities.mapTo(linkedSetOf()){it.requiredByOperation(type)})
@Serializable data class ConditionDescriptor(val type:String,val parameters:List<ParameterDescriptor>,val runtimeEntry:String,val requiredCapabilities:Set<RuntimeCapabilityId> = emptySet(),val memberPath:List<String> = type.split("::"),val contracts:ContractVersions=ContractVersions(),val capabilityRequirements:Set<CapabilityRequirement> = requiredCapabilities.mapTo(linkedSetOf()){it.requiredByOperation(type)})
...
class ExtensionCatalog private constructor(val snapshot:CatalogSnapshot,private val actions:Map<String,RegisteredAction>,private val conditions:Map<String,RegisteredCondition>,private val providers:List<ExtensionProvider>,private val valueTypes:Map<ValueTypeId,ValueTypeDescriptor>){
 ...
 companion object {
  ...
  private fun freeze(e:ExtensionDescriptor)=e.copy(actions=e.actions.map{it.copy(parameters=it.parameters.toList(),requiredCapabilities=it.requiredCapabilities.toSet(),memberPath=it.memberPath.toList(),capabilityRequirements=it.capabilityRequirements.toSet())},conditions=e.conditions.map{it.copy(parameters=it.parameters.toList(),requiredCapabilities=it.requiredCapabilities.toSet(),memberPath=it.memberPath.toList(),capabilityRequirements=it.capabilityRequirements.toSet())},lifecycleHooks=e.lifecycleHooks.toList(),aliases=e.aliases.map{it.copy(canonical=it.canonical.copy(path=it.canonical.path.toList()))},dependencies=e.dependencies.toList(),valueTypes=e.valueTypes.toList(),metadataMembers=e.metadataMembers.map{it.copy(path=it.path.toList(),parameters=it.parameters.toList())})
  ...
  private fun canonical(ds:List<ExtensionDescriptor>)=ds.joinToString("\n"){e->listOf(e.identity.namespace,e.identity.version,e.identity.origin,e.contracts.toString(),e.dependencies.sortedBy{it.namespace}.joinToString(),(e.actions.map{"a:${it.memberPath.joinToString("/")}:${it.parameters.joinToString{p->"${p.name}:${p.serializedType}:${p.valueType.value}:${p.optional}:${p.defaultValue}"}}:${it.runtimeEntry}:${it.contracts}:${capabilities(it.capabilityRequirements)}"}+e.conditions.map{"c:${it.memberPath.joinToString("/")}:${it.parameters.joinToString{p->"${p.name}:${p.serializedType}:${p.valueType.value}:${p.optional}:${p.defaultValue}"}}:${it.runtimeEntry}:${it.contracts}:${capabilities(it.capabilityRequirements)}"}+e.metadataMembers.map{"m:${it.kind}:${it.path.joinToString("/")}:${it.parameters.joinToString{p->"${p.name}:${p.serializedType}:${p.valueType.value}:${p.optional}:${p.defaultValue}"}}:${it.runtimeEntry}:${it.contracts}"}).sorted().joinToString("|"),e.aliases.sortedBy{it.serializedName}.joinToString()).joinToString(";")}
  private fun capabilities(requirements:Set<CapabilityRequirement>)=requirements.map{"${it.id.value}:${it.supportedVersions.minimum}..${it.supportedVersions.maximum}:${it.use}:${it.scope.kind}:${it.scope.value}"}.sorted().joinToString(",")
