...
object Phase0Catalog {
	val builtins = ExtensionDescriptor(
		...
		actions = listOf(
			ActionDescriptor(
				...
				listOf(
					ParameterDescriptor("variable", ValueTypes.Variable),
					ParameterDescriptor("operator", ValueTypes.Identifier),
					ParameterDescriptor("value", ValueTypes.Number)
				),
				"builtin.setNumber"
			),
		),
	...
	val myDummyExtension = ExtensionDescriptor(
		...
		actions = listOf(
			ActionDescriptor(
				"MyDummyExtension::DoSomething",
				listOf(ParameterDescriptor("number1", "expression"), ParameterDescriptor("number2", "expression")),
				"gdjs._myExtension.doSomething"
			)
		),
		lifecycleHooks = emptyList(),
		conditions = listOf(
			ConditionDescriptor(
				"MyDummyExtension::MyCondition",
				listOf(ParameterDescriptor("number1", "expression"), ParameterDescriptor("number2", "expression")),
				"gdjs._myExtension.myConditionFunction"
			)
		),
		metadataMembers = listOf(
			MetadataMemberDescriptor(
				ExtensionMemberKind.EXPRESSION,
				listOf("MyDummyExtension", "MyExpression"),
				listOf(ParameterDescriptor("number1", "expression")),
				"gdjs._myExtension.myExpressionFunction"
			),
