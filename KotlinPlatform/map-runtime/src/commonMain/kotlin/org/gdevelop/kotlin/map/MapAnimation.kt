...
sealed interface MapAnimationTraceRecord {
    ...
    @Serializable
    data class CameraStateObserved(val sequence: Long?, val event: String, val camera: MapCameraState) : MapAnimationTraceRecord
    @Serializable
    data class Cancelled(val target: String, val sequence: Long? = null, val overlayId: MapOverlayId? = null) : MapAnimationTraceRecord
    @Serializable
    data class Completed(val target: String, val sequence: Long? = null, val overlayId: MapOverlayId? = null) : MapAnimationTraceRecord
}
...
class MapOverlayAnimationRuntime(
    ...
    fun start(id: MapOverlayId, start: GeoCoordinate, end: GeoCoordinate, durationMillis: Long,
              easing: MapAnimationEasing = MapAnimationEasing.LINEAR): MapOverlayAnimationState {
        cancel(id)
