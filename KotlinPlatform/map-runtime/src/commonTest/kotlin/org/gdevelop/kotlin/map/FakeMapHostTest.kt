...
private class FakeMapHost : MapHost {
    ...
    fun click(screen: ScreenCoordinate, coordinate: GeoCoordinate) = sink?.invoke(MapInteractionEvent.Clicked(screen, coordinate))
    ...
    override suspend fun project(coordinate: GeoCoordinate) = ready(ScreenCoordinate(coordinate.longitude * 100 + 20, coordinate.latitude * 100 + 10))
    override suspend fun unproject(point: ScreenCoordinate) = ready(GeoCoordinate((point.x - 20) / 100, (point.y - 10) / 100))
    override suspend fun projectToWorld(coordinate: GeoCoordinate) = ready(ProjectedPoint(coordinate.longitude / 360 + .5, .5 - coordinate.latitude / 180))
    override suspend fun unprojectFromWorld(point: ProjectedPoint) = ready(GeoCoordinate((point.x - .5) * 360, (.5 - point.y) * 180))
    override suspend fun queryFeatures(query: FeatureQuery) = ready(listOf(MapFeatureRecord("fixture", geometry = JsonObject(emptyMap()))))
    ...
    private fun <T> ready(value: T): MapHostResult<T> = if (lifecycle == MapHostLifecycle.READY) MapHostResult.Success(value) else failure()
}
...
class FakeMapHostTest {
    @Test fun projectionIsDeterministicAndRoundTrips() {
        val host = FakeMapHost()
        runSuspend { host.initialize {} }
        val projected = assertIs<MapHostResult.Success<ScreenCoordinate>>(runSuspend { host.project(GeoCoordinate(10.0, 20.0)) }).value
        assertEquals(ScreenCoordinate(1020.0, 2010.0), projected)
        assertEquals(GeoCoordinate(10.0, 20.0), assertIs<MapHostResult.Success<GeoCoordinate>>(runSuspend { host.unproject(projected) }).value)
    }

    @Test fun clickTracePreservesHostOrderAndDisposeStopsCallbacks() {
        val host = FakeMapHost()
    ...
    @Test fun cameraCommandsRetainInvocationOrder() {
    ...
    @Test fun geographicBoundariesAreExplicit() {
