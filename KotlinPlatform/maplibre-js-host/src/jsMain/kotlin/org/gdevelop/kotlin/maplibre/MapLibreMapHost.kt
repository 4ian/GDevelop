...
class MapLibreMapHost(
    ...
    override suspend fun initialize(eventSink: (MapInteractionEvent) -> Unit): MapHostResult<Unit> {
        ...
        return suspendCoroutine { continuation ->
            ...
            listen("error") { event ->
                ...
                val code = if (sourceId != null) MapLibreDiagnosticCode.SOURCE_LOAD_FAILED else MapLibreDiagnosticCode.INVALID_STYLE
    ...
    override suspend fun queryFeatures(query: FeatureQuery) = ready { target ->
        val queryOptions = jsObject<QueryOptions> { layers = query.layerIds.map { it.value }.toTypedArray().takeIf { it.isNotEmpty() } }
        val records = target.queryRenderedFeatures(arrayOf(query.screen.x, query.screen.y), queryOptions).map(::portableFeature)
        MapHostResult.Success(records)
    }

    fun addGeoJsonSource(id: MapSourceId, geoJson: JsonElement): MapHostResult<Unit> = readySync { target ->
        target.addSource(id.value, jsObject<GeoJsonSourceOptions> { type = "geojson"; data = JSON.parse(geoJson.toString()) })
        MapHostResult.Success(Unit)
    }
    ...
    private fun emitPointer(event: dynamic, clicked: Boolean) {
        ...
        emit(if (clicked) MapInteractionEvent.Clicked(screen, coordinate) else MapInteractionEvent.PointerMoved(screen, coordinate))
    }

    private fun emit(event: MapInteractionEvent) { if (lifecycle != MapHostLifecycle.DISPOSED) eventSink?.invoke(event) }
    ...
    private fun readCamera(target: MapLibreGl.Map): MapCameraState {
        ...
        return MapCameraState(GeoCoordinate(center.lng, center.lat), target.getZoom(), target.getBearing(), target.getPitch())
    }
    ...
    private fun <T> unavailable(message: String): MapHostResult<T> = mapOptionsFailure(MapLibreDiagnosticCode.CAPABILITY_UNAVAILABLE, message)
...
private fun cameraOptions(camera: MapCameraState, duration: Long? = null): CameraOptions =
    cameraOptions(camera.bearing, camera.pitch, duration, null).also { it.center = camera.center.array(); it.zoom = camera.zoom }

private fun cameraOptions(bearing: Double?, pitch: Double?, duration: Long?, padding: Double?): CameraOptions = jsObject {
