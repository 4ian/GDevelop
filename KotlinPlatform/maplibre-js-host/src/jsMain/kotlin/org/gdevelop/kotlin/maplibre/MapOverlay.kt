...
class MapOverlayAdapter(
	...
	init {
		layers.root.addEventListener("pointerdown", pointerListener, true)
	}

	fun put(value: GeoOverlayObject) {
		objects[value.id] = value
	}

	fun remove(id: MapOverlayId) {
		objects.remove(id)
	}
	...
	private suspend fun updateProjection(eventCamera: MapCameraState?) {
		...
		projected = objects.values.map { value ->
			...
			ProjectedOverlay(
				value.id, position, value.width, value.height,
