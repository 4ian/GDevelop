...
import org.gdevelop.kotlin.extensions.CapabilityProviderIdentity
import org.gdevelop.kotlin.extensions.ProvidedCapability
...
import org.gdevelop.kotlin.runtime.HostInvocation
import org.gdevelop.kotlin.runtime.HostOperationResult
import org.gdevelop.kotlin.runtime.HostTransientInput
import org.gdevelop.kotlin.runtime.OrderedRuntimeHost
...
class MapTilesHostOperationExecutor(
	...
	fun acceptEvent(event: MapInteractionEvent) {
		when (event) {
			...
			is MapInteractionEvent.CameraIdle -> {
				cameraAnimating = false; cameraIdle = true
			}
	...
	fun clearTransientEvents() {
		clicked = false; cameraIdle = false
	}
	...
	override fun invoke(invocation: HostInvocation, completion: (HostOperationResult) -> Unit) {
		scope.launch {
			completion(
				if (executeAction(invocation.operation)) HostOperationResult.Success()
	...
	suspend fun executeAction(operation: ExtensionHostOperation): Boolean {
		...
		val result: MapHostResult<*> = when (operation.runtimeEntry) {
			MapTilesEntries.SET_CAMERA -> {
				...
				target.execute(
					if (duration > 0) MapCameraCommand.Ease(
						requested,
						duration
					) else MapCameraCommand.Jump(requested)
				)
			}
			...
			MapTilesEntries.READ_CAMERA -> target.cameraState().also { result ->
				result.valueOrNull()?.let { camera ->
					...
					variables.writeNumber(a[2], camera.zoom); variables.writeNumber(
					a[3],
					camera.bearing
				); variables.writeNumber(a[4], camera.pitch)
				}
			}
			...
			MapTilesEntries.UNPROJECT -> target.unproject(ScreenCoordinate(a.number(0), a.number(1))).also { result ->
				result.valueOrNull()
					?.let { variables.writeNumber(a[2], it.longitude); variables.writeNumber(a[3], it.latitude) }
			}
			...
			MapTilesEntries.QUERY_FEATURES -> target.queryFeatures(
				FeatureQuery(
	...
	private fun markerVisibility(arguments: List<String>, visible: Boolean): MapHostResult<Unit> {
		...
		marker.anchor =
			marker.anchor.copy(visibility = if (visible) GeoAnchorVisibility.VISIBLE else GeoAnchorVisibility.HIDDEN)
		overlay?.put(marker)
		return MapHostResult.Success(Unit)
	}
	...
	private fun reportFailure(operation: ExtensionHostOperation, failure: MapHostResult.Failure) {
		diagnosticSink(
			Diagnostic(
				"GDKP_RUNTIME_MAP_HOST_${failure.code}",
				Severity.ERROR,
				failure.message,
				operation.origin
			)
