...
private class DemoEditor(private val root: HTMLDivElement) {
		...
		bind("sample") {
			scope.launch {
				try {
					...
					status("Could not load sample: ${error.message}", true)
				}
			}
		}
		createMap()
	...
	private fun createMap() {
		...
		map = MapLibre.Map(options).also { created ->
			...
			created.on("click") { event ->
				if (event.originalEvent.defaultPrevented != true) addMarker(
					event.lngLat.lng as Double,
					event.lngLat.lat as Double
				)
			}
	...
	private fun styleValue(url: String): dynamic =
		if (url.isBlank()) js("({version: 8, sources: {}, layers: []})") else url
	...
	private fun runOperations(event: String) {
		model.operations.filter { it.event == event }.forEach { operation ->
			...
			when (operation.action) {
				"select" -> {
					selectedId = marker.id; updateInputs(); renderMarkers()
				}

				"flyTo" -> flyTo(marker, operation.durationMs ?: 1000)
				"animateMarker" -> animate(
					marker,
					operation.longitude ?: marker.longitude,
					operation.latitude ?: marker.latitude,
	...
	private fun flyTo(marker: MarkerDocument, duration: Int) {
		val options = js("({})"); options.center = arrayOf(marker.longitude, marker.latitude); options.zoom =
			14; options.duration = duration
	...
	private fun animate(marker: MarkerDocument, endLng: Double, endLat: Double, duration: Int) {
		...
		fun frame(now: Double) {
			...
			rendered[marker.id]?.setLngLat(
				arrayOf(
					marker.longitude + (endLng - marker.longitude) * eased,
					marker.latitude + (endLat - marker.latitude) * eased
				)
			)
			if (progress < 1) window.requestAnimationFrame(::frame)
			else {
				model = model.copy(overlays = model.overlays.map {
					if (it.id == marker.id) it.copy(
						longitude = endLng,
						latitude = endLat
					) else it
				})
	...
	private fun operation(event: String, action: String) =
		model.operations.firstOrNull { it.event == event && it.action == action }
	...
	private fun bind(id: String, action: () -> Unit) {
		(root.querySelector("#$id") as HTMLButtonElement).onclick = { action(); Unit }
	}

	private fun status(message: String, error: Boolean = false) {
		root.querySelector("#status")
			?.apply { textContent = message; setAttribute("style", if (error) "color:#ff9da8" else "") }
	}
}
