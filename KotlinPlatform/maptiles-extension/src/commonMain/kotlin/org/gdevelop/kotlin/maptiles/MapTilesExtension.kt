...
private fun p(name: String, type: String) = ParameterDescriptor(
	name, when (name) {
...
object MapTilesExtension : ExtensionProvider {
	override val descriptor = ExtensionDescriptor(
		...
		actions = listOf(
			...
			action(
				"ReadCamera",
				MapTilesEntries.READ_CAMERA,
				p("longitudeVariable", "variable"),
				p("latitudeVariable", "variable"),
				p("zoomVariable", "variable"),
				p("bearingVariable", "variable"),
				p("pitchVariable", "variable")
			),
			action(
				"Project",
				MapTilesEntries.PROJECT,
				p("longitude", "number"),
				p("latitude", "number"),
				p("xVariable", "variable"),
				p("yVariable", "variable")
			),
			action(
				"Unproject",
				MapTilesEntries.UNPROJECT,
				p("x", "number"),
				p("y", "number"),
				p("longitudeVariable", "variable"),
				p("latitudeVariable", "variable")
			...
			action(
				"QueryFeatures",
				MapTilesEntries.QUERY_FEATURES,
				p("layerId", "layer"),
				p("x", "number"),
				p("y", "number"),
				p("resultVariable", "variable")
			),
		),
		conditions = listOf(
