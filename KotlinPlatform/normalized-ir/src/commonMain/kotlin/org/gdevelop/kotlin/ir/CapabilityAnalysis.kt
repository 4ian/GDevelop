...
import org.gdevelop.kotlin.extensions.CapabilityManifest
import org.gdevelop.kotlin.extensions.CapabilityRequirement
import org.gdevelop.kotlin.extensions.CapabilityUse
import org.gdevelop.kotlin.extensions.ResolvedCapability

data class CapabilityAnalysis(
	val requirements: List<CapabilityRequirement>,
	val resolved: List<ResolvedCapability>,
...
object CapabilityAnalyzer {
	fun analyze(program: ProgramIr, manifest: CapabilityManifest): CapabilityAnalysis {
		...
		val resolved = pairs.map { (operation, requirement) ->
			val provided = manifest.capabilities.firstOrNull {
		...
		val diagnostics = pairs.zip(resolved).mapNotNull { (pair, resolution) ->
			...
			if (requirement.use == CapabilityUse.OPTIONAL || resolution.contractVersion?.let { it in requirement.supportedVersions } == true) null
				...
				"GDKP_SEM_MISSING_CAPABILITY",
				Severity.ERROR,
				"Target ${manifest.provider.deterministicId} does not provide ${requirement.id.value} for ${requirement.scope.value}",
				operation.origin
			)
			else Diagnostic(
				"GDKP_SEM_INCOMPATIBLE_CAPABILITY",
				Severity.ERROR,
				"Target provides ${requirement.id.value} contract ${resolution.contractVersion}; supported range is ${requirement.supportedVersions.minimum}..${requirement.supportedVersions.maximum}",
				operation.origin
			)
		}
