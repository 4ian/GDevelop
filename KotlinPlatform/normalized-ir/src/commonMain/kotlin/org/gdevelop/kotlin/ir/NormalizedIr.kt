...
sealed interface ConditionIr {
	...
	data class PickByX(val objectOrGroup: String, val operator: String, val value: Double, val origin: SourceLocation) :
		ConditionIr
...
sealed interface ActionIr {
	...
	data class SetSelectedX(
		val objectOrGroup: String,
		val operator: String,
		val value: Double,
		val origin: SourceLocation
	) : ActionIr

	data class SetSelectedString(
		val objectOrGroup: String,
		val property: String,
		val value: String,
		val origin: SourceLocation
	...
	data class ExtensionCall(
		val memberId: QualifiedMemberId,
		val serializedType: String,
		val arguments: List<ResolvedArgument>,
...
data class ExtensionHostOperation(
	...
	val contracts: ContractVersions,
	val origin: SourceLocation,
	val capabilityRequirements: Set<CapabilityRequirement> = requiredCapabilities.mapTo(
		linkedSetOf()
	) { it.requiredByOperation(serializedType) }
) {
	val descriptorType get() = memberId.path.joinToString("::");
	val extensionIdentity get() = memberId.extension;
	val parameterOrder get() = parameters.map { it.name }
}
