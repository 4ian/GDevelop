...
class CapabilityAnalysisTest {
	...
	private val requirement = CapabilityRequirement(
		id, CapabilityVersionRange(2, 3), CapabilityUse.REQUIRED,
		...
		emptyMap(), listOf(
			SceneIr(
				"Main", emptyMap(), listOf(
					EventIr(emptyList(), listOf(ActionIr.HostOperation(operation)), emptyList(), location)
				)
			)
		), "Main"
	)
	...
	@Test
	fun reportsMissingCapabilityAtOperationSource() {
	...
	@Test
	fun rejectsIncompatibleContractAndRecordsProviderVersion() {
	...
	@Test
	fun negotiatesSupportedContractDeterministically() {
