...
class MapTilesLoweringTest {
		...
		ProjectDocument(
			...
			listOf(
				SceneDeclaration(
					"Map",
					emptyList(),
					listOf(EventDeclaration(emptyList(), listOf(action), emptyList(), origin))
				)
			)
		)
	)
	...
	@Test
	fun cameraActionLowersToResolvedHostOperation() {
		val result = lower(OperationDeclaration("MapTiles::SetCamera", listOf("12.5", "48.25", "9", "400"), origin))
		val operation =
			((result.value!!.scenes.single().events.single().actions.single()) as ActionIr.HostOperation).operation
	...
	@Test
	fun malformedCoordinateAndUnknownActionHaveLocatedDiagnostics() {
	...
	@Test
	fun parameterCountIsValidatedBeforeHostDispatch() {
	...
	@Test
	fun latitudeOutsideMapBoundsIsRejectedDuringLowering() {
