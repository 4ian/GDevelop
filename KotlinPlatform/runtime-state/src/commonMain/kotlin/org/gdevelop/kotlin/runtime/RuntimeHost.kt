...
data class HostTraceRecord(
	val sequence: Long,
	val invocationId: HostInvocationId,
	val phase: String,
	val result: HostOperationResult? = null
)
...
interface RuntimeHost {
	...
	val capabilityManifest: CapabilityManifest
		get() = CapabilityManifest(
			CapabilityProviderIdentity("unspecified-target", this::class.simpleName ?: "runtime-host", "unspecified"),
			capabilities
		)
...
abstract class OrderedRuntimeHost : RuntimeHost {
	...
	final override fun initialize() {
		if (!active) {
			active = true; onInitialize()
		}
	}

	final override fun beginFrame(frame: Int, input: HostTransientInput) {
		check(active); onBeginFrame(frame, input)
	}
	...
	final override fun dispose() {
		if (active) {
			pending.clear(); onDispose(); active = false
		}
	}
	...
	protected open fun invoke(invocation: HostInvocation, completion: (HostOperationResult) -> Unit) =
		completion(invoke(invocation))
...
class DeterministicHeadlessHost : OrderedRuntimeHost() {
	...
	override fun evaluate(invocation: HostInvocation) =
		HostOperationResult.Unsupported(invocation.operation.requiredCapabilities)

	override fun invoke(invocation: HostInvocation) =
		HostOperationResult.Unsupported(invocation.operation.requiredCapabilities)
}
