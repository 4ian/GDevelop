/** \file
 *  Game Develop
 *  2008-2013 Florian Rival (Florian.Rival@gmail.com)
 */

#include "GDL/BuiltinExtensions/NetworkExtension.h"

NetworkExtension::NetworkExtension()
{
    DECLARE_THE_EXTENSION("BuiltinNetwork",
                          _("Basic internet features"),
                          _("Builtin extension providing network features."),
                          "Compil Games",
                          "Freeware")
    #if defined(GD_IDE_ONLY)

    DECLARE_ACTION("EnvoiDataNet",
                   _("Send datas to a website"),
                   _("Send datas to a specified web site.\nYou need to set up a .php page on your web site so as to receive this datas.\nEnter here a password, and enter the same in the configuration of your .php page.\nRead the help file to get more informations."),
                   _("Send to _PARAM0_ the following datas : _PARAM2_, _PARAM3_,_PARAM4_,_PARAM5_,_PARAM6_,_PARAM7_"),
                   _("Network"),
                   "res/actions/net24.png",
                   "res/actions/net.png");

        instrInfo.AddParameter("string", _(".php page URL ( Don't forget the protocol http:// ) "), "",false);
        instrInfo.AddParameter("password", _("Password"), "",false);
        instrInfo.AddParameter("string", _("Data 1"), "",false);
        instrInfo.AddParameter("string", _("Data 2"), "",true);
        instrInfo.AddParameter("string", _("Data 3"), "",true);
        instrInfo.AddParameter("string", _("Data 4"), "",true);
        instrInfo.AddParameter("string", _("Data 5"), "",true);
        instrInfo.AddParameter("string", _("Data 6"), "",true);

        instrInfo.cppCallingInformation.SetFunctionName("SendDataToPhpWebPage").SetIncludeFile("GDL/BuiltinExtensions/NetworkTools.h");

    DECLARE_END_ACTION()

    DECLARE_ACTION("DownloadFile",
                   _("Download a file"),
                   _("Download a file from a web site"),
                   _("Download file _PARAM1_ from _PARAM0_ under the name of _PARAM2_"),
                   _("Network"),
                   "res/actions/net24.png",
                   "res/actions/net.png");

        instrInfo.AddParameter("string", _("Host ( For example : http://www.website.com )"), "",false);
        instrInfo.AddParameter("string", _("Path to file ( For example : /folder/file.txt )"), "",false);
        instrInfo.AddParameter("string", _("Save as"), "",false);

        instrInfo.cppCallingInformation.SetFunctionName("DownloadFile").SetIncludeFile("GDL/BuiltinExtensions/NetworkTools.h");

    DECLARE_END_ACTION()
    #endif
}

