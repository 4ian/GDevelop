...
const getFolderStableKey = (objectFolder, isGlobal) => {
  const parts = [];
...
export const folderColors = {
  ...
  set(objectFolder, isGlobal, color) {
    try {
      const key = getFolderStableKey(objectFolder, isGlobal);
      const saved = localStorage.getItem('gdevelop_custom_folder_colors');
      const colors = saved ? JSON.parse(saved) : {};
      colors[key] = color;
      localStorage.setItem('gdevelop_custom_folder_colors', JSON.stringify(colors));
    } catch (e) {
      console.error("Error saving folder color:", e);
    }
  },
  remove(objectFolder, isGlobal) {
    try {
      const key = getFolderStableKey(objectFolder, isGlobal);
      const saved = localStorage.getItem('gdevelop_custom_folder_colors');
      if (saved) {
...
export type ObjectFolderTreeViewItemProps = {|
  ...
  openColorPicker: (objectFolder: gdObjectFolderOrObject, isGlobal: boolean) => void,
|};
