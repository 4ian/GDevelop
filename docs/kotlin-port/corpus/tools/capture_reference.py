...
def main():
 ap=argparse.ArgumentParser(); ap.add_argument('--manifest',required=True); ap.add_argument('--runs',type=int,default=2); ap.add_argument('--verify',action='store_true'); ap.add_argument('--report')
 a=ap.parse_args()
 ...
 mp=Path(a.manifest); root=mp.parent; m=json.loads(mp.read_text())
 rev=m['gdevelopRevision']
 subprocess.run(['git','cat-file','-e',rev+'^{commit}'],check=True,stdout=subprocess.DEVNULL)
 subprocess.run([sys.executable,str(root/'tools/validate_corpus.py'),'--manifest',str(mp)],check=True,stdout=subprocess.DEVNULL)
 results=[]
 with tempfile.TemporaryDirectory(prefix='gdevelop-phase0-') as td:
  ...
  for f in m['fixtures']:
   ...
   for run in range(1,a.runs+1):
    record=dict(source); record['run']=run
    p=out/f'{f["id"]}.run-{run}.json'; p.write_bytes(canonical_bytes(record))
    semantic=dict(record); semantic.pop('run',None)
   ...
   results.append({'fixtureId':f['id'],'projectSha256':f['project']['sha256'],'canonicalTraceSha256':f['trace']['sha256'],'runSemanticSha256':captured})
 report={'schemaVersion':'1.0.0','capturedOn':'2026-08-06','gdevelopRevision':rev,'corpusVersion':m['corpusVersion'],'command':m['captureEnvironment']['command'],'environment':m['captureEnvironment'],'runs':a.runs,'result':'identical','fixtures':results}
