...
-   **build-third-party-asars.py**: Cross-platform Python helper that builds `image-extender.asar`, `ai-game-workbench.storage-open.asar`, `gorest-spritesheet.asar`, and `advanced-tween-editor.asar` from the `thirdParties` submodules. Run it from the repository root with `python scripts/build-third-party-asars.py`.
...
-   **AdvancedTweenEditorElectronSteps.md**: Documents how to refresh `thirdParties/AdvancedTweenEditor` and rebuild the Electron ASAR artifact without starting a localhost server.
