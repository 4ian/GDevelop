...
- `gdevelop_get_event_dsl_reference`: compact native-event DSL reference and examples.
- `gdevelop_validate_events`: compiles and validates an event DSL patch on a temporary project without mutation.
...
- `put_3d_instances`
- `add_scene_events`
- `gdevelop_apply_events`: primary AI event writer using compact DSL, simple patch operations, dry-run, and revision guards.
...
The primary AI event interface is a compact DSL compiled inside the editor:

- Events use `kind` (`standard`, `group`, `comment`, `repeat`, `while`, `for_each`, `for_each_child_variable`, `else`, `link`, or explicitly requested `javascript`) instead of serializer class names.
- Instructions use exact metadata `type` values and named `parameters`. Metadata exposes a normalized `dslName` even when GDevelop only provides a display description.
- Logical conditions use `any`, `all`, and `not`; the compiler emits serializer `subInstructions`.
- Sub-events use `children`; repeat/for-each/while fields use readable DSL names.
- Missing event ids and Group colors are generated by the editor.
- `gdevelop_validate_events` applies the compiled patch to a temporary project and returns diagnostics/rendered events without mutation.
- `gdevelop_apply_events` supports simple patch operations, dry-run, and an `expected_revision` concurrency guard.
- Raw `events_json`, `event_changes`, and file replacement remain available as advanced compatibility paths.
