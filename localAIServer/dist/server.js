...
app.post('/ai-request', async (req, res) => {
    try {
        const { userRequest, gameProjectJson, mode = 'agent', toolsVersion, } = req.body;
...
app.post('/ai-request/:id/action/add-message', async (req, res) => {
    try {
        const aiRequest = aiRequests.get(req.params.id);
        if (!aiRequest) {
            return res.status(404).json({ error: 'AI request not found' });
...
async function processAiRequest(requestId, userRequest, gameProjectJson, mode) {
...
async function continueAiRequest(requestId, gameProjectJson, mode) {
    const aiRequest = aiRequests.get(requestId);
    if (!aiRequest)
        return;
    try {
...
function buildClaudeMessages(output) {
    const messages = [];
    for (const msg of output) {
        if (msg.type === 'message') {
            if (msg.role === 'user') {
                const text = msg.content
                    .filter((c) => c.type === 'user_request')
                    .map((c) => c.text)
                    .join('\n');
                if (text) {
