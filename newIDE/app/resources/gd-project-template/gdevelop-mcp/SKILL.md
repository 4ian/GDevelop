...
- `render_scene_to_png`: STATIC layout render — draws one colored, positioned box per initial instance (sized to the object/instance, colored stably per object name) to a PNG WITHOUT running the game. This is the reliable way to verify object placement and layout when a live preview is unavailable (throttled window, or before you've launched anything). It is schematic, not pixel-accurate art; for final visuals use `capture_preview_screenshot`.
