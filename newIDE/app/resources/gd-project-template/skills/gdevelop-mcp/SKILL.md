...
Global Config syntax: config paths are not variables. In scene events, use the read-only `Global configuration` conditions and expressions with exact placeholder paths, for example `ConfigNumber("{{cards.Sunflower.price}}")`, `ConfigString("{{cards.Sunflower.name}}")`, and `ConfigBool("{{cards.Sunflower.canUse}}")`. There are no runtime Global Config actions for setting or deleting config values; use scene/global/object variables for mutable gameplay state. Do not write `GlobalVariable(cards.Sunflower.price)` for config data. Read/edit the project config map at design time with `gdevelop_get_global_config`, `gdevelop_set_global_config_value`, and `gdevelop_delete_global_config_value`; the path argument must include the outer `{{...}}`.
