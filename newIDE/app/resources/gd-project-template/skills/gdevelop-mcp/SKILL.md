...
- `gdevelop_inspect_signal_usage`: inspect signal emit actions, scene-level `Signal received` conditions, and object `onSignal` lifecycle handlers across the project. Pass `signal_name` to filter emit/receive sites; `onSignal` handlers are listed by receiver because they can branch on `SignalName` at runtime.
