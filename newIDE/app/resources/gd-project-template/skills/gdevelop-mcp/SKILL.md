...
- `gdevelop_inspect_signal_usage`: inspect signal emit actions, scene-level `Signal received` conditions, and object/behavior `onSignal` lifecycle handlers across the project. Pass `signal_name` to filter emit/receive sites; `onSignal` handlers are listed by receiver because they can branch on `SignalName` at runtime.
...
- `create_signal_emit_action` / `create_signal_received_condition`: build signal instruction JSON without hand-aligning target-specific parameter order. `create_signal_emit_action` supports `target_kind:"scene" | "object" | "object_instance" | "picked_objects" | "object_group" | "behavior"`, quotes `signal_name`, treats `payload` as a string expression, and accepts optional `emitter_object`. For object-instance sends, pass `object_name` and `instance_id` (for example `Object.UniqueId()` or `SignalSenderInstanceId()`).
...
- `gdevelop_create_or_update_on_signal`: create/update the reserved object/behavior `onSignal` lifecycle function. Use this instead of `gdevelop_create_or_update_extension_function` for signal receivers. Pass `extension_name`, `parent_kind:"object"` or `"behavior"`, `parent_name`, optional `events_json`, `dry_run`, and `summary_only`. Do not supply custom parameters: GDevelop maintains the fixed signature (`Object`, optional `Behavior`, `SignalName`, `Payload`, `EmitterObjectName`, `EmitterInstanceId`).
