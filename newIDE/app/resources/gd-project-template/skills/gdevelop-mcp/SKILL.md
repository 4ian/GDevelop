...
Global Config syntax: config paths are not variables. In scene events, use the read-only `Global configuration` conditions and expressions with exact placeholder paths, for example `ConfigNumber("{{cards.Sunflower.price}}")`, `ConfigString("{{cards.Sunflower.name}}")`, and `ConfigBool("{{cards.Sunflower.canUse}}")`. A `$name` token inside a placeholder path segment reads global variable `name`, so `{{i18n.button1.label_$locale}}` can resolve to `i18n.button1.label_en` when global variable `locale` is `en`. There are no runtime Global Config actions for setting or deleting config values; use scene/global/object variables for mutable gameplay state. Do not write `GlobalVariable(cards.Sunflower.price)` for config data. Read/edit the project config map at design time with `gdevelop_get_global_config`, `gdevelop_set_global_config_value`, and `gdevelop_delete_global_config_value`; the path argument must include the outer `{{...}}`.
...
- Use Global Config for read-only data tables shared by object properties and events. Runtime events can read it with Global Config conditions/expressions, but cannot set or remove values. Edit project config at design time with `gdevelop_get_global_config` and focused `gdevelop_set_global_config_value` / `gdevelop_delete_global_config_value` instead of raw project patches. Paths must be exact placeholders like `{{cards.Sunflower.price}}`.
...
- Treating Global Config paths as variables. Fix: use read-only Global Config expressions/conditions with exact `{{cards.Sunflower.price}}` placeholder paths, and edit project config data through `gdevelop_set_global_config_value`.
