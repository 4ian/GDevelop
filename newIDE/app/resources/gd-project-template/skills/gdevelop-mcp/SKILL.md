...
- `gdevelop_inspect_signal_usage`: inspect signal emit actions, scene/external-scene `Signal received` conditions, and object `onSignal` lifecycle handlers across the project. Pass `signal_name` to filter emit/receive sites; `onSignal` handlers are listed by receiver because they branch on their fixed `SignalName` parameter at runtime.
...
- `create_signal_emit_action` / `create_signal_received_condition`: build signal instruction JSON without hand-aligning target-specific parameter order. `create_signal_emit_action` supports `target_kind:"scene" | "object" | "object_instance" | "picked_objects" | "object_group"` in scene events, but extension/object/behavior function events should use only `target_kind:"scene"` or `"object_instance"` so extensions stay decoupled from scene object and group names. Pass `target_scope:"extension_function" | "object_function" | "behavior_function" | "async_function"` to enforce this guard. It quotes `signal_name`, treats `payload` as a string expression, and does not take an explicit emitter argument. The runtime automatically uses the owner object as sender in object/prefab functions. Scene and external scene events emit from the scene, including scene `Signal received` dispatcher/forwarding events. For object-instance sends, pass only `instance_id` (for example `Object.InstanceId()` or `SignalSenderInstanceId()`). `create_signal_received_condition` is only for scene and external scene event sheets; do not use it in prefab/object/behavior function events.
...
- `gdevelop_create_or_update_on_signal`: create/update the reserved object `onSignal` lifecycle function. Use this instead of `gdevelop_create_or_update_extension_function` for prefab signal receivers. Pass `extension_name`, `parent_kind:"object"`, `parent_name`, optional `events_json`, `dry_run`, and `summary_only`. Do not supply custom parameters: GDevelop maintains the fixed signature (`Object`, `SignalName`, `Payload`, `EmitterObjectName`, `EmitterInstanceId`). Inside `onSignal`, read those fixed parameters directly; the `SignalName()`, `SignalPayload()`, `SignalSenderObjectName()`, and `SignalSenderInstanceId()` expressions are scene/external-scene only.
...
Global Config syntax: config paths are not variables. In scene/external events, use the read-only `Global configuration` conditions and expressions with exact placeholder paths, for example `ConfigNumber("{{cards.Sunflower.price}}")`, `ConfigString("{{cards.Sunflower.name}}")`, and `ConfigBool("{{cards.Sunflower.canUse}}")`. Do not use these direct Global Config conditions/expressions inside extension function, event-based object, or event-based behavior event sheets; inject config through parameters/properties instead, preferably a JSON-object property like `CardConfig` with an object-editor value such as `{{cards.Sunflower}}`, then read `CardConfig.price` in the extension events. A `$name` token inside a placeholder path segment reads global variable `name`, so `{{i18n.button1.label_$locale}}` can resolve to `i18n.button1.label_en` when global variable `locale` is `en`. There are no runtime Global Config actions for setting or deleting config values; use scene/global/object variables for mutable gameplay state. Do not write `GlobalVariable(cards.Sunflower.price)` for config data. Read/edit the project config map at design time with `gdevelop_get_global_config`, `gdevelop_set_global_config_value`, and `gdevelop_delete_global_config_value`; the path argument must include the outer `{{...}}`.
...
- Use Global Config for read-only data tables shared by object properties and scene/external events. Scene/external events can read it with Global Config conditions/expressions, but cannot set or remove values. Extension events must not use direct Global Config conditions/expressions; pass config through parameters/properties, preferably JSON-object properties. Edit project config at design time with `gdevelop_get_global_config` and focused `gdevelop_set_global_config_value` / `gdevelop_delete_global_config_value` instead of raw project patches. Paths must be exact placeholders like `{{cards.Sunflower.price}}`.
...
- Treating Global Config paths as variables. Fix: in scene/external events, use read-only Global Config expressions/conditions with exact `{{cards.Sunflower.price}}` placeholder paths; in extension events, inject config through parameters/properties instead. Edit project config data through `gdevelop_set_global_config_value`.
