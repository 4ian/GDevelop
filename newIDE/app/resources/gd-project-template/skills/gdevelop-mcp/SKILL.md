...
- `create_signal_emit_action` / `create_signal_received_condition`: build signal instruction JSON without hand-aligning target-specific parameter order. `create_signal_emit_action` supports `target_kind:"scene" | "object" | "object_instance" | "picked_objects" | "object_group"`, quotes `signal_name`, treats `payload` as a string expression, and requires `emitter_object`. For object-instance sends, pass only `instance_id` (for example `Object.InstanceId()` or `SignalSenderInstanceId()`). `create_signal_received_condition` is only for scene and external scene event sheets; do not use it in prefab/object/behavior function events.
