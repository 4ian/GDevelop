# Use GDevelop Constants and placeholders

Constants is project-wide, TOML-compatible, static authoring data. Use it
...
1. [Choose Constants or variables](#choose-constants-or-variables)
2. [Multi-file source ownership](#multi-file-source-ownership)
3. [Author constants.toml correctly](#author-constantstoml-correctly)
...
Changing Constants does not mutate a running preview. Its placeholders are
resolved while code and runtime object data are generated, so regenerate or
relaunch the preview/export after every relevant constants change.
...
In a canonical multi-file project, `constants.toml` is the sole source for the
complete Constants object. It is discovered by its fixed root path; do not
add a reference to it in `project.gdevelop`.

Ownership is strict:

- The whole TOML document is user configuration data.
- There is no `[settings]` or `[constants]` wrapper and no format metadata.
- `project.gdevelop` must not contain a constants table.

`constants.toml` is required. Empty Constants is an empty file.
...
same source; neither path adds Constants to `project.gdevelop`.

Do not edit `.gdevelop/game.json`; it is generated output and does not contain
Constants. Runtime exports also omit the map after resolving supported
placeholders.

## Author constants.toml correctly

The Constants root is the TOML document itself. Strings, finite numbers,
...
`constants.toml` has no hidden fallback or reserved metadata namespace. Values
...
The runtime game does not load the Constants map and there is no runtime
Constants event-tool API. Once generation succeeds, the game contains the
...
code substitution that is not supported, and the third is not a constants path.
...
Constants replacement for both actions and conditions. String-expression
...
Constants subtree. Do not duplicate the subtree as leaf-by-leaf variable
descriptors.

The canonical localization shape for the Constants window is row-oriented:
...
column is a locale, without an extra generic `value` column. In the Constants
...
the Constants window unnecessarily difficult.
...
- Prefer one exact subtree placeholder such as `{{localization}}` when the
  Constant value is an object or array. The result is compact JSON text.
...
- The runtime variable is ordinary mutable data after initialization or JSON
  conversion. Changing it does not change Constants, and changing Constants
...
```text
project constants: cards.Sunflower
...
JSON example, and avoids coupling extension source to one project's constants
paths.
...
reference Constants directly in its initial `value`; use an initialization
...
This value cannot be stored in `constants.toml`. Omit `release.label` or replace it
with an explicit TOML-compatible status, and model `spawnPattern` as a table or
an array whose elements share one TOML type.

## Edit rules

- Read the whole current `constants.toml` before modifying one subtree.
- Edit direct-root data only; do not add metadata or wrapper tables.
- Do not duplicate constants in `project.gdevelop`.
- Do not add a manifest/index entry for `constants.toml`.
- Never use an MCP Constants tool; none exists. Modify `constants.toml`
  directly.
- Keep `constants.toml` present even when Constants is empty; represent that
  state with an empty file.
- Search every `{{...}}` reference before renaming or deleting a key. Update
  constants and all consumers atomically.

## Validate and debug

After editing:

1. Parse `constants.toml` as a standalone root TOML document.
2. Confirm every value is directly TOML-compatible and every number is finite
   and within the safe integer range.
3. Confirm there are no `[settings]`, `[constants]`, format-version, or
   serializer wrappers.
...
7. Launch a fresh preview/export generation. Missing placeholders appear in
   diagnostics under the message family “A project constant”
...
diagnostic remains visible. Fix the constants path or consumer; do not hide the
error with a fabricated value unless that default is part of the user's design.

## Failure patterns

- Treating Constants as runtime mutable state.
- Editing generated `.gdevelop/game.json` instead of `constants.toml`.
- Putting constants in `project.gdevelop`.
- Adding `[settings]`, `[constants]`, or serializer metadata wrappers.
...
- Embedding direct project constants paths in an extension intended for export.
...
- Decide explicitly why the value belongs in Constants rather than runtime state.
- Read the complete `constants.toml` and relevant owner `.settings`/events.
...
- Inject constants through properties/parameters for reusable extensions.
- Update all consumers atomically when paths change.
- Reload and regenerate a fresh preview/export, then resolve all missing-constants
  diagnostics.
