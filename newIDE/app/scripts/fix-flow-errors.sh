...
# Convert React.AbstractComponent<Props, Instance> -> React.ComponentType<Props>
# React.AbstractComponent does not exist in modern Flow (0.299+).
# This handles pre-existing occurrences in the source and any produced by
# the React$AbstractComponent -> React.AbstractComponent sed above.
python3 << 'PYEOF'
import os, re
...
def split_top_level_comma(s):
    """Split string by top-level commas (respecting nested <>, {}, (), [])."""
    parts = []
    depth = {'<': 0, '{': 0, '(': 0, '[': 0}
    openers = {'<': '<', '{': '{', '(': '(', '[': '['}
    closers = {'>': '<', '}': '{', ')': '(', ']': '['}
...
def replace_abstract_component(content):
    """Replace React.AbstractComponent<Props, Instance> with React.ComponentType<Props>."""
    marker = 'React.AbstractComponent'
    result = []
    i = 0
    while i < len(content):
        ...
        if not stripped_rest.startswith('<'):
            # No type params, just replace name
            result.append('React.ComponentType')
            i = after_marker
            continue
        # Find the opening <
        angle_start = content.index('<', after_marker)
        ...
        if depth != 0:
            ...
            i = after_marker
            continue
        angle_end = j  # position of closing >
        type_params_str = content[angle_start + 1:angle_end]
        params = split_top_level_comma(type_params_str)
        if len(params) >= 2:
            # Drop second (and further) params, keep only first
            first_param = params[0].rstrip()
            # Preserve trailing whitespace/newline style
            result.append(f'React.ComponentType<{first_param}\n>')
            # Check if the first param already ends with newline for formatting
            if first_param.endswith('\n') or '\n' in first_param:
                result[-1] = f'React.ComponentType<{first_param}>'
            else:
                result[-1] = f'React.ComponentType<{first_param}>'
        elif len(params) == 1:
            result.append(f'React.ComponentType<{params[0]}>')
        else:
            result.append(f'React.ComponentType<{type_params_str}>')
        i = angle_end + 1
    return ''.join(result)

def remove_flowfixme_prop_missing_for_abstract(content):
    """Remove // $FlowFixMe[prop-missing] lines that were added for React.AbstractComponent."""
    lines = content.split('\n')
    result = []
    i = 0
    while i < len(lines):
        stripped = lines[i].strip()
        # Check for both // and {/* styles of FlowFixMe[prop-missing]
        is_flowfixme_prop_missing = (
            stripped == '// $FlowFixMe[prop-missing]' or
            stripped == '{/* $FlowFixMe[prop-missing] */}'
        )
        if is_flowfixme_prop_missing:
            # Look ahead to see if nearby lines contain React.AbstractComponent
            should_remove = False
            for j in range(i + 1, min(i + 5, len(lines))):
                if 'React.AbstractComponent' in lines[j]:
                    should_remove = True
                    break
...
for root, dirs, files in os.walk(src_dir):
    for fname in files:
        ...
        with open(filepath) as f:
            content = f.read()
        original = content
        if 'React.AbstractComponent' in content:
            # First remove FlowFixMe[prop-missing] that was added for AbstractComponent
            content = remove_flowfixme_prop_missing_for_abstract(content)
            # Then replace AbstractComponent with ComponentType
            content = replace_abstract_component(content)
        if content != original:
            with open(filepath, 'w') as f:
                f.write(content)
            fixed += 1

print(f"  Converted {fixed} files from React.AbstractComponent to React.ComponentType")
PYEOF
...
# Second pass: convert any React.AbstractComponent generated by the codemod
# or remaining from previous runs. Same logic as in Step 3.
echo "  Converting any remaining React.AbstractComponent to React.ComponentType..."
python3 << 'PYEOF'
import os, re
...
print(f"  Post-codemod: converted {fixed} files from React.AbstractComponent to React.ComponentType")
PYEOF
