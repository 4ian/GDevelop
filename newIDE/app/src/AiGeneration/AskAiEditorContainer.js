...
const useProcessFunctionCalls = ({
  ...
  onSendEditorFunctionCallResults: (
    options: null | {| createdScenes: Array<string> |}
  ) => Promise<void>,
...
|}) => {
  ...
  const onProcessFunctionCalls = React.useCallback(
    async (
      ...
      const { results, createdScenes } = await processEditorFunctionCalls({
        project,
        editorCallbacks,
    ...
    (
      ...
      const onSendMessage = React.useCallback(
        ...
        |}) => {
          ...
          if (selectedAiRequest && createdScenes) {
            onOpenAskAi({
              mode: selectedAiRequest.mode || 'agent',
              aiRequestId: selectedAiRequestId,
              paneIdentifier: 'right',
            });
            createdScenes.forEach(sceneName => {
      ...
      const onSendEditorFunctionCallResults = React.useCallback(
        (options: null | {| createdScenes: Array<string> |}) =>
          onSendMessage({
            userMessage: '',
            createdScenes: options ? options.createdScenes : [],
          }),
        [onSendMessage]
      );
