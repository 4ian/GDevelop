 ...
 *
 * The JSON decompiler deliberately emits catalog-backed `@exact`
 * instructions. This makes JSON -> DSL -> JSON independent of a loaded
 * project. Friendly instructions are accepted by the compiler when a caller
 * supplies `resolveInstruction` from the loaded project catalog.
 */
...
const formatInstructionLines = (
  ...
  lines.push(
    `${prefix}${
      instructionDepth === 0 ? (kind === 'condition' ? 'if ' : 'do ') : ''
    }${instructionText}`
  );
