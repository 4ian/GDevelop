...
const getObjectContent = object => {
  if (!object || object.getType() !== 'Spine43Object::Spine43Object') {
    return null;
  }

  try {
    return gd.asObjectJsImplementation(object.getConfiguration()).content || {};
  } catch (error) {
    return null;
  }
};

const resolveSkeletonUrl = (project, skeletonResource) => {
...
const loadSkeletonJson = async (project, skeletonResource) => {
...
const getAnimationNames = skeletonJson => {
  const animations = skeletonJson && skeletonJson.animations;
  if (Array.isArray(animations)) {
    return animations
      .map(animation => animation && animation.name)
      .filter(Boolean);
  }
...
const getSkinNames = skeletonJson => {
  const skins = skeletonJson && skeletonJson.skins;
  if (Array.isArray(skins)) {
    return skins.map(skin => skin && skin.name).filter(Boolean);
  }
...
const getSlotNames = skeletonJson => {
  const slots = skeletonJson && skeletonJson.slots;
  if (!Array.isArray(slots)) return [];
  return slots.map(slot => slot && slot.name).filter(Boolean);
};

const getSkinAttachments = skin => {
  if (!skin) return null;
  if (skin.attachments) return skin.attachments;
  return skin;
};

const getAttachmentNames = (skeletonJson, slotName: ?string) => {
  const targetSlotName = String(slotName || '').trim();
  if (!targetSlotName) return [];

  const skins = skeletonJson && skeletonJson.skins;
  const names = new Set();
  const collectFromSkin = skin => {
...
const getNamesForKind = (skeletonJson, kind, slotName) => {
