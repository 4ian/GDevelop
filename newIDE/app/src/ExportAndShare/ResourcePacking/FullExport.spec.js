...
describe('Full HTML5 export with packed resources', () => {
  ...
  it('packs the exported resources and keeps the game readable', async () => {
    ...
    const packNames = filesAfterPacking.filter(name => name.endsWith('.gdpak'));
    ...
    expect(await readPack('resources.gdpak')).toEqual(['global.png']);
    expect(await readPack('scene-0.gdpak')).toEqual(['menu.png']);
    expect(await readPack('scene-1.gdpak')).toEqual(['level.png']);
  });
