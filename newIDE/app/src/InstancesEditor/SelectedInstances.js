...
export default class SelectedInstances {
  ...
  /**
   * When using moving the view with Space key pressed, a click on the buttons
   * generated by this class won't trigger a resize and interrupt the move of the view.
   * On the opposite, when resizing an instance, if the Space key is pressed,
   * the resizing is interrupted to move the view.
   * This boolean is used to detect the change and call the onResizeEnd action.
   * Otherwise, the InstancesResizer stays in an intermediate state.
   */
  _previousPanMovingGoal: null | 'resizing' | 'viewMoving' = null;
  ...
  _makeButton({
    ...
    objectButton.addEventListener('panmove', event => {
      if (this.keyboardShortcuts.shouldMoveView()) {
        if (this._previousPanMovingGoal === 'resizing') {
          onEnd();
        }
        onPanMove(event);
        this._previousPanMovingGoal = 'viewMoving';
      } else {
        onMove(event);
        this._previousPanMovingGoal = 'resizing';
      }
    });
    ...
    objectButton.addEventListener('panend', event => {
      ...
      this._previousPanMovingGoal = null;
    });
