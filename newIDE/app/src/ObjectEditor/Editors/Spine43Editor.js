...
const normalizePreviewColor = (color: any) => ({
...
const getMeshPreviewColor = (content: any) =>
...
const getContent = (objectConfiguration: gdObjectConfiguration): Object => {
  try {
    return gd.asObjectJsImplementation(objectConfiguration).content || {};
  } catch (error) {
    return {};
  }
};

const resolveResourceUrl = (project, resourceNameOrFile) => {
  ...
  if (resourcesManager && resourcesManager.hasResource(value)) {
    ...
    if (resource && resource.getFile) file = resource.getFile() || value;
  }
...
const getSlotAttachments = (skeletonJson, slotName) => {
  const attachments = [];
...
const getAllAttachments = skeletonJson =>
  getSlots(skeletonJson).reduce(
    (all, slot) => all.concat(getSlotAttachments(skeletonJson, slot.name)),
    []
  );
...
const parseColorString = color => {
  ...
  const parse = index => parseInt(normalized.substr(index, 2), 16);
  return {
...
const dialogRuntimeScriptPromises = {};

const getSpine43Adapter = () =>
...
const ensureDialogRuntime = async () => {
  ensurePixi();
...
const getRuntimeSlotAttachment = slot => {
  if (!slot) return null;
...
const computeRuntimeWorldVertices = (attachment, slot, skeleton) => {
  if (
    !attachment ||
    !slot ||
...
const getRuntimeTriangleIndices = (attachment, vertices) => {
...
const getBonePose = bone => {
  if (!bone) return null;
  return bone.appliedPose || bone.pose || bone;
};

const getBoneWorldPoint = bone => {
...
const findRuntimeBone = (skeleton, name) => {
...
const getRuntimeBoneParent = (skeleton, bone) => {
  if (!bone) return null;
...
const worldDeltaToBoneLocal = (parentBone, dx, dy) => {
...
const getRuntimeBoneSegment = bone => {
...
const getPointToSegmentDistance = (px, py, ax, ay, bx, by) => {
  const abX = bx - ax;
  const abY = by - ay;
...
const findRuntimeBoneAtPoint = (skeleton, x, y, threshold) => {
...
const drawRuntimeConstraints = (graphics, skeleton) => {
  const constraints =
...
const fitRuntimePreview = (app, contentLayer, spineContainer) => {
  if (
    !app ||
    !contentLayer ||
    !spineContainer ||
...
const Preview = ({
  ...
  React.useEffect(
    () => {
      ...
      const waitForUsablePreviewRect = () =>
        new Promise(resolve => {
          let attempts = 0;
          const read = () => {
      ...
      const getPointerStagePoint = event => {
      ...
      const getPointerLocalPoint = event => {
      ...
      const updateHoveredBone = boneName => {
...
const getBoneValue = (bone, override, field, fallback) =>
...
const MeshPanel = ({
  ...
  const updateMeshPreview = changes => {
    const next = clone(draft);
    next.meshPreview = {
      ...(next.meshPreview || {}),
      ...changes,
...
const SlotsPanel = ({ skeletonJson, draft, onDraftChange }) => {
  const slots = getSlots(skeletonJson);
  const overrides = draft.inspectorOverrides || emptyInspectorOverrides();
...
const describeTimeline = timeline => {
  ...
  if (Array.isArray(timeline)) {
    ...
    if (firstTime !== null && lastTime !== null) {
      details.push(`时间：${firstTime} - ${lastTime}s`);
    }
...
const formatTimelinePath = path =>
  path.length ? path.map(formatTimelineSegment).join(' / ') : 'Timeline';

const collectTimelineEntries = (value, path = []) => {
...
const getAnimationData = (skeletonJson, animationName) => {
...
const AnimationPanel = ({ skeletonJson, draft, onDraftChange }) => {
...
const createDefaultPreviewOptions = mode => ({
  showImage: true,
...
const getBoneOverridesSnapshot = content =>
  clone(
...
const applyBoneOverrides = (content, boneOverrides) => {
...
|}) => {
  ...
  const [runtimeResetRequest, setRuntimeResetRequest] = React.useState(null);
  const draftRef = React.useRef(draft);
  const boneUndoStackRef = React.useRef([]);
  const pendingBoneChangesRef = React.useRef({});
  ...
  const requestRuntimeReset = React.useCallback((type, boneName) => {
  ...
  const pushBoneUndoSnapshot = React.useCallback(snapshotOverride => {
  ...
  const setBoneOverrides = React.useCallback(boneOverrides => {
  ...
  const updateMeshPreviewColor = React.useCallback(colorResult => {
  ...
  const updateBoneOverride = (boneName, changes, options) => {
    if (!boneName) return;
  ...
  const resetSelectedBone = boneName => {
    if (!boneName) return;
