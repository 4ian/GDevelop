...
export default (React.forwardRef<Props, LeaderboardIdPropertyFieldInterface>(
  function LeaderboardIdPropertyField(props, ref) {
    ...
    const isCurrentValueInLeaderboardList = leaderboards &&
    ...
    const selectOptions = React.useMemo(
      () => leaderboards && gameHasLeaderboards
        ? leaderboards.map(
          leaderboard => <SelectOption
            key={leaderboard.id}
            value={leaderboard.id}
            label={`${leaderboard.name} ${leaderboard.id
        ...
        {({i18n}) => <>
          <TextFieldWithButtonLayout
            renderTextField={() => !isExpressionField
              ? <SelectField
                ...
                translatableHintText={gameHasLeaderboards
                  ? t`Choose a leaderboard`
                  : t`No leaderboards`}
                helperMarkdownText={!gameHasLeaderboards
                  ? i18n._(
              ...
              : <SemiControlledTextField
                ...
                errorText={leaderboards
                  ? null
                  : isOnline
                    ? <Trans>
                      Your game may not be registered, create one in the leaderboard manager.
                    </Trans>
                    : <Trans>
                      Unable to fetch leaderboards as you are offline.
                    </Trans>}
              />}
            renderButton={style => <FlatButtonWithSplitMenu
          ...
          {isAdminOpen && !!props.project &&
            <LeaderboardDialog
              ...
              leaderboardId={isCurrentValueInLeaderboardList
                ? props.value
                : undefined}
            />}
        </>}
...
): React.AbstractComponent<{ ...Props, +ref?: React.RefSetter<LeaderboardIdPropertyFieldInterface> }, React.RefSetter<LeaderboardIdPropertyFieldInterface>>);
