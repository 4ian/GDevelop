...
import { truncTo } from '../Utils/Mathematics';
...
const getFieldValue = ({
  ...
  return typeof value === 'number' && typeof truncDecimals === 'number'
    ? truncTo(value, truncDecimals)
    : value;
};
  ...
  const renderInputField = React.useCallback(
    (field: ValueField) => {
      ...
      if (field.valueType === 'boolean') {
        ...
        const { setValue, truncDecimals } = field;
        return (
          <SemiControlledTextField
            value={getFieldValue({ instances, field, truncDecimals })}
            key={field.name}
            id={field.name}
            ...
            onChange={newValue => {
              ...
              cleanedValue = truncDecimals
                ? truncTo(cleanedValue, truncDecimals)
                : cleanedValue;
