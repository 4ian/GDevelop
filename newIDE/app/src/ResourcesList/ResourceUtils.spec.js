...
describe('ResourceUtils', () => {
  it('can rename a resource in the whole project', () => {
    ...
    expect(project.getResourcesManager().hasResource('fake-image1.png')).toBe(false);
    expect(project.getResourcesManager().hasResource('Image1')).toBe(true);
    expect(project.getResourcesManager().hasResource('fake-audio1.mp3')).toBe(false);
    expect(project.getResourcesManager().hasResource('Audio1')).toBe(true);

    // Verify files have not changed:
    expect(project.getResourcesManager().getResource('Image1').getFile()).toBe("fake-image1.png");
    expect(project.getResourcesManager().getResource('Audio1').getFile()).toBe("fake-audio1.mp3");
