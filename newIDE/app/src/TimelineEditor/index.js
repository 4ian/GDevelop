...
const evaluateTimelineCurve = (
  ...
  const curveObject = asTimelineCurveObject(curve);
  if (curveObject) {
    if (curveObject.type === 'preset') {
      return evaluateTimelineCurve(curveObject.name, t);
    }
    if (curveObject.type === 'steps') {
      const steps = Math.max(
        1,
        Math.floor(getFiniteNumber(curveObject.steps, 1))
      );
      return curveObject.position === 'start'
        ? Math.min(1, Math.ceil(t * steps) / steps)
        : Math.floor(t * steps) / steps;
    }
    if (curveObject.type === 'cubicBezier') {
      return evaluateCubicBezier(
        getFiniteNumber(curveObject.x1, 0),
        getFiniteNumber(curveObject.y1, 0),
        getFiniteNumber(curveObject.x2, 1),
        getFiniteNumber(curveObject.y2, 1),
        t
      );
    }
  }
...
const getCurvePresetId = (curve: TimelineCurveDefinition | null): ?string => {
  ...
  if (curveObject) {
    if (curveObject.type === 'preset')
      return getCurvePresetId(curveObject.name);
    if (curveObject.type === 'steps') return 'hold';
    if (curveObject.type === 'cubicBezier') {
      return getCurvePresetId([
        curveObject.x1,
        curveObject.y1,
        curveObject.x2,
        curveObject.y2,
...
const getCurveModeId = (curve: TimelineCurveDefinition | null): ?string => {
  ...
  const curveObject = asTimelineCurveObject(curve);
  if (curveObject) {
    if (curveObject.type === 'steps') return 'stepped';
    if (curveObject.type === 'cubicBezier') return 'bezier';
  }
  return null;
};
...
const normalizeTimelineNumberValueForProperty = (
  property: string,
  value: any,
...
const buildPropertyTrackGraphPaths = (
  ...
  if (keyframes.length < 2) {
    return ([]: Array<{| channel: string, color: string, path: string |}>);
  }

  const channels = getTimelineValueChannels(keyframes[0].value);
  const paths: Array<{| channel: string, color: string, path: string |}> = [];
  channels.forEach(({ channel, color }) => {
    ...
    for (let index = 0; index < keyframes.length; index++) {
      ...
      commands.push(
        `${commands.length ? 'L' : 'M'}${point.x.toFixed(2)} ${point.y.toFixed(
          2
        )}`
      );
    }
    ...
    if (commands.length) {
      paths.push({ channel, color, path: commands.join(' ') });
    }
  });
  return paths;
};
...
const getCubicBezierControlPoints = (
  ...
  const curveObject = asTimelineCurveObject(curve);
  if (curveObject && curveObject.type === 'cubicBezier') {
    return {
      x1: clamp01(getFiniteNumber(curveObject.x1, 0.42)),
      y1: clamp01(getFiniteNumber(curveObject.y1, 0)),
      x2: clamp01(getFiniteNumber(curveObject.x2, 0.58)),
      y2: clamp01(getFiniteNumber(curveObject.y2, 1)),
    };
  }
...
|} => {
  ...
  for (const track of tracks) {
    ...
    let normalizedTrack: TimelineTrack = track;
    let didNormalizePropertyTracks = false;
    const normalizedPropertyTracks: Array<TimelinePropertyTrack> = [];
...
const getTrackPropertyValueFromInstance = (
  ...
  switch (property) {
    ...
    case 'opacity':
      return normalizeTimelineValueForProperty(
        property,
        typeof instanceAsAny.getOpacity === 'function'
          ? instanceAsAny.getOpacity()
          : 255
      );
    case 'width':
...
const getPropertyValueFromInitialInstance = (
  ...
  if (
    ...
    typeof instanceAsAny.getRawDoubleProperty === 'function'
  ) {
    return normalizeTimelineValueForProperty(
      'animationIndex',
      instanceAsAny.getRawDoubleProperty('animation')
    );
  }
  if (
    ...
    return normalizeTimelineValueForProperty(
      'animationFrame',
      instanceAsAny.getRawDoubleProperty('animationFrame')
    );
  }
...
const applyValueToInitialInstance = (
  ...
  if (propertyName === 'position' && typeof value === 'object') {
    ...
    instanceAsAny.setOpacity(
      normalizeTimelineNumberValueForProperty('opacity', value)
    );
  } else if (
    propertyName === 'animationIndex' &&
    typeof value === 'number' &&
    typeof instanceAsAny.setRawDoubleProperty === 'function'
  ) {
    instanceAsAny.setRawDoubleProperty(
      'animation',
      normalizeTimelineNumberValueForProperty('animationIndex', value)
    );
  } else if (
    propertyName === 'animationFrame' &&
    typeof value === 'number' &&
    ...
    instanceAsAny.setRawDoubleProperty(
      'animationFrame',
      normalizeTimelineNumberValueForProperty('animationFrame', value)
    );
  }
};
...
export default function TimelineEditor({
  ...
  React.useEffect(
    () => {
      ...
      const tick = (frameTime: number) => {
        ...
        const deltaTime =
          typeof previousFrameTime === 'number'
            ? Math.min((frameTime - previousFrameTime) / 1000, 0.1) * speed
            : 1 / frameRate;
  ...
  const curveEditableKeyframes = React.useMemo(
    () => {
      ...
      if (!selectedIds.size) return ([]: Array<TimelineKeyframe>);
  ...
  const onScrollRailMouseDown = React.useCallback(
    (event: SyntheticMouseEvent<HTMLDivElement>) => {
      ...
      if (event.button !== 0 || !scrollRail) return;
  ...
  const onScrollThumbMouseDown = React.useCallback(
    (event: SyntheticMouseEvent<HTMLDivElement>) => {
      ...
      scrollThumbDragStateRef.current = {
        startClientX: event.clientX,
        startFrame: safeViewStartFrame,
        railWidth: scrollRail.getBoundingClientRect().width,
      };
  ...
  const beginKeyframeDrag = React.useCallback(
    (
      ...
      const snapshots: Array<KeyframeDragSnapshot> = [];
      timeline.tracks.forEach(track => {
      ...
      const onMouseMove = (event: any) => {
        ...
        const nextTimeline = {
          ...
          tracks: dragState.timeline.tracks.map(track => ({
            ...track,
            propertyTracks: track.propertyTracks.map(propertyTrack => {
              const getDraggedValueDelta = (
                snapshot: KeyframeDragSnapshot
              ): number =>
    ...
    () => {
      ...
      const onMouseUp = (event: any) => {
        ...
        if (marqueeState) {
          ...
          const getSelectedIdsForPropertyTrack = (
            propertyTrack: TimelinePropertyTrack
          ): Array<string> =>
          ...
          let selectionPropertyTrackId: string = marqueeState.propertyTrackId;
          ...
          if (marqueeState.multiTrack) {
            ...
            let firstSelectedRowPropertyTrackId: ?string = null;
