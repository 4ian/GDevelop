...
export default React.memo<Props>(
  function CloseConfirmDialog(
    {shouldPrompt, i18n, language, hasUnsavedChanges}: Props,
  ) {
    ...
    React.useEffect(
      () => {
        if (Window.isDev())
          return; // Don't prevent live-reload in development
        ...
        const unsavedChangesMessage = hasUnsavedChanges
          ...
          : '';
        const message = [quitMessage, unsavedChangesMessage].filter(Boolean).join(
          '\n',
        );
