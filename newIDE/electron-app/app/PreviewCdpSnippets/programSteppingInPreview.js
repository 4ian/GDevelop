/**
 * Programs the stepping FSM so the next `__checkBreakpoint` trips on the
 * target event, then the caller issues `Debugger.resume`.
 *
 * V8 pauses on the `debugger;` that sits *after* `__checkBreakpoint(N)`,
 ...
 * @returns {boolean} `true` if applied, `false` if `gdjs.game` isn't available yet.
 */
