...
export default class SelectionRectangle {
  ...
  render() {
    ...
    const sceneEndPoint = this.viewPosition.toSceneCoordinates(this.viewPosition.getWidth(), this.viewPosition.getHeight());

    const startXPos = Math.floor(sceneStartPoint[0] / options.gridWidth) * options.gridWidth;
    const startYPos = Math.floor(sceneStartPoint[1] / options.gridHeight) * options.gridHeight;

    const endXPos = Math.ceil(sceneEndPoint[0] / options.gridWidth) * options.gridWidth;
    const endYPos = Math.ceil(sceneEndPoint[1] / options.gridHeight) * options.gridHeight;

    for (let Xpos = startXPos + options.gridOffsetX; Xpos < endXPos; Xpos += options.gridWidth) {
    ...
    for (let Ypos = startYPos + options.gridOffsetY; Ypos < endYPos; Ypos += options.gridHeight) {
